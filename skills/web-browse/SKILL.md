...
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py goto <url>
```

### screenshot — Capture a page screenshot
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py screenshot [url] [/tmp/screenshot.png]
# With element annotations:
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py screenshot <url> /tmp/s.png --annotate
```

### snapshot — Get page elements with clickable refs
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py snapshot
# Interactive mode:
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py snapshot -i
```
Returns elements like `@e1 [button] "Submit"` — use refs with click/fill commands.

### click — Click an element by ref
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py click @e2
```

### fill — Fill a form field by ref
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py fill @e3 "user@example.com"
```

### text — Extract text from an element
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py text @e1
```

### markdown — Extract page content as clean markdown
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py markdown [url]
```

### search — Web search via browser
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py search "query terms"
```

### agent — LLM-driven autonomous browsing
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py agent "Find the latest Python release date"
# Or with BrowserAgent for auth-heavy tasks:
python3 $CLARVIS_WORKSPACE/scripts/tools/browser_agent.py agent "Log in and check notifications"
```

### upload — Upload a file
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/browser_agent.py upload /path/to/file.png [css-selector]
```

### status — Check browser connection
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py status
```

### Session management
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/browser_agent.py save-session [path]
python3 $CLARVIS_WORKSPACE/scripts/tools/browser_agent.py session-info [path]
# Auto-save session on exit:
python3 $CLARVIS_WORKSPACE/scripts/tools/browser_agent.py --persist navigate <url>
```
...
- **Always use full paths**: `python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py`
...
```bash
python3 $CLARVIS_WORKSPACE/scripts/tools/clarvis_browser.py screenshot https://github.com/trending /tmp/github_trending.png
```
