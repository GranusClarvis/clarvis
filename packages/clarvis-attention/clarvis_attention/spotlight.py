...
Features:
  - Salience scoring: importance + recency + relevance + access frequency + boost
  - Competition: items compete for spotlight slots; losers decay
  - Spreading activation: boost items related to a query
  - Broadcast hooks: register callbacks to receive spotlight contents
  - Optional file persistence: save/load state to JSON
...
class AttentionItem:
    ...
    def salience(self) -> float:
        ...
        Components:
          - importance (0.25): base priority of the item
          - recency (0.20): exponential decay, half-life ~6 hours
          - relevance (0.30): contextual relevance, decays when out of spotlight
          - access_count (0.10): log-scaled access frequency
          - boost (0.15): external boost, decays fast when out of spotlight
        """
...
class AttentionSpotlight:
    ...
    Args:
        capacity: Maximum items in spotlight (default 7, cognitive science's 7+/-2)
        persist_path: Optional path to a JSON file for state persistence.
                      If set, state auto-saves on mutations.
    """
    ...
    def __init__(
        ...
        self._broadcast_hooks: List[Callable[[str, List[Dict[str, Any]]], None]] = []
    ...
    def _save(self):
        """Persist state to disk if persist_path is configured."""
    ...
    def on_broadcast(self, callback: Callable[[str, List[Dict[str, Any]]], None]):
        """Register a callback for broadcast events.

        The callback receives (summary_text, spotlight_items_list).
        """
        self._broadcast_hooks.append(callback)

    def broadcast(self) -> str:
        """Broadcast spotlight contents via registered hooks.
    ...
    def add(self, content: str, importance: float = 0.5, source: str = "system") -> AttentionItem:
        """Working-memory-compatible add.

        High-importance items (>=0.8) get a boost for auto-focus.
        """
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        if not self.items:
            return {"total_items": 0, "spotlight_size": 0, "capacity": self.capacity,
                    "avg_salience": 0, "sources": {}}
