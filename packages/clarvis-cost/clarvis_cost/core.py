        ...
        Returns:
            {
                ...
                "remaining": float,
                "pct_used": float,
                "alert": str,  # "ok", "warning" (>75%), "exceeded"
            }
        """
        now = datetime.now(timezone.utc)
...
def analyze_savings(tracker: CostTracker, router_log_path: str = "") -> Dict:
    ...
    Reads the cost log and optionally the router decisions log to find:
    - Tasks routed to expensive models that could be cheaper
    - High-cost sources that need compression
    ...
    # Check router decisions for fallback rate
    ...
    # Input/output ratio — high output ratio means expensive generation
...
def import_router_decisions(router_log_path: str, tracker: CostTracker) -> int:
    ...
    Reads router_decisions.jsonl and creates cost entries for Gemini calls
    that have token counts. Claude Code calls get estimated costs.
    ...
    with open(router_log_path) as f:
        for line in f:
            try:
                ...
                # Estimate tokens from task length if not provided
                task = entry.get("task", "")
                est_input = estimate_tokens(task, model) + 500  # Add system prompt overhead
                est_output = 300  # Typical short response
