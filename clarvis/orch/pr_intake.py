...
def generate_sector_playbook(workspace: Path) -> dict:
    """Derive sector/product playbook from repo documentation."""
    playbook = {
        "sector": "",
        "constraints": [],
        "product_rules": [],
    ...
    SECTOR_SIGNALS = {
        "defi": ["defi", "swap", "liquidity", "amm", "yield", "staking", "vault"],
        "dao": ["dao", "governance", "proposal", "vote", "quorum", "delegation"],
        "gaming": ["game", "nft", "arena", "battle", "quest", "reward", "leaderboard"],
        "trading": ["trading", "order", "position", "margin", "slippage", "execution"],
        "social": ["social", "feed", "profile", "follow", "content", "moderation"],
        "fintech": ["payment", "transaction", "wallet", "balance", "transfer", "kyc"],
        "saas": ["subscription", "tenant", "billing", "plan", "usage", "api key"],
        "ai_ml": ["model", "inference", "training", "embedding", "pipeline", "dataset"],
    }
    ...
    for readme_name in ("README.md", "readme.md", "README.rst"):
        ...
        if readme.exists():
            try:
                text = readme.read_text()[:5000]
                all_text += text + "\n"
                playbook["doc_sources"].append(readme_name)
            except OSError:
                pass
            break
    ...
    if claude_md.exists():
        try:
            text = claude_md.read_text()[:5000]
            all_text += text + "\n"
            playbook["doc_sources"].append("CLAUDE.md")
        except OSError:
            pass
        ...
        try:
            for doc_file in sorted(docs_dir.iterdir()):
                if doc_file.is_file() and doc_file.suffix in (".md", ".rst", ".txt"):
                    try:
                        text = doc_file.read_text()[:3000]
                        all_text += text + "\n"
                        playbook["doc_sources"].append(f"docs/{doc_file.name}")
                    except OSError:
                        pass
                if len(playbook["doc_sources"]) > 12:
                    break
        except OSError:
            pass
    ...
    for sector, keywords in SECTOR_SIGNALS.items():
        score = sum(all_lower.count(kw) for kw in keywords)
        if score > 0:
            sector_scores[sector] = score

    if sector_scores:
        playbook["sector"] = max(sector_scores, key=sector_scores.get)

    constraint_patterns = [
        "must ", "must not ", "never ", "always ", "required ",
        "invariant:", "constraint:", "rule:", "safety:",
        "do not ", "forbidden", "mandatory",
    ]
    ...
    for line in all_text.splitlines():
        ...
        if stripped.startswith(("- ", "* ", "\u2022 ")):
            ...
            if any(p in lower for p in constraint_patterns):
                if body not in playbook["constraints"]:
                    playbook["constraints"].append(body[:200])

        if any(lower.startswith(tag) for tag in ("invariant:", "rule:", "constraint:")):
            val = stripped.split(":", 1)[1].strip()
            if val and val not in playbook["safety_invariants"]:
                playbook["safety_invariants"].append(val[:200])

    playbook["constraints"] = playbook["constraints"][:15]
    playbook["safety_invariants"] = playbook["safety_invariants"][:10]

    return playbook


def seed_sector_to_brain(agent_dir: Path, workspace: Path) -> int:
    """Generate sector playbook and seed it into the agent's LiteBrain."""
    playbook = generate_sector_playbook(workspace)
    if not playbook.get("sector") and not playbook.get("constraints"):
        return 0
    ...
    if playbook.get("sector"):
        lb.store_sector(
            f"Project sector: {playbook['sector']}",
            constraint_type="domain",
            importance=0.8,
            source="sector_playbook_gen",
        )
        count += 1

    for constraint in playbook.get("constraints", []):
        lb.store_sector(
    ...
    for inv in playbook.get("safety_invariants", []):
        lb.store_sector(
            f"Safety invariant: {inv}",
...
def refresh_artifacts(agent_dir: Path, workspace: Path) -> dict:
    ...
    artifact_generators = {
        ...
        "sector_playbook": generate_sector_playbook,
    }
