...
def backfill_ingest(candidates: list[dict], dry_run: bool = False) -> list[dict]:
    ...
    ingest_file = _load_script("wiki_ingest", "wiki").ingest_file
...
def backfill_compile(dry_run: bool = False) -> list[dict]:
    ...
    from clarvis._script_loader import load as _load_script
    compile_all_pending = _load_script("wiki_compile", "wiki").compile_all_pending
...
def backfill_sync(dry_run: bool = False) -> list[dict]:
    ...
    from clarvis._script_loader import load as _load_script
    _wbs = _load_script("wiki_brain_sync", "wiki")
    sync_changed, sync_all = _wbs.sync_changed, _wbs.sync_all
