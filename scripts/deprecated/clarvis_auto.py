...
from brain import brain as _brain, capture
...
def process_message(message: str, role: str = "user") -> dict:
    ...
    # 1. Extract what matters - store in memory via smart capture
    ...
    if message and len(message.strip()) > 3:
        try:
            related = _brain.recall(message, n=3)
            if related:
                results["related_memories"] = [r["id"] for r in related]
        except Exception:
    ...
    if message:
        try:
            ...
            for goal in goals:
                goal_text = goal.get("document", "")
                if any(word in message.lower() for word in goal_text.lower().split()[:3]):
...
def auto_start() -> dict:
    ...
    # Get recent memories
    try:
        recent = _brain.recall("recent", n=5)
        if recent:
            results["recent_memories"] = [r["id"] for r in recent]
    except Exception:
        pass
    ...
    try:
        goals = _brain.get_goals()
        results["pending_goals"] = [g for g in goals if g.get("metadata", {}).get("status") in ["pending", "in_progress"]]
    except Exception:
        pass
