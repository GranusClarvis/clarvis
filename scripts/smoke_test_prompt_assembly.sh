...
for script in cron_autonomous.sh cron_evening.sh cron_evolution.sh \
              ...
              cron_morning.sh spawn_claude.sh cron_env.sh; do
...
if grep -q 'PROMPT_GUARD' "$SCRIPTS_DIR/cron_autonomous.sh" 2>/dev/null; then
    pass "cron_autonomous.sh has PROMPT_GUARD"
else
    fail "cron_autonomous.sh missing PROMPT_GUARD"
fi
if grep -q 'PROMPT_GUARD' "$SCRIPTS_DIR/cron_env.sh" 2>/dev/null; then
