...
# Regex patterns for JS/TS exports
    ...
    for root, dirs, filenames in os.walk(workspace):
        ...
        for fname in sorted(filenames):
            ...
            if file_symbols:
                symbols.append({
                    "file": rel_path,
                    "symbols": file_symbols[:30],  # cap per file
                })
...
def _parse_js_symbols(content: str) -> list:
    ...
    for match in JS_EXPORT_RE.finditer(content):
        name = match.group(1)
        # Determine kind from the export line
        line = match.group(0)
        kind = "unknown"
        if "function" in line:
            kind = "function"
...
def build_route_index(workspace: Path, stack: Optional[dict] = None) -> dict:
    ...
    # Express/Flask/FastAPI routes from source files
...
def _scan_nextjs_routes(workspace: Path) -> list:
    ...
    for root, dirs, files in os.walk(app_dir):
        ...
        # Clean up Next.js route groups: (group) -> remove from path
...
def refresh_indexes(agent_dir: Path, workspace: Path,
    ...
    # Load stack from artifacts if not provided
    if stack is None:
...
def format_indexes_for_prompt(indexes: dict, max_tokens: int = 800) -> str:
    ...
    # File index summary (just counts by tag)
