...
if [ "$STUCK_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    python3 "$SCRIPTS/agents/agent_orchestrator.py" heal >> "$LOGFILE" 2>&1
fi
...
if ! grep -q '\[EXTERNAL_CHALLENGE:' "$QUEUE_FILE" 2>/dev/null || \
   ! grep -q '^\- \[ \] \[EXTERNAL_CHALLENGE:' "$QUEUE_FILE" 2>/dev/null; then
    python3 "$SCRIPTS/evolution/external_challenge_feed.py" inject >> "$LOGFILE" 2>&1 || true
fi
...
python3 "$SCRIPTS/pipeline/heartbeat_preflight.py" > "$PREFLIGHT_FILE" 2>> "$LOGFILE"
PREFLIGHT_EXIT=$?
...
if [ "$PF_STATUS" = "queue_empty" ] || [ "$PF_STATUS" = "no_tasks" ]; then
    ...
    COMPRESSOR="$SCRIPTS/tools/context_compressor.py"
...
PROJECT_DELIVERY_BLOCK
    ...
    python3 "$SCRIPTS/pipeline/execution_monitor.py" "$_output_file" "$_timeout" "$_claude_pid" >> "$LOGFILE" 2>&1 &
    local _monitor_pid=$!
...
" 2>> "$LOGFILE"
    ...
    TASK_CONTEXT="$CONTEXT_BRIEF" TASK_PROC_HINT="$PROC_HINT" TASK_EPISODE_HINT="$COMPRESSED_EPISODES" \
        python3 "$SCRIPTS/evolution/task_router.py" execute-openrouter "$NEXT_TASK" > "$TASK_OUTPUT_FILE" 2> "$OPENROUTER_STDERR"
    TASK_EXIT=$?
...
" 2>> "$LOGFILE"
    ...
    TASK_CONTEXT="$CONTEXT_BRIEF" TASK_PROC_HINT="$PROC_HINT" TASK_EPISODE_HINT="$COMPRESSED_EPISODES" \
        python3 "$SCRIPTS/evolution/task_router.py" execute-openrouter "$NEXT_TASK" > "$TASK_OUTPUT_FILE" 2>&1
    TASK_EXIT=$?
...
POSTFLIGHT_OUTPUT=$(python3 "$SCRIPTS/pipeline/heartbeat_postflight.py" "$TASK_EXIT" "$TASK_OUTPUT_FILE" "$PREFLIGHT_FILE" "$TASK_DURATION" 2>> "$LOGFILE")
POSTFLIGHT_EXIT=$?
...
GIT_AUTOFIX=$(python3 "$SCRIPTS/hooks/obligation_tracker.py" auto-fix 2>> "$LOGFILE")
