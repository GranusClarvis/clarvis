...
python3 "$SCRIPTS/pipeline/evolution_preflight.py" > "$EVO_PREFLIGHT_FILE" 2>> "$LOGFILE"
EVO_EXIT=$?
...
if [ "$EVO_EXIT" -ne 0 ]; then
    ...
    COMPRESSED_QUEUE=$(python3 "$SCRIPTS/tools/context_compressor.py" queue 2>> "$LOGFILE")
    COMPRESSED_HEALTH=""
...
EVO_CONTEXT_BRIEF=$(python3 "$SCRIPTS/tools/prompt_builder.py" context-brief --task "strategic evolution analysis" --tier full 2>> "$LOGFILE" || echo "")
