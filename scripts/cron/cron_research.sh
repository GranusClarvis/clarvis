...
# Arm outer script timeout (2700s = 45 min) — kills entire script on hang
set_script_timeout 2700 "$LOGFILE"
...
if [ -z "$RESEARCH_TASK" ]; then
    ...
    # Research auto-replenish: gated by durable config file (data/research_config.json).
    # The config file is the single source of truth; env var is a legacy override.
    # Check durable config first, fall back to env var for backward compat.
    RESEARCH_DISCOVERY_ALLOWED=$(python3 -c "
from clarvis.research_config import is_enabled
print('1' if is_enabled('research_discovery_fallback') else '0')
" 2>/dev/null || echo "0")

    if [ "$RESEARCH_DISCOVERY_ALLOWED" != "1" ]; then
        # Legacy env var override (if someone sets it explicitly)
        RESEARCH_AUTO_REPLENISH="${RESEARCH_AUTO_REPLENISH:-0}"
        if [ "$RESEARCH_AUTO_REPLENISH" != "1" ]; then
            echo "[$(date -u +%Y-%m-%dT%H:%M:%S)] Auto-replenish OFF (research_config.json); exiting without discovery fallback" >> "$LOGFILE"
            exit 0
        fi
    fi
...
NOVELTY_RESULT=$(python3 "$SCRIPTS/evolution/research_novelty.py" classify "$RESEARCH_TASK" 2>> "$LOGFILE")
NOVELTY_EXIT=$?
...
CONTEXT_BRIEF=$(python3 "$SCRIPTS/tools/prompt_builder.py" context-brief --task "$RESEARCH_TASK" --tier standard 2>> "$LOGFILE")

# Inject cross-run lessons (MetaClaw-inspired pattern)
PAST_LESSONS=$(python3 "$SCRIPTS/evolution/research_lesson_store.py" inject 2>/dev/null || echo "")
...
3. Store 3-5 insights: python3 scripts/brain_mem/brain.py remember 'I learned that [insight]' --importance 0.7 --collection clarvis-learnings
...
2. Store 3-5 insights: python3 scripts/brain_mem/brain.py remember 'I learned that [insight]' --importance 0.8 --collection clarvis-learnings
...
if [ "$TASK_EXIT" -eq 0 ]; then
    ...
    for research_file in "$RUN_DIR"/*.md; do
        ...
        FILE_NOVELTY=$(python3 "$SCRIPTS/evolution/research_novelty.py" evaluate-file "$research_file" 2>> "$LOGFILE")
        FILE_NOVELTY_EXIT=$?
...
if [ "$FINAL_RC" != "0" ] && [ "$FINAL_RC" != "$POSTFLIGHT_RC" ]; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%S)] EXIT_PRESERVE: task=${TASK_EXIT:-0} verifier=$POSTFLIGHT_RC -> exit $FINAL_RC" >> "$LOGFILE"
fi
