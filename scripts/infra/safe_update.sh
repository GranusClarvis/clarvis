...
import sys
sys.path.insert(0, '$SCRIPTS_DIR/brain_mem')
...
create_update_backup() {
  ...
  bash "$SCRIPTS_DIR/infra/backup_daily.sh" --full 2>&1 | tee -a "$LOG_FILE"
...
post_update_check() {
  ...
  local brain_check
  brain_check=$(python3 "$SCRIPTS_DIR/brain_mem/brain.py" health 2>/dev/null | head -n 5 || true)
  ...
  if [ -f "$SCRIPTS_DIR/brain_mem/brain.py" ] && [ -f "$SCRIPTS_DIR/brain_mem/working_memory.py" ]; then
...
do_rollback() {
  ...
  bash "$SCRIPTS_DIR/infra/backup_restore.sh" --latest --yes 2>&1 | tee -a "$LOG_FILE" || {
...
do_full_update() {
  ...
  if post_update_check; then
    ...
    echo "  [ ] Check brain recall: python3 scripts/brain_mem/brain.py recall 'test query'"
