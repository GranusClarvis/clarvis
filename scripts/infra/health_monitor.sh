...
python3 /home/agent/.openclaw/workspace/scripts/tools/daily_memory_log.py ensure >/dev/null 2>&1 || true
...
if [ "$BRAIN_STALE" = true ]; then
    python3 /home/agent/.openclaw/workspace/scripts/brain_mem/brain_hygiene.py check >> "$LOG_DIR"/health.log 2>&1 || {
