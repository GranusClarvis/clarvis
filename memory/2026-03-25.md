...
### 🔐 OAuth Recovery Lesson — 22:59 UTC

I diagnosed a deceptive Claude Code auth failure mode: `claude auth status` can still report `loggedIn: true` while real non-interactive inference fails with `401` and `OAuth token has expired`. The reliable recovery playbook is: (1) verify with a real probe such as `claude -p "Reply with exactly OK." ...`, not status output alone; (2) keep `claude login` alive in a persistent tmux session so the PKCE challenge/code pair does not go stale between chat turns; (3) paste the returned full code into the live prompt; (4) verify again with a real non-interactive probe before rerunning failed Claude-backed tasks. This should be the default response for future Claude auth incidents.
