...
- [x] **[BB_VERIFY_PAGE_ERROR_STATES]** Tighten `apps/web/src/app/verify/[betId]/page.tsx` error/edge handling: missing/invalid `betId` (route param), API 404 from `/api/verify/[betId]`, parity-mismatch state (recomputed commit ≠ on-chain commit), stale-revealed-but-unsettled state, and network-error fallback. Each branch must have a visible UI state. Add unit tests (Vitest + React Testing Library) under `apps/web/src/app/verify/[betId]/__tests__/page.test.tsx` covering all five branches. Acceptance: tests green via `pnpm --filter @bunnybagz/web test`. (PROJECT:BUNNYBAGZ) — DONE 2026-04-30 (bunnybagz commit `b81eff3` on `feature/mvp-planning-and-rebrand`); 8/8 vitest pass + typecheck clean. Vitest+RTL+jsdom now wired into apps/web; 5 branch test-IDs (`verify-invalid-bet-id`, `verify-not-found`, `verify-mismatch`, `verify-stale-unsettled`, `verify-network-error`) plus happy-path. NOTE: API edge fn `apps/api/verify/[betId].ts` still returns only `pending-indexer` — page is now ready to render `stale-unsettled` once that handler emits it (Phase 1 indexer follow-up).

- [x] **[BB_GEO_ENFORCE_FLAG]** `apps/api/geo/index.ts` currently ships in log-only mode. Add a `GEO_ENFORCE=1` env-flag enforce path that returns HTTP 403 `{country, blocked: true}` for blocked regions, leaves log-mode behavior unchanged when the flag is unset, and unit-tests both modes. Acceptance: `pnpm --filter @bunnybagz/api test` green; toggling the env flips the response without code change. (PROJECT:BUNNYBAGZ) — DONE 2026-04-30 (bunnybagz commit `b1ff46a` on `feature/mvp-planning-and-rebrand`); 7/7 node:test pass via `tsx --test`. Extracted pure `decide(country, mode)` so policy is testable without a Request. Test runner switched from never-actually-run `node --experimental-strip-types` shim to `tsx --test` (Node 20.20.2 on the agent box has no native TS strip). NOTE: the legacy `globalThis.BUNNYBAGZ_GEO_MODE` runtime override is gone — `process.env.GEO_ENFORCE` is the sole switch.
