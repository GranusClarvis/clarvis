...
- [x] **[BB_HILO_CONTRACT_PHASE2_REAL]** ([SHIPPED] 2026-05-03 commit `70b53c9` on `feature/mvp-planning-and-rebrand`). `BunnyBagzHiLo.sol` + `BunnyBagzHiLo.t.sol` shipped with full per-step commit-reveal lifecycle (openSession / playStep / cashOut / refundSession), wad-scaled cumulative multiplier, and `(12/(13-c))*0.99` formula on both Higher and Lower with ties → house. `forge test` → 102 passed, 0 failed (25 unit/fuzz HiLo + 2 HiLo invariant runs at 64×64 with zero reverts). `packages/verify` parity suite extended 22 → 40 tests (added hilo expectation to v1..v3 + new vectors v4..v6 covering low/mid/high cards). Side fixes: `BunnyBagzDice.t.sol` MIN/MAX_ROLL_UNDER references; `Deploy.s.sol` NatSpec parser fix. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_PLAYWRIGHT_E2E_SETUP_REAL]** ([REOPENED] from `[BB_PLAYWRIGHT_E2E_SETUP]`, drift confirmed 2026-05-01 + 2026-05-02). `apps/web/package.json` has zero playwright deps; no `playwright.config.*`; no spec. Phase 2 exit criterion per `docs/ROADMAP.md`. Install `@playwright/test`; ship `apps/web/playwright.config.ts` (Chromium + Mobile Safari emulation, 30s timeout, retries=1 in CI) and `apps/web/e2e/coinflip.spec.ts` walking: load `/`, click connect (mocked wallet via `playwright`'s `page.addInitScript` + a wagmi mock), navigate to `/play`, click Coinflip card, set stake, tap Heads, see settled outcome (mocked api/seed). Add CI job that runs `pnpm --filter @bunnybagz/web exec playwright test` on Linux headless. **Acceptance:** spec runs green locally + in CI; deps lockfile updated; new CI job in `.github/workflows/ci.yml`. (PROJECT:BUNNYBAGZ)

#### BunnyBagz — process + verification (P1, autonomous-doable)

- [x] **[BB_PHASE1_VERIFICATION_PASS_RECURRING]** DONE 2026-05-03. Shipped `scripts/audit/bb_phase_verification.py` (parses QUEUE_ARCHIVE for `[x] [BB_*]` items in last N days, verifies cited commits exist in `~/agents/mega-house/workspace` git log, verifies cited `apps/`/`packages/` paths exist, optional `--run-tests` fires `pnpm --filter @bunnybagz/<pkg> test`, suppresses items already acknowledged via `[REOPENED] from`-pattern). Cron wrapper `scripts/cron/cron_bb_phase_verification.sh` acquires `/tmp/clarvis_maintenance.lock` and is wired into `clarvis cron` as `bb_phase_verification` running `35 4 * * 0` (Sunday 04:35 CET, +5 min from 04:30 to dodge graph_compaction's lock window) and added to `recommended` + `research` + `full` presets. First weekly run produced `memory/cron/bb_phase_verification_2026-05-03.md` (37 items audited, 0 drift). Planted-drift integration test in isolated `/tmp/bb_test_workspace`: synthetic archive with fake commit `feedbeef9876543` + fake file path → script flagged drift, wrote report, and appended `[BB_PLANTED_FAKE_REAL]` to QUEUE.md via `clarvis.queue.writer.add_task` (the writer exposes `add_task`, not `append_task` — used the actual API). (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_END_TO_END_UI_REVIEW]** Single-shot UI review pass over the live BB web surface — does the 30-second flow per UX_PLAN §2 actually feel right? Use the existing `clarvis_browser.py` (Playwright CDP path) to: visit `localhost:3000/`, screenshot LCP at 1.0s, navigate `/play`, screenshot, tap Coinflip card, screenshot, simulate keyboard-only (Tab → Enter), screenshot, take a screenshot under both dark and light themes, take a screenshot at 375px-wide viewport. Save under `memory/cron/bb_ui_review_<YYYY-MM-DD>/`. Then write a 10-line review verdict scoring each surface 1-5 against UX_PLAN §3 layout primitives + §6 visual system + §7 accessibility. **Acceptance:** review folder exists with ≥6 screenshots; verdict file at `memory/cron/bb_ui_review_<YYYY-MM-DD>.md` lists ≥3 concrete issues with file:line references where the fix should land; if any issue is `severity: high`, auto-append a `[BB_UI_<TAG>_FIX]` task to QUEUE.md. (PROJECT:BUNNYBAGZ)
