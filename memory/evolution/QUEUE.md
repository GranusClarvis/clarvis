...
- [x] **[BB_MASCOT_PLACEHOLDER_REAL]** ([REOPENED] from `[BB_MASCOT_PLACEHOLDER_ART]`) Real SVG mascots at `apps/web/public/mascot/{idle,win,loss-streak}.svg`, each with `role="img"` + `aria-label` + `viewBox`. Idle frame rendered on `/` as decorative (`alt=""`). **Verification gate (BEFORE archiving):** confirm `apps/web/src/app/__tests__/mascot-assets.test.ts` passes (file presence + size bounds + valid SVG markup), AND the win-state mascot is wired into `/play/coinflip` settle flow per UX_PLAN §5 ("BunnyBagz mascot does a 2-frame celebrate animation top-right" — currently NOT wired; idle frame on `/` only). The win/loss-streak SVGs ship in `de58447` but only idle is mounted; mascot-on-settle is a follow-up. (PROJECT:BUNNYBAGZ) — **Done 2026-05-01**: mascot-assets.test.ts passes (3/3 variants × 2 assertions); `MascotCelebrate.tsx` mounted in `/play/coinflip` (win mascot top-right, 600ms 2-frame keyframe, reduced-motion → static frame). New `MascotCelebrate.test.tsx` (5 tests) covers (a) win-on-pick-match, (b) reduced-motion static variant, (c) loss does not mount, (d) pending does not mount. Web suite 69/69.
...
- [x] **[BB_MASCOT_WIN_CELEBRATE_WIRE]** Wire the `win.svg` mascot into the `/play/coinflip` settle flow per UX_PLAN.md §5. On a winning settle, render the win mascot top-right of the coin viewport with a 2-frame celebrate animation (framer-motion, ≤600ms, capped by reduced-motion preference per existing `useReducedMotionPreference` hook in `CoinSpin.tsx`). Static idle frame stays on the lobby/landing. **Acceptance:** `apps/web/src/components/__tests__/CoinSpin.test.tsx` (or new `MascotCelebrate.test.tsx`) covers (a) win mascot mounts when `result==="heads"` AND user picked heads, (b) reduced-motion suppresses the celebrate animation but still mounts the static frame, (c) loss does NOT mount the win mascot. (PROJECT:BUNNYBAGZ) — **Done 2026-05-01** alongside `[BB_MASCOT_PLACEHOLDER_REAL]`: new `MascotCelebrate.tsx` consumes shared `useReducedMotionPreference` (now exported from `CoinSpin.tsx`); 600ms framer-motion keyframe (opacity/scale/y), top-right of relative-positioned coin viewport wrapper. Test file `MascotCelebrate.test.tsx` ships all three acceptance cases plus tails-win and pending-state coverage.
