...
- [ ] [OSR_CRON_INSTALLER] Add guided cron/system-scheduler installation so users can select which Clarvis cron jobs to enable. Include dry-run preview, install/update/remove flows, and safe defaults for a first-time setup.
- [ ] [OSR_CRON_PROFILE_PRESETS] Define cron presets (`minimal`, `recommended`, `full`, maybe `research-heavy`) mapping to actual scripts/schedules so install is selectable instead of hand-edited.
...
- [ ] [CLEANUP_POLICY_JSONL_COVERAGE] Extend `cleanup_policy.py` to cover all 20+ unbounded JSONL files discovered in `data/` subdirectories (broadcast_log, obligations_log, daily_brain_eval/history, calibration/predictions, etc.) — current rotation only handles a subset. Add a `JSONL_DIRS` scan that auto-discovers `.jsonl` files >500KB and applies the same gzip+age policy as existing rotation.
