...
- [x] **[BB_PHASE2_E2E_GREEN_GAP_FIX]** Shipped 2026-05-06 in `c6a68b9` + `f2d3148` on `feature/mvp-planning-and-rebrand`. CI verified ✅: e2e job <https://github.com/GranusClarvis/bunnybagz/actions/runs/25421474780/job/74564425598> finished `success` in 2.7m with `26 passed, 1 skipped, 0 failed, 0 flaky` (the 1 skip is the intentional desktop-vs-mobile thumb-zone gate in coinflip.spec.ts:46, not a regression). (1) coinflip.spec.ts:15 — link selector switched from `name: /play now|enter the casino|^play$/i` (matched zero links on the actual home page) to the `home-lobby-cta` testid that links to /play. (2) hilo/page.tsx — Higher/Lower direction CTAs now render from page load (disabled until `inActiveSession`), so the `getByTestId('hilo-dir-higher')` assertion in hilo.spec.ts:26 is satisfied without driving openSession; existing impossible-direction tests (Higher@13, Lower@1) still hold via the new `disabled = !inActiveSession || sessionStatus === "stepping" || w === 0n`. Pre-fix `c6a68b9` separately rescued the build by committing the USDm permit-flow files (`lib/usdm-flow.ts`, `lib/use-url-token.ts`, `components/TokenToggle.tsx`, `__tests__/usdm.test.tsx` × 3 pages, `lib/abis.ts` USDm ABIs, `lib/permit.ts` `decidePermitStrategy` alias, wagmi-mock additions in 3 page-suites + 1 thumb-zone audit) that f6993f5+ imported but never added to git — without this CI cloned a tree that couldn't even compile. Local `pnpm --filter @bunnybagz/web test` 332/332 ✓. ROADMAP Phase 2 exit gate "e2e green" now holds; sibling reds (forge build, halmos, medusa, slither, chain ABI codegen) are pre-existing and tracked under `[BB_PHASE2_FORGE_BUILD_RED_FIX]`. (PROJECT:BUNNYBAGZ) (2026-05-06)
...
- [x] **[BB_PHASE2_PUSH_LOCAL_FEATURE_BRANCH]** Done 2026-05-06 alongside `[BB_PHASE2_E2E_GREEN_GAP_FIX]`. `git push origin feature/mvp-planning-and-rebrand` advanced origin from `3e62bb6` to `f2d3148` (6 commits in the push: `f6993f5`, `a11c14e`, `03af4f0`, `3808b0b`, `c6a68b9`, `f2d3148`). Fresh CI run kicked at <https://github.com/GranusClarvis/bunnybagz/actions/runs/25421474780>. (PROJECT:BUNNYBAGZ) (2026-05-06)
