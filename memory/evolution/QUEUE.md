...
- [x] **[BB_PHASE2_HILO_FRONTEND_REAL]** Shipped 2026-05-04 (commit `8f5482b` on `feature/mvp-planning-and-rebrand`). Real `apps/web/src/app/play/hilo/page.tsx` replaces the Phase-2 placeholder with the full keeper-aware state machine: open-session form (stake + per-bet `/api/seed/claim` commit, mirroring the coinflip / dice CommitAlreadyUsed guard), current-card display, Higher/Lower step picker (impossible boundaries — Higher from 13, Lower from 1 — disabled), cumulative-multiplier track, live cash-out CTA labelled with the current multiplier, refund-expired-session escape hatch, aria-live region announcing every transition (opened → stepping → cashed-out / lost / pushed / refunded — the pushed copy reflects the tie-as-push semantics that anchor the published 1% edge). Player only signs `openSession` / `cashOut` / `refundSession`; keeper drives `playStep` off-chain and dispatches `bunnybagz:hilo-{opened,step,settle}` custom DOM events back into the page. 19 new vitest cases cover the full state machine + the pure exported `quoteStepMultiplierWad` / `winChanceFor` helpers (`12 * 99 / ((CARD_MOD − c) * 100)` mirroring `BunnyBagzHiLo.quoteStepMultiplier`). apps/web stays green at 188/188. (PROJECT:BUNNYBAGZ) (2026-05-03)
