...
- [x] **[BB_PHASE3_BANKROLL_CIRCUIT_BREAKER]** 2026-05-05 — Drawdown circuit-breaker shipped contract-side. `BunnyBagzBankroll.sol` adds `CircuitBreaker { maxDrawdown24hWei, windowStartBalance, windowStartedAt, drawdownStartedAt, halted }` + `tripCircuitBreaker`/`resetCircuitBreaker`/`pokeCircuitBreaker`/`setMaxDrawdown24hWei` + `whenNotHalted` modifier on `payout()`; auto-trip after a payout that crosses the rolling-24h threshold; auto-reset / permissionless poke after the 24h cooldown. New `BunnyBagzBankrollCircuitBreaker.t.sol` ships 19 forge tests (trip-on-threshold, owner-only manual trip, payout-reverts-when-halted, settle-stays-open, auto-reset-after-cooldown, poke-cooldown gates, window-roll, threshold-zero disables auto-trip, pause+halt interaction). Full forge suite green: 147 / 147 (incl. invariants 64×4096 calls, 0 reverts). `Deploy.s.sol` wires `maxDrawdown24hWei = BB_MAX_DRAWDOWN_24H || initialDeposit/2`, idempotent across re-runs. UX_PLAN §11 documents trip-recovery operator playbook. Slither/aderyn/medusa not run locally (no toolchain) — left to CI; halmos halted-invariant proof deferred to a follow-up task. (PROJECT:BUNNYBAGZ)
