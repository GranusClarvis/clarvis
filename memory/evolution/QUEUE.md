...
- [ ] **[BB_PHASE2_HILO_TIE_PUSH_OR_EDGE_DOC]** HiLo's published "1% house edge" is mis-stated. With cards drawn with replacement and ties going to the house, the contract's `(12 / (13 - c)) * 0.99` formula gives an actual ≈8.5% edge per step, not 1%. Either (a) flip ties to a "push" refund branch in `BunnyBagzHiLo.playStep` (recommended — preserves advertised odds, ship a `BetPushed` event for the indexer), or (b) update `docs/MVP_GAMES.md` and the contract's docstring to publish the actual edge. Acceptance: contracts test `testFuzz_houseEdge_holds()` running 1k+ trials against a target edge ±0.5%; docs updated. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_SEED_LIFECYCLE_API_CLEANUP]** Now that `/api/seed/claim` is the canonical write path (per the commit-replay fix), the legacy "active session" model in `apps/api/seed/{rotate,commit}.ts` is redundant. Either deprecate `/seed/commit` GET (return null + deprecation header) or remove it entirely; `ROTATE_AFTER_BETS` and the `bets` counter on `ActiveSession` are dead code. Frontend no longer needs a page-level commit. Acceptance: `apps/api/seed/commit.ts` either deleted or returns 410 Gone; lifecycle.test.ts updated; no callers remain in `apps/web/src/app/play/**`. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_SLITHER_CI]** Wire slither into CI for `packages/contracts`. Suppress the intentional Phase 1 patterns (single-key owner, etc.) in a `.slither.config.json` so the noise floor is zero. Acceptance: new CI job; first run green; slither version pinned. (PROJECT:BUNNYBAGZ) (2026-05-03)
...
- [ ] **[BB_PHASE2_UINT96_STAKE_GUARD]** Defense-in-depth: validate `newMax <= type(uint96).max` in `setBetBounds` for Coinflip, Dice, HiLo. Today the casts `uint96(msg.value)` in `placeBet`/`openSession` would silently truncate if maxBet were ever set above `2^96 - 1`. Acceptance: 3 contract-level tests asserting the revert; ABI regen. (PROJECT:BUNNYBAGZ) (2026-05-03)
...
- [ ] **[BB_PHASE2_ALLOWANCE_INVARIANT]** Encode `allowance ≥ ((PAYOUT_NUM - PAYOUT_DEN) * maxBet) / PAYOUT_DEN` as a contract-level guarantee so the operational footgun fixed live during the 2026-05-03 night audit pass cannot recur. Two acceptable patterns: (a) add a `view` helper on each game (`requiredAllowance() returns (uint256)`) that the deploy script asserts against the bankroll's `allowanceOf(this)` post-`registerGame`; or (b) reject `setBetBounds` calls that would push `requiredAllowance > allowanceOf(this)`. (a) is simpler and keeps the bankroll de-coupled. Acceptance: contract test asserts `requiredAllowance()` for Coinflip (1.96e15 at maxBet=2e15), Dice (`(PAYOUT_NUM/(rollUnder_min - 1) - 1) * maxBet`), HiLo (operator-tunable cashOut cap); deploy script asserts post-deploy. Reference: `docs/PHASE1_REPORT.md` § HIGH (allowance shortfall). (PROJECT:BUNNYBAGZ) (2026-05-03)
