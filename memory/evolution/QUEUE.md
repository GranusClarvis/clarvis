...
- [ ] **[SWO_SANCTUARY_COMPANION_SPRITE]** **ASSET-ONLY — code infrastructure is done.** Need 10 constellation sprite sets (aether, spectra, solveil, nebulu, chroma, rose, monflare, auracore, parallel, prime) × 6 moods (happy, excited, calm, sleepy, curious, idle) = 60 PNGs at 32×32 native. `CompanionSprite` component already renders them with pixelated scaling + mood-based CSS animation. **Style: must pass doctrine §8 checklist — charming purple-base blob/star forms with constellation accent colors (see doctrine §3 table). NOT dark/menacing.** Asset workflow: Retro Diffusion or Aseprite, NOT AI image generators. Drop PNGs into `public/sanctuary/companions/{constellation}/{mood}.png`. (PROJECT:SWO)
...
- [ ] **[SWO_SANCTUARY_EMPTY_STATES]** Improve empty/zero states: (1) no-companion-selected needs `CompanionSprite` silhouette + "Select your Skrumpey" CTA, (2) empty journal: pixel-art open book illustration, (3) quests empty: fix "Loading quests..." permanent state when API returns empty array, (4) traits empty: encouraging progress message. **Asset-dependent items (illustrations) can use CSS-only placeholders until sprites arrive.** (PROJECT:SWO)
