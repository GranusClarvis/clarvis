...
- [x] **[BB_PHASE2_CHAIN_ABIS_REGEN]** [VERIFIED LOCAL] Regenerated `packages/chain/src/abis/bankroll.ts` via `pnpm --filter @bunnybagz/chain codegen:abis` (script is `codegen:abis`, not `regen:abis`). Bankroll ABI was missing the recently-shipped circuit-breaker surface (`CIRCUIT_BREAKER_COOLDOWN`, `circuitBreaker`, `circuitBreakerState`, etc., +198 lines). Local `pnpm --filter @bunnybagz/chain check:abis` now reports "OK — 4 contract(s) checked, 0 placeholder(s)". Pushed as commit `c57985c` on `feature/mvp-planning-and-rebrand`. **Acceptance:** `chain-abis` job green on the next CI run — needs CI confirmation. (PROJECT:BUNNYBAGZ) (2026-05-06)
