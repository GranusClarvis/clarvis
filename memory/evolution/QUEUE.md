...
- [x] **[BB_PHASE2_CHAIN_ABIS_REGEN]** [VERIFIED CI GREEN] Regenerated `packages/chain/src/abis/bankroll.ts` via `pnpm --filter @bunnybagz/chain codegen:abis` (script is `codegen:abis`, not `regen:abis`). Bankroll ABI was missing the recently-shipped circuit-breaker surface (`CIRCUIT_BREAKER_COOLDOWN`, `circuitBreaker`, `circuitBreakerState`, etc., +198 lines). Local `check:abis` reports "OK — 4 contract(s) checked, 0 placeholder(s)". Pushed as commit `c57985c` on `feature/mvp-planning-and-rebrand`. **Acceptance MET:** CI run 25431401209 — `packages/chain (ABI codegen ↔ contracts sync)` job: success ✓. (PROJECT:BUNNYBAGZ) (2026-05-06)
