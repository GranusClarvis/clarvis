...
- [ ] **[P0_MIRROR_REPO_SLUG_DOT_SAFE]** `scripts/agents/project_agent.py::_extract_gh_repo()` uses `r'[:/]([^/]+/[^/.]+?)(?:\.git)?$'`, which fails for valid GitHub repo names containing dots (for example `owner/repo.name`). This can break mirror-gate PR flagging/comment/close flows because `gh_repo` becomes empty. Replace with a robust parser that supports SSH/HTTPS remotes and dotted repo names; add tests covering `.git` and dot-name variants. (PROJECT:CLARVIS)
- [ ] **[P0_SYNC_WORKSPACE_STASH_RECOVERY_TESTS]** `scripts/cron/cron_env.sh::sync_workspace()` now surfaces stash-pop failure loudly, but it still leaves recovery as a manual shell step and has no regression coverage for the dirty-tree + merge-abort / merge-success paths. Add deterministic tests or harness coverage for stash creation/pop failure handling and ensure cron callers surface non-zero exit clearly without silently skipping downstream work. (PROJECT:CLARVIS)
