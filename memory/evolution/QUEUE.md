...
- [ ] **[CRON_AUDIT_CRONTAB_PERMISSION_ERROR]** `scripts/cron/cron_schedule_audit.sh` currently swallows `crontab -l` permission/read failures via `crontab -l 2>/dev/null || true`, which can turn a real audit error into an empty-source "clean" or misleading drift result. Make unreadable crontab an explicit audit failure (exit 2) with a logged reason unless `CRON_AUDIT_CRONTAB_FILE` override is in use. **Acceptance:** a fixture/test simulating `crontab -l` permission denial fails loudly, logs the error cause, and cannot report zero-drift from an unreadable live crontab. (PROJECT:CLARVIS)
