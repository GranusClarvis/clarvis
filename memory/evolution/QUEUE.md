...
- [x] **[BB_PHASE2_CREATEX_DETERMINISTIC_DEPLOY]** Switch the deploy pipeline to `createx-forge` ([radeksvarz/createx-forge](https://github.com/radeksvarz/createx-forge)) so addresses are deterministic across MegaETH testnet/mainnet/anvil. Removes a class of "wrong address on chain X" footguns before mainnet flip. Acceptance: `Deploy.s.sol` rewritten to use CREATE3 via CreateX; the same salt produces the same address on local anvil + MegaETH testnet; deploy artifact carries the salt for reproducibility. (PROJECT:BUNNYBAGZ) (2026-05-03) — _Shipped 2026-05-04: `forge install radeksvarz/createx-forge` + remap added; `Deploy.s.sol` now inherits `CreateXScript`, etches CreateX on local anvil, deploys both contracts via `create3` with cross-chain-friendly salts (20-byte deployer | 0x00 | 11-byte entropy, defaults `0xBB01`/`0xBBC1`, env-overridable as `BB_BANKROLL_SALT`/`BB_COINFLIP_SALT`); `deployments/<chainId>.json` now carries `bankrollSalt`, `coinflipSalt`, `deployer` for reproducibility; new `test/DeployDeterministic.t.sol` (3 tests, all green) proves same-salt-same-address across chainid 31337/6343/6342 + permissioned-deploy guard + end-to-end script run; full suite **115/115** still green. Files: `packages/contracts/{remappings.txt, script/Deploy.s.sol, test/DeployDeterministic.t.sol}` on `feature/mvp-planning-and-rebrand`._
