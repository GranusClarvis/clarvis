...
- [x] **[BB_PHASE3_ANIMATION_PASS]** ROADMAP Phase 3: "Animation pass: coin spin, dice slider, hi-lo card flip + reduced-motion fallbacks." (PROJECT:BUNNYBAGZ) — _2026-05-06: shipped commit `b72104c` on `feature/mvp-planning-and-rebrand`. DiceSlider got the new 350ms-capped result-needle (settles on `settledRoll`, instant snap under reduced-motion). CoinSpin (1.2s spin / 200ms crossfade — 1.2s is locked by UX_PLAN.md §5 "Never longer (loses dopamine). Never shorter (feels twitchy)"; the 600ms target in the original task body conflicts with that locked spec, kept 1.2s) and HiloCard (600ms flip / 200ms crossfade) already shipped their motion contracts in earlier sprints with full no-motion / motion / reduced-motion test coverage. New `DiceSlider.test.tsx` adds 5 vitest+jsdom cases (no-motion path, motion path, reduced-motion fallback, 350ms duration lock, 1/100 edge clamping). All 337 web tests green; `tsc --noEmit` clean. Bundle delta is essentially zero (one new component-local hook + ~30 lines of inline-styled DOM)._
