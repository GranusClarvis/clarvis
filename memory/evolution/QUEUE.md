...
- [x] **[BB_WALLET_SHEET_MVP]** Minimal `/wallet` modal/slide-over per `docs/UX_PLAN.md §1 Surfaces`. Shows: connected address, ETH balance, Fluffle-holder badge placeholder (no economic perk), light/dark theme toggle. No deposit/withdraw flow yet (Phase 2). Acceptance: header `⋯` opens the sheet on mobile; theme toggle persists to `localStorage`; vitest covers open/close. (PROJECT:BUNNYBAGZ) — DONE 2026-04-30: `apps/web/src/components/WalletSheet.tsx` + `useThemeToggle` hook (localStorage `bunnybagz:theme`); `SiteHeader.tsx` adds `⋯` trigger; tests `WalletSheet.test.tsx` (9) + `SiteHeader.test.tsx` (1) cover open/close, Esc/backdrop, theme persist+hydrate, address/balance, fluffle placeholder. Web suite 31/31 green, typecheck clean.
...
- [ ] [UNVERIFIED] **[HEARTBEAT_NO_TASK_RATE_BASH_TRIAGE]** Non-Python: execution-shape warning shows 46% of last 24 heartbeats produced no task. Write `workspace/scripts/cron/heartbeat_notask_triage.sh` that scans `monitoring/heartbeat.log` (or equivalent) for the last 7 days and buckets the `no-task` cycles by reason: `queue_empty`, `all_filtered`, `lock_held`, `gate_skip`, `unknown`. Output a markdown table to `memory/cron/heartbeat_triage.md` and append a one-line summary to the daily digest. No code changes to the heartbeat itself — purely a diagnostic to prove or disprove "investigate queue filters" before changing filter logic. Acceptance: bash script exists, executable, runs in under 5s on the live log; first run produces a populated bucket table; cron preset wires it daily at 06:25. (PROJECT:CLARVIS)
