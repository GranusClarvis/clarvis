...
_**Test inventory (all green as of 2026-04-30 evening polish pass):** `@bunnybagz/verify` 22/22 (+3 since audit: dice/hilo helper coverage), `@bunnybagz/api` 36/36 (was 30; +6 new since audit), `@bunnybagz/web` 44/44 (was 16 → 31 after wallet sheet → 44 after polish pass: +13 covering `/`, `/play` lobby, `/wallet` standalone). Foundry suite extends with `BunnyBagzBankroll.t.sol` + `BunnyBagzDice.t.sol`. All TS typechecks pass._

_**Open testing gaps (post-audit):** Playwright e2e (Phase 2 exit), local anvil deploy smoke, USDM behavior tests, animation/reduced-motion regression test, accessibility audit (contrast + keyboard + screen-reader), no mascot art assets, no Tailwind/Framer Motion polish stack (framer-motion is installed but Tailwind isn't wired). **NEW (2026-04-30 evening, this session):** landing surface (`/`), game lobby (`/play`), and standalone wallet route (`/wallet`) shipped — operator-test entry points are now navigable. Dice + Hi-Lo render as Phase-2-locked cards on the lobby._
...
- [x] **[EPISODE_CALIBRATION_FIELD]** Targets the weakest metric (Episode Success Rate=0.868, target 0.85, margin=0.018). Episodes in `data/episodes.json` currently store `outcome: success|failure` as a binary label, but `clarvis.cognition.confidence.outcome()` already produces a continuous calibration score that is discarded at episode-encode time. Wire `clarvis/heartbeat/episode_encoder.py` to persist `calibration_score: float` and `confidence_band: low|medium|high` on each episode, then add a 7-day rollup CLI (`python3 -m clarvis brain episodes calibration-report`) that flags weeks where mean calibration < success-rate by ≥0.05 (over-counted-success symptom). Acceptance: new fields present in episodes written after merge; rollup CLI returns JSON; one regression test confirms field is written when `conf_outcome` is available and skipped (with a logged reason) when it is None. (PROJECT:CLARVIS) (targets Episode Success Rate) — DONE 2026-05-01: `EpisodicMemory.encode()` accepts `calibration_score`/`confidence_band` kwargs; `clarvis/heartbeat/episode_encoder._derive_calibration()` computes Brier-style score `1-(c-int(correct))**2` from `conf_outcome` dict and bands confidence at <0.5 / <0.8 / ≥0.8; `scripts/pipeline/heartbeat_postflight.py` captures `conf_outcome()` return into `ctx["conf_outcome_result"]` and forwards to `_episode_encode`; new `python3 -m clarvis brain episodes calibration-report --days 7 --gap 0.05` CLI returns JSON week rollup with flagged weeks; 13 regression tests in `tests/test_episode_calibration_field.py` (all pass), including the required "field written when conf_outcome available" + "skipped with logged reason when None" cases.
- [ ] **[POSTFLIGHT_IMPORT_MISS_TELEMETRY]** `scripts/pipeline/heartbeat_postflight.py` has 8+ `try: from … import X except ImportError: X = None` blocks (conf_outcome, pred_resolve_enhanced, close_chain, record_use, learn_from_task, EpisodicMemory, write_digest, log_decision). When any of these silently resolve to None, episode outcome enrichment degrades without any signal in `data/audit/`. Add a single startup probe that records each None-resolved hook to `data/audit/postflight_import_misses.jsonl` (one line per heartbeat: `{ts, missing: [names]}`) and surfaces the count in the existing 22:30 digest report. Acceptance: file appears on first heartbeat after merge; entry written iff ≥1 import is None; digest line `postflight_import_misses_24h: N` shows in the next report. Indirectly defends Episode Success Rate by surfacing silent encoder degradation. (PROJECT:CLARVIS)
- [ ] **[DIGEST_ACTIONABILITY_BASH_GUARD]** (non-Python) Phase 12 audit found digest actionability at 56.5% (REVISE). Add a bash regression guard `scripts/maint/digest_actionability_check.sh` that greps `memory/cron/digest.md` for the patterns `^- \[ \]`, `^Next:`, or `^Action:` and exits non-zero (with a one-line append to `monitoring/digest_actionability.log`) when zero matches are found. Schedule via crontab at 22:35 (right after the 22:30 digest writer) so a garbled or actionability-empty digest paginates an alert into the next morning planning slot. Acceptance: shell script with `set -euo pipefail`, exits 0 on a healthy digest fixture and 1 on a stripped fixture (both committed under `tests/fixtures/digest_*.md`); cron entry added to `clarvis cron presets` recommended preset. (non-Python)
- [ ] **[QUEUE_V2_FILTER_DROP_REASON_LOG]** The 2026-04-30 bash triage found 100% of no-task heartbeats are `all_filtered_by_v2`, but `clarvis/orch/queue_engine.py::ranked_eligible()` does not record per-item drop reasons — only the aggregate count. Add a `--debug-drops` flag (and matching env `CLARVIS_QUEUE_DEBUG_DROPS=1`) that writes one line per filtered item to `data/audit/queue_filter_drops.jsonl`: `{ts, task_id, reasons: [cap_p0|cap_p1|dedup|lane_gate|stale|cooldown|...]}`. Then add a 7-day histogram CLI (`python3 -m clarvis queue drop-histogram`) so we can see which gate is chronically over-aggressive. Acceptance: flag-gated (off by default to keep prod free of extra writes), one unit test per drop-reason path, histogram CLI returns sorted reason counts. Pairs with `[ACTION_ACCURACY_PRECHECK_GUARD]` to attribute "action" failures to the right upstream cause. (PROJECT:CLARVIS)
