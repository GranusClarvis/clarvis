...
- [x] **[ESR_DENOMINATOR_FORMULA_AUDIT]** Shipped 2026-05-12. Authored `docs/internal/audits/ESR_DENOMINATOR_AUDIT_2026-05-02.md`. 5 formula sites enumerated with file:line (metacognition.py:193 canonical; performance_benchmark.py:299 PI; episodic_memory.py:1207 synth; quality.py:46 task_quality; quality.py:485 efficiency) + 2 adjacent (quality.py:212 first_pass; website/server.py:128 dashboard). Variants recomputed on n=100/500/500 from `data/episodes.json`: sites A & B = 0.828 (= live PI value); sites C/D undercount at 0.826 (omit `transient_auth` exclusion); site E uses raw total. Confirmed `precheck_fail`/`no_task`/`skipped` are heartbeat-health buckets — never written as episode outcomes (grep verified), so the 3 buckets named in the brief are *not* and cannot be in any ESR denominator. Canonical = site A (`successes / (total − soft_failure − transient_auth)`). R1 converges C/D/E onto site A via `compute_episode_success_rate()` (≤30 LOC, exact diff in §4). 0.860→0.828 drift is real, driven by 71 `action.unverified` episodes (not a formula bug). Acceptance: file exists ✓; 5 sites ≥3 ✓; 4 concrete recommendations ≥1 ✓. Non-Python markdown audit. Targets weakest metric Episode Success Rate=0.860 by verifying the metric itself is computed correctly before optimizing it. (PROJECT:CLARVIS)
