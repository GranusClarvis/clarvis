...
- [ ] **[AUDIT_PHASE_4_BRAIN_RETRIEVAL_TRACE_WIRING]** [P0] Extend `heartbeat_preflight._finalize_preflight_trace` and the brain-recall helpers so each heartbeat writes a structured `preflight.brain_retrieval: [{memory_id, collection, distance, text, rank}, ...]` list into the audit trace. Today the trace captures the rendered `prompt.context_brief` string but not the underlying per-memory retrieval payload, so `scripts/audit/brain_attribution.py` cannot compute per-collection attribution. `brain_attribution.py` is already forward-compatible with the structured field. Acceptance: after one heartbeat with the new wiring lands, `brain_attribution.py run` reports `fallback_used_everywhere: false` and ≥1 collection has `block_count > 0`. Blocks the Phase 4 attribution gate. Source: `docs/internal/audits/decisions/2026-04-16_phase4_brain_usefulness.md`.
...
- [x] **[SWO_PR178_LINT_CLEANUP]** (2026-04-17) Replaced 14 `any` with narrow types in sanctuary.test.ts + db.ts. PR #182. ESLint errors: 14→0.
...
- [x] **[SWO_SERVER_TRUST_BOUNDARY_AUDIT]** (2026-04-17) Audited all API routes. Found 2 CRITICAL (raffle bonus entries + votingPower spoofable), 5 HIGH (chat/presence/messages/voice unauthenticated, quests no objective check), 2 MEDIUM. Full report: `memory/evolution/SWO_TRUST_BOUNDARY_AUDIT_2026-04-17.md`.
...
- [x] **[COGNITION_FEATURE_TRANSLATION_REVIEW]** Review neuroscience/paper-inspired features for translation quality: keep / revise / demote / archive classification. _(Closed 2026-04-17: subsumed by Phase 9 decision doc `docs/internal/audits/NEURO_FEATURE_DECISIONS_2026-04-17.md` — 16 features scored, 2 PROMOTE / 6 KEEP / 4 REVISE / 4 SHADOW.)_
- [x] **[CONSCIOUSNESS_EMULATION_BLOAT_GUARD]** Anti-bloat review pass for consciousness-/neuroscience-inspired features. Demote if not showing value in task success, retrieval, or prompt quality. _(Closed 2026-04-17: subsumed by Phase 9. 4 features SHADOW, 4 REVISE. No premature demotion — subtle-feature guard respected.)_
- [x] **[EXECUTION_FIRST_COGNITION_POLICY]** Define policy: cognitive features justified by better execution, not by resemblance to consciousness on paper. Triage active cognition tasks. _(Closed 2026-04-17: Phase 9 operationalizes this policy via EVS/TCS scoring — features justified by execution value, not theory elegance.)_
- [x] **[BRAIN_WIKI_CONTEXT_ROUTE_BENCH]** (2026-04-17) Benchmarked 4 routes × 5 tasks. Combined (122ms, 138 tokens) is best quality/token. Wiki found 0 matches (too sparse for preflight). Minimal (1ms, 37 tokens) for routing only. No route change needed. Results: `data/benchmarks/brain_wiki_route_bench_analysis.md`.
- [x] **[LOW_VALUE_FEATURE_DEMOTION_SWEEP]** Sweep active queue/cognition work for features adding complexity without clear output quality gains. _(Closed 2026-04-17: subsumed by Phase 9. Analogy_engine identified as DORMANT→SHADOW; dream_engine, absolute_zero, theory_of_mind also SHADOW.)_
...
- [x] **[SPINE_COGNITION_DEAD_SURFACE_PHASE9_INTAKE]** No code change. Feed the 20 DEAD cognition exports (across `cognitive_load`, `conceptual_framework`, `intrinsic_assessment`, `attention` module surfaces) into the Phase 9 EVS/TCS scorecard as inputs. _(Closed 2026-04-17: Phase 9 scorecard at `data/audit/neuro_feature_scorecard.jsonl` covers all cognition features with EVS/TCS. Dead exports from these modules are addressed in their REVISE rulings — e.g. thought_protocol strip in `[THOUGHT_PROTOCOL_SLIM]`.)_
