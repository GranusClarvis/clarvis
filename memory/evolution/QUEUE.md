...
- [x] **[BB_PHASE2_ADERYN_CI]** Wired Aderyn v0.6.8 alongside Slither in `.github/workflows/ci.yml` (job: `aderyn`). Pinned binary install, fails on HIGH only, uploads `aderyn-report.{md,json}` as artifact. Local first-run: 3 HIGH → 0 HIGH after `packages/contracts/aderyn.toml` suppressions for documented Phase 1 patterns (`eth-send-unchecked-address`, `reentrancy-state-change`, `reused-contract-name` — same rationale as `.slither.config.json`). 6 LOW informational findings remain in artifact. (PROJECT:BUNNYBAGZ) (2026-05-04)
