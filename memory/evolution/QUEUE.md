...
- [x] [UNVERIFIED] **[BB_PHASE2_FORGE_BUILD_RED_FIX]** Shipped 2026-05-06 (commits `9dd1cd7` + `2400d82`). Root cause was **not** a Phase-3 contract regression — it was a CI-side missing dependency: the install step in `.github/workflows/ci.yml` only shallow-cloned `forge-std` + `openzeppelin-contracts`, but `script/Deploy.s.sol` + `test/DeployDeterministic.t.sol` import from `createx-forge/script/CreateXScript.sol`. The `lib/` dir is gitignored, so `lib/createx-forge/` only ever existed locally. CI failure log on run 25388220194: `Error (6275): Source "lib/createx-forge/script/CreateXScript.sol" not found`. **Fix 1** (`9dd1cd7`): add a pinned shallow clone of `radeksvarz/createx-forge` (no upstream release tags exist — pinned to the SHA matching the locally-vendored copy `cef15824154b2a7117bdac60870466b185fba684`) to every CI job that runs a full forge build (contracts, halmos, medusa, slither, aderyn, chain-abis). Verified on CI run 25426180890: **contracts ✅ 29s, halmos ✅ 38s, aderyn ✅ 15s** all green. **Fix 2** (`2400d82`): medusa was *also* failing — but for an unrelated, pre-existing infra flake at `solc-select install 0.8.24` (HTTP 403 from api.github.com on shared CI IPs). Patched by pre-staging the solc 0.8.24 binary directly from `binaries.soliditylang.org` into `~/.solc-select/artifacts/` so the rate-limited GitHub-API call is never made. Local `forge build --sizes` is green; full `forge test` (excl. halmos symbolic) is **147/147** green. (PROJECT:BUNNYBAGZ) (2026-05-06)
