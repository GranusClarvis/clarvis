...
- [ ] **[P3_PER_QUERY_FAILURE_TRIAGE]** Non-Python markdown. Targets weakest metric Precision@3=0.683 from an angle the 5 pending P@3 items do not cover: which specific queries are dragging the score down and why. Cheapest possible probe (no embedding/runtime changes). Run the canonical retrieval benchmark (whichever source `[P3_DASHBOARD_SOURCE_AUDIT]` resolves as canonical) with verbose top-10 output; identify the 10 lowest-P@3 queries. For each: (a) `python3 -m clarvis brain show <expected_id>` to confirm the expected memory exists, (b) inspect what the top-3 actually returned (text + similarity score), (c) categorize the failure as one of `expected_missing` / `near_duplicate_crowd` / `cross_collection_bleed` / `query_phrasing_mismatch` / `embedding_low_quality`, (d) propose one concrete fix per query (rewrite memory, add memory, retag, or move collection). Write `docs/internal/audits/P3_PER_QUERY_TRIAGE_2026-05-09.md`. **Acceptance:** file exists; ≥10 queries triaged; each has a category + one concrete fix; ≥3 cross-cutting patterns named in a "common causes" section; recommendation on which existing P@3 probe (HyDE / rerank / chunk) is most likely to address the dominant pattern. (PROJECT:CLARVIS)
- [ ] **[BB_PHASE3_RECENT_OUTCOMES_CROSS_GAME]** Non-Python (TSX UI). Closes Phase 2 truth-audit gap #2 from `bb_phase2_truth_audit_2026-05-04.md`: `RecentOutcomesStrip` is hardcoded `game=coinflip` and only mounts on `/play/coinflip`; Dice + HiLo have no recent-outcomes surface; WalletSheet has no cross-game recent-bets surface. Mount the strip on `apps/web/src/app/play/dice/page.tsx` and `apps/web/src/app/play/hilo/page.tsx` with the respective `game` prop. Add a cross-game variant inside `WalletSheet` that aggregates the most-recent N bets across all 3 games (sort by block timestamp; show game tag + outcome + stake per row). Vitest cases: each game route renders the strip with the correct `game` prop; WalletSheet's cross-game variant renders mixed games from a fixture. **Acceptance:** strip mounted on `/play/dice` + `/play/hilo`; WalletSheet shows cross-game recent bets; web vitest grows by ≥6 cases; all suites green (web + api + indexer + forge); no regressions in existing strip mount on `/play/coinflip`. (PROJECT:BUNNYBAGZ)
- [ ] **[BB_PHASE3_KEYBOARD_NAV_AUDIT]** Non-Python (TSX harness + markdown report). UX_PLAN §7 requires keyboard-only navigation across all redesigned surfaces; the recently-shipped Phase 3 items (motion pass, payout counter, mascot frames, wallet PnL card, first-bet hint toast) have not been audited as a cohort. Run a `clarvis_browser.py` (Playwright CDP) keyboard-only walk through Coinflip, Dice, HiLo, Lobby, WalletSheet, TrustFooter — Tab through interactive elements, verify focus rings visible in light + dark themes, verify Enter/Space activates primary CTAs, verify no focus traps, verify Esc closes WalletSheet. Capture screenshots of focus state on each surface (`memory/cron/bb_keyboard_nav_audit_2026-05-09/`). Write `memory/cron/bb_keyboard_nav_audit_2026-05-09.md` scoring each surface 1-5 against UX_PLAN §7 with file:line references for any failing focus rings or tab orders. If any surface scores ≤3, auto-append a `[BB_PHASE3_KBNAV_<TAG>_FIX]` task to QUEUE.md. **Acceptance:** audit folder exists with ≥6 screenshots (one per surface, focus-state visible); verdict file lists per-surface 1-5 score; ≥2 file:line references for any flagged issues; binary go/no-go on whether keyboard nav is Phase 3 ready. (PROJECT:BUNNYBAGZ)
