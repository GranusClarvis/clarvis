...
- [x] **[SWO_V2_DESLOP_SHADER]** Phaser post-FX `PostFXPipeline` palette-quantize (+ optional Bayer dither) pass over the canvas to a fixed N-color SWO palette. Dev-flag toggle (`?v=2&deslop=1` + `localStorage.swo_deslop`); ship enabled-by-default only after operator A/B inspection. **DONE — PR #264 merged 2026-04-26 (commit `47d9b08`, merge `d562891`).** Shipped opt-in (`DESLOP_DEFAULT_ON=false` in `game/shaders/deslopPalette.ts`); 18 unit tests. Operator A/B inspection still pending before flipping default. (PROJECT:SWO, P0, Track B)
