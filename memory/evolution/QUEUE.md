...
- [x] **[BB_PHASE2_RECENT_OUTCOMES_DICE_HILO]** Shipped 2026-05-05 (mega-house). `RecentOutcomesStrip` now takes `game: "coinflip"|"dice"|"hilo"` (threaded into `?game=`); mounted above slider on `/play/dice` and above `HiloCard` on `/play/hilo`. Per-game pill branches: H/T for coinflip, `≤R` won/lost for dice, `↑/↓ + multiplier` for hilo. Settle event refetch wired per game (`bunnybagz:<game>-settle`). 23 vitest cases (8 coinflip + 7 dice + 7 hilo + shared util) green; 332 web suite green; thumb-zone audit unchanged.
