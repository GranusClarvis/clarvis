...
- [x] **[P3_DASHBOARD_DAILY_REFRESH_CRON]** Shipped 2026-05-04. Installed `cron_retrieval_quality` at 06:25 CET (06:20 slot taken by llm_brain_review): `scripts/cron/cron_retrieval_quality.sh` runs `retrieval_benchmark.py golden_qa` then new `scripts/brain_mem/retrieval_dashboard.py` (interim aggregator over benchmark + retrieval_quality/report.json + context_relevance.jsonl) and writes `data/retrieval_quality/dashboard.md` with `_last_updated:_` line. Logs to both `memory/cron/retrieval_quality.log` (watchdog-monitored, 26h grace) and `monitoring/retrieval_quality.log` (ops tail). Watchdog `check_job` + `recheck_job` entries added → Telegram alert fires via existing hook on stale log. Cron comment notes canonical-source TBD pending `[P3_DASHBOARD_SOURCE_AUDIT]`. Verified: `clarvis cron list` shows entry, manual run wrote dashboard with `last_updated=0s`, watchdog reports `OK retrieval_quality`. (PROJECT:CLARVIS)
- [ ] **[DASHBOARD_FRESHNESS_AUDIT_2026-05-04]** Non-Python markdown audit. The 7-week staleness of `data/retrieval_quality/dashboard.md` is unlikely to be the only stale dashboard — many `data/**/*.md` and `data/**/*.json` reports are written by ad-hoc scripts with no recurring refresh. Enumerate every file under `data/` named `dashboard.md`, `*_metrics.json`, `*_summary.md`, or carrying a `last_updated:` frontmatter line. For each: record path, `last_updated` (frontmatter > file mtime), generator script (grep for the path), associated cron entry (if any). Classify as `fresh (<24h)`, `recent (1-7d)`, `stale (7-30d)`, `dead (>30d)`. Write `docs/internal/audits/DASHBOARD_FRESHNESS_AUDIT_2026-05-04.md`. **Acceptance:** file exists, ≥5 dashboards enumerated with all four columns populated, every `dead` entry has a concrete recommendation (cron entry to add, or retire as artifact). (PROJECT:CLARVIS)
