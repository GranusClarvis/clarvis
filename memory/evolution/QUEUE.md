...
- [x] **[BB_FIRST_BET_HINT]** (2026-05-03) Shipped `apps/web/src/components/FirstBetHint.tsx` (client component, dismissible pill, localStorage key `bunnybagz:first-bet-hint-dismissed`) wired into `apps/web/src/app/play/page.tsx` above the games grid; 3 vitest cases in `FirstBetHint.test.tsx` (renders by default, dismissible+persists, stays dismissed across re-mounts). Web suite passing 139/139, typecheck clean. Indexer `hasPlacedBet` prop wired but defaults to `false` until BB_INDEXER_PHASE1_PONDER_REAL surfaces real history. UX_PLAN §2 prescribes a "Tap **HEADS** or **TAILS** to play" first-bet hint on `/play`. Today the lobby shows the Coinflip card with no instructional copy for first-time visitors. Add a small dismissible hint pill above the card (state stored in `localStorage` under `bunnybagz:first-bet-hint-dismissed`); auto-hides after the first connected wallet places a bet (read from indexer when wired, otherwise stay until manually dismissed). **Acceptance:** new component + 3 vitest cases (renders by default, dismissible, stays dismissed across re-mounts via localStorage); does NOT render in test setups where `localStorage["bunnybagz:first-bet-hint-dismissed"]` is set; web suite +3. (PROJECT:BUNNYBAGZ)
