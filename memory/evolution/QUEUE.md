...
- [x] **[BB_THEME_CONTRAST_AUDIT]** Done 2026-05-02. Token-level WCAG-AA audit added at `apps/web/src/lib/contrast-audit.ts` + acceptance test `apps/web/src/__tests__/contrast-audit.test.ts` (49 new vitest cases — token sync vs `globals.css`, AA gate per surface×theme, AAA gate on primary CTAs). Pre-fix audit found `--bb-fg-subtle` failing AA on every surface in both themes (e.g. 4.10:1 dark surface, 3.40:1 light elevated) and `--bb-brand-gold/--bb-success/--bb-danger` collapsing to 1.6–2.3:1 as text on light surfaces. Fix: bumped `--bb-fg-subtle` (`#7f7866`→`#a39a88` dark; `#8a7e64`→`#665a45` light) and added `--bb-link-fg` / `--bb-success-fg` / `--bb-danger-fg` text-only tokens with darker light-mode variants (`#945e07` / `#2a6f35` / `#a93b27`). Component swaps in `page.tsx`, `play/page.tsx`, `play/coinflip/page.tsx`, `wallet/page.tsx`. **Final state: 0 failed nodes on every surface×theme** (well under the <3 ceiling); primary CTA brand-ink-on-brand-gold = 9.53:1 (AAA passes). Baseline JSON at `apps/web/test-results/axe-baseline.json` (deterministic, no timestamp churn). CI step added in `.github/workflows/ci.yml` (`web` job now runs `pnpm --filter @bunnybagz/web test` + uploads the baseline as an artefact); regressions fail the job. Web suite 58→125 cases all green; `pnpm -r typecheck` clean.
