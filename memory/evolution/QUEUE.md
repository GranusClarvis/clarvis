...
- [x] **[BB_WAGMI_CLI_ABI_GEN_REAL]** Shipped 2026-05-03 (commit 85d82c9 on feature/mvp-planning-and-rebrand). `packages/chain/scripts/codegen-abis.mjs` mirrors `codegen-addresses.mjs`: reads forge artifacts at `packages/contracts/out/<Name>.sol/<Name>.json`, emits typed `as const` ABIs into `packages/chain/src/abis/<name>.ts` (coinflip + bankroll/dice placeholders) plus an index re-exporter. `@bunnybagz/chain` re-exports them from main; `apps/web/src/lib/abis.ts` is now `export {COINFLIP_ABI} from "@bunnybagz/chain"` (literal "Phase 1 deploy will replace this" comment gone). New CI job `chain-abis` mirrors `chain-addresses` (installs Foundry → `forge build --skip test --skip script` → `pnpm --filter @bunnybagz/chain check:abis`). Verified locally: codegen idempotent, `--check` exits 1 on drift / 0 in sync, web suite 132/132 still green. (PROJECT:BUNNYBAGZ)
