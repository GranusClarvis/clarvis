...
- [ ] **[BB_TAILWIND_TOKENS_REAL]** ([REOPENED] from `[BB_TAILWIND_TOKENS_INSTALL]`) Real CSS-variable token palette in `apps/web/src/app/globals.css` with `:root` (dark) + `[data-theme="light"]` blocks + `@media (prefers-color-scheme: light)` no-JS fallback. Inline color values across `/`, `/play`, `/play/coinflip`, `/verify/[betId]`, `/wallet`, `SiteHeader`, `WalletSheet` replaced with `var(--bb-*)` references. **Verification gate (BEFORE archiving):** confirm `apps/web/src/app/__tests__/theme-tokens.test.ts` passes locally AND no inline `style={{... color: "#xxx"...}}` literals remain on the listed surfaces (a `grep -rn '#[0-9a-f]\{6\}' apps/web/src` baseline of <20 unique hex literals is acceptable; everything else must be a token reference). **Operator-test:** in `pnpm dev`, open `/`, the `⋯` wallet sheet, click "Switch to light theme" — entire layout must visibly repaint, not just the sheet. (PROJECT:BUNNYBAGZ)
...
- [x] **[QUEUE_UNVERIFIED_ARCHIVE_GUARD]** (2026-05-01) Added a verification gate to `clarvis/queue/writer.archive_completed()`. Default mode `log` always appends `[UNVERIFIED]` archive events to `monitoring/queue_unverified_archive.log` (audit trail). When env `CLARVIS_QUEUE_UNVERIFIED_GUARD=block` is set, items with `[UNVERIFIED]` in the body are NOT archived unless a sidecar verification record exists at `data/audit/queue_verifications/<tag>.json`. Held items stay in QUEUE.md as `[x]` so operator review can resolve them. 4/4 tests pass (`tests/test_queue_unverified_guard.py`); existing 22 archive/runnable tests still green.
