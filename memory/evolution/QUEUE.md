...
- [ ] **[BB_PHASE2_TESTNET_BET_SMOKE]** Operator-driven follow-up to fully close ROADMAP Phase 2 exit (e). Connect MetaMask to chain 6343, place one minimum-stake ETH bet on `/play/coinflip`, `/play/dice`, `/play/hilo`, capture explorer links to the settled `BetSettled` (Coinflip/Dice) and `SessionCashedOut` (HiLo) events, and append a "Smoke" appendix to `memory/evolution/bb_phase2_testnet_live_2026-05-05.md`. Until that lands the testnet-live wiring is verified at the contract layer but the in-browser round-trip is asserted only through CI Playwright (mocked wallet). Agent host has no GUI so cannot drive a real wallet. (PROJECT:BUNNYBAGZ)
