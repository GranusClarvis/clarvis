...
- [x] **[BB_PHASE2_LOBBY_UNLOCK_DICE_HILO]** SHIPPED 2026-05-04 — `apps/web/src/app/play/page.tsx` now resolves each card's status from `ADDRESSES[DEFAULT_CHAIN_ID]` via `resolveGames()`; stable sort keeps phase2 cards at top, live cards at bottom (thumb zone). Lobby vitest now mocks `@bunnybagz/chain` with mutable ADDRESSES and covers: (a) `lobby renders dice + hilo as phase-2 when their addresses are absent`, (b) `lobby renders dice + hilo as live when their addresses resolve` (asserts `game-card-dice-link` + `-hilo-link` exist), (c) ordering regression for partial deploy. Thumb-zone audit re-runs green (7/7) with all 3 cards live; full web suite 189/189. (PROJECT:BUNNYBAGZ)
