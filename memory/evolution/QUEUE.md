...
- [x] **[BB_PHASE2_HILO_CARD_VISUAL]** Shipped 2026-05-05. New `apps/web/src/components/HiloCard.tsx` renders rank (A/2..10/J/Q/K) + suit glyph with framer-motion `rotateY` flip; `HILO_FLIP_DURATION_MS=600` constant locks UX_PLAN §6 motion contract; ease `[0.22, 1, 0.36, 1]` (ease-out-quart). Suit derived deterministically via FNV-1a hash of `${sessionId}:${stepCount}` mod 4 (replayable from verify page). `prefers-reduced-motion: reduce` swaps to a 200 ms opacity crossfade. Wired into `/play/hilo` (replacing the plain `<span>`) and `/verify/session/[sessionId]` (initial-card). Tests: new `__tests__/HiloCard.test.tsx` (12 specs covering rank, suit determinism, glyph, flip variant, crossfade fallback, duration constant); HiLo page test grew 19→24 (5 new card-visual cases). Full web suite green: 220/220. (PROJECT:BUNNYBAGZ)
