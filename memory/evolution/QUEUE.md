...
- [x] **[BB_PHASE2_RECENT_BETS_WALLET_SHEET]** Shipped 2026-05-05. (a) `apps/api/history/wallet.ts` edge endpoint with `handleWalletHistoryRequest` test seam + `makeIndexerProxy` production resolver (limit default 10 / max 50, cursor on overflow, 502 on aggregator throw, lowercase address normalisation) — 13 tsx --test cases pass. (b) `apps/web/src/components/RecentBetsList.tsx` renders game icon + outcome chip + stake→payout columns; links `betId`→`/verify/[betId]` and `txHash`→MegaETH Blockscout (gracefully omitted when null). (c)+(d) Mounted in `WalletPanel`, so both the modal `WalletSheet` and the standalone `/wallet` route surface it. (e) 13 vitest cases (loading, empty, mixed-game, mixed ETH/USDm formatting, error, 404 not-wired-yet, refetch on `bunnybagz:bet-settled`, link href contract, single live-region announce, helper coverage). (f) Audit extended with `wallet-recent-bets-list` surface + audit-test mounting. Web suite 316/316, api suite 88/88. (PROJECT:BUNNYBAGZ)
