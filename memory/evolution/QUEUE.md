...
- [x] **[BB_PHASE2_COMMIT_REPLAY_REDEPLOY]** **DONE 2026-05-03 night.** Patched coinflip live at `0x963cAaDD6be6F18D9AA19843d568cBF31d2AE11a` (chain 6343); old `0x6fb59e…bEE99` paused (tx `0x8448ffd4…959151`) AND `bankroll.revokeGame`'d (tx `0xc482f0d4…20b43d`). Patched build wired via `bankroll.registerGame(NEW, 1e15)` (tx `0x46d3dec9…125af7f`); allowance bumped to `2e15` to fix max-bet-win allowance shortfall (tx `0x5ef8b08d…308f4a5da`). Live smoke test: placeBet (tx `0x69c0318b…508412a`) → settleBet (won, tx `0xd47b9f8b…7e5f40d779`); replay attack against settled commit reverts `CommitAlreadyUsed()` selector `0x86dbb72a` ✓. `deployments/6343.json` + `addresses.generated.ts` updated; `chain check:addresses` + `check:abis` clean. Test sweep: contracts 106, web 165, api 50, indexer 21, verify 40 = 382 passing. See `docs/PHASE1_REPORT.md` § (post-redeploy verdict) and `docs/CRYPTO_RUNBOOK.md` § 1, § 3. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_HILO_TIE_PUSH_OR_EDGE_DOC]** **DONE 2026-05-03 (Phase 2 ship 1).** Picked option (a): `BunnyBagzHiLo.playStep` now branches on `newCard == cur` and routes to a new `_settlePush` internal that refunds the stake to the player and transitions the session to `Status.Pushed` (new enum value). New `event SessionPushed(sessionId, player, stake, card, multiplier)` for the indexer. With ties returning, the conditional fair payout `12/(13-c)` × 0.99 honestly encodes the published 1% edge. Tests rewritten: `test_playStep_tieIsPushAndRefundsStake` (was `test_playStep_losingClosesSessionAndForfeitsStake`), new `test_playStep_strictLossClosesSessionAndForfeitsStake`, both fuzz tests (`testFuzz_settlement_winLossPaths`, `testFuzz_sessionLength_compoundsCorrectly`) + invariant handler updated for the push branch. `BunnyBagzHiLoTest`: 22 → 28 passing; invariants still hold. ABI regenerated with `SessionPushed` + `InvalidBetBounds`. Frontend `/play/hilo` placeholder copy refreshed to surface the tie-as-push semantics + new test for that copy. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_SEED_LIFECYCLE_API_CLEANUP]** **DONE 2026-05-03 (light-touch).** Both `/seed/commit` and `/seed/rotate` now emit RFC 8594/9745 `Deprecation: true`, `Sunset: 2026-12-31`, and `Link: </api/seed/claim>; rel="successor-version"` headers on every response (including 405/403/503). `ROTATE_AFTER_BETS` and `ROTATE_AFTER_MS` annotated `@deprecated` in `apps/api/lib/seedStore.ts`. Header invariant locked by new `commit() + rotate() emit Deprecation headers` test in `lifecycle.test.ts` (api 50 → 51 passing). Full endpoint deletion deferred until the frontend page-level commit fetch is removed from `apps/web/src/app/play/coinflip/page.tsx` (separate sweep — that change rewrites the "Waiting for server seed" CTA state). (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_SLITHER_CI]** **DONE 2026-05-03.** New `slither` job in `.github/workflows/ci.yml` running `slither . --config-file .slither.config.json --fail-high` against `packages/contracts`. Slither pinned at `slither-analyzer==0.10.4`; foundry-toolchain + shallow-cloned OZ v5.0.2 + forge-std v1.9.4 reused from `contracts` job for compilation. `.slither.config.json` filters `lib/|test/|script/`, excludes informational/low/optimization, and silences Phase-1 expected detectors (`naming-convention,solc-version,pragma,assembly,low-level-calls,timestamp,reentrancy-events,reentrancy-benign,reentrancy-no-eth,external-function,immutable-states,calls-loop,boolean-equal,similar-names`) so the noise floor is zero. Job fails CI only on **high**-severity findings — single-key Ownable owner, ReentrancyGuard-gated `.call{value:}` payouts, and other intentional Phase 1 patterns are suppressed. Config + workflow YAML/JSON validated locally; `slither --list-detectors` parses the config cleanly. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_HALMOS_COMMITUSED_PROOF]** Symbolic proof via halmos that the per-commit single-use guard holds across `placeBet → settleBet → placeBet'` for arbitrary callers. Pairs with the existing Foundry invariant suite. Acceptance: `halmos --contract BunnyBagzCoinflip` passes the invariant; CI job. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_UINT96_STAKE_GUARD]** **DONE 2026-05-03 (Phase 2 ship 2).** All three games' `setBetBounds(uint256 newMin, uint256 newMax)` now revert with a new shared `InvalidBetBounds()` error when (a) `newMax < newMin` or (b) `newMax > type(uint96).max`. Three regression tests landed (`test_setBetBounds_rejectsMaxBelowMin`, `test_setBetBounds_rejectsMaxAboveUint96`) per game. ABI regenerated. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_BANKROLL_RECEIVE_EVENT]** `BunnyBagzBankroll.receive()` silently accepts ETH today (no event), so direct sends are invisible to the indexer. Either revert `receive()` to force `deposit()`, or emit `Deposited(msg.sender, msg.value)` from `receive()`. Acceptance: contract test asserts the event; indexer schema updated if applicable. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_ALLOWANCE_INVARIANT]** **DONE for Coinflip 2026-05-03 (Phase 2 ship 3).** Picked option (b): `BunnyBagzCoinflip.setBetBounds` now reverts with `BankrollAllowanceTooLow(required, available)` if `bankroll.allowanceOf(this) < (PAYOUT_NUM - PAYOUT_DEN) * newMax / PAYOUT_DEN`. The bankroll interface in Coinflip gained an `allowanceOf(address) view returns (uint256)` declaration. Two regression tests: `test_setBetBounds_rejectsAllowanceShortfall` (asserts the exact required value 0.98 × newMax) and `test_setBetBounds_acceptsAfterAllowanceTopUp`. Dice + HiLo intentionally do NOT encode this invariant — Dice's payout multiplier varies per bet (`99/(R-1)` from 1.01× to 99×) so the right enforcer is per-bet in `placeBet`; HiLo's compounded multiplier is bounded by `setMaxPayout` instead. The `setMaxPayout` allowance hook lands in a follow-up. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_ADERYN_CI]** Companion to slither: Cyfrin Aderyn (Rust, AST-based, near-zero setup) catches a different class of issues. Wire into CI alongside slither; both fail-on-HIGH. Acceptance: new CI step running `aderyn .` on `packages/contracts`; first run green; output uploaded as artifact. Reference: `docs/CRYPTO_RUNBOOK.md` § 5 ("Top-5 wires"). (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_MEDUSA_FUZZ]** Coverage-guided invariant fuzzing via Trail of Bits Medusa (the 2026 standard, supersedes Echidna for new projects per `docs/CRYPTO_RUNBOOK.md` § 5). Layout from [Recon-Fuzz/create-chimera-app](https://github.com/Recon-Fuzz/create-chimera-app). Acceptance: `medusa.json` config; new CI job; one shared invariant file proving `invariant_systemNeverInsolvent` on Coinflip/Dice/HiLo runs in <2 min; coverage uploaded as artifact. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_DEFENDER_MONITOR]** OpenZeppelin Defender 2.0 Monitor on the live coinflip + bankroll. Sentinels for: `BetPlaced` rate spike (>10/min from one address); owner-key actions (`Paused`, `Unpaused`, `GameRegistered`, `GameRevoked`, `setGameAllowance`); bankroll ETH-balance threshold breach (<10% of `INITIAL_DEPOSIT`). Webhook → operator Telegram via existing `BUNNYBAGZ_TELEGRAM_*` creds. Acceptance: Defender project provisioned (operator-gated for the org token); monitors deployed; one synthetic alert fired into Telegram during setup; setup notes added to `docs/CRYPTO_RUNBOOK.md` § 7. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_CREATEX_DETERMINISTIC_DEPLOY]** Switch the deploy pipeline to `createx-forge` ([radeksvarz/createx-forge](https://github.com/radeksvarz/createx-forge)) so addresses are deterministic across MegaETH testnet/mainnet/anvil. Removes a class of "wrong address on chain X" footguns before mainnet flip. Acceptance: `Deploy.s.sol` rewritten to use CREATE3 via CreateX; the same salt produces the same address on local anvil + MegaETH testnet; deploy artifact carries the salt for reproducibility. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [x] **[BB_PHASE2_DICE_FRONTEND_REAL]** **DONE 2026-05-03 (Phase 2 ship 4).** Replaced the `/play/dice` Phase-2 placeholder with a real first-playable surface: roll-under range slider (2–98), live multiplier preview (`99/(R-1)`), live win-probability preview (`(R-1)/100`), per-bet `/api/seed/claim` flow (NO fallback to a stale page-level commit — same hardening as the Coinflip page), wagmi `placeBet` write, settle-event aria-live announcer, full primary-CTA state machine (disconnected → wrong-chain → not-deployed → confirm-in-wallet → roll). Pure helpers `multiplierFor`/`winProbabilityFor` exported and unit-tested against the on-chain `quotePayout` formula. New `HILO_ABI` + `DICE_ABI` re-exported from `@/lib/abis`. Tests: 10 Vitest specs in `apps/web/src/app/play/dice/__tests__/page.test.tsx` (was 3 placeholder specs). Web suite: 165 → 173 passing. (PROJECT:BUNNYBAGZ) (2026-05-03)
- [ ] **[BB_PHASE2_HILO_FRONTEND_REAL]** The HiLo frontend is still a placeholder; the contract is shipped + audited (with the new tie-as-push semantics) and the ABI is generated. Build the open-session form, current-card display, Higher/Lower step picker, multiplier track + cash-out CTA. The `playStep` call is keeper-driven off-chain; the player surface only signs `openSession` + `cashOut`. The placeholder now surfaces the post-fix tie-as-push edge copy + remaining-work hint so it is a clean handoff target. Acceptance: deploy-time wagmi calls for `openSession`, `cashOut`, `refundSession`; settle event wiring; aria-live region; full state machine tests. (PROJECT:BUNNYBAGZ) (2026-05-03)

- [x] **[BB_PHASE1_TESTNET_FAUCET_AND_DEPLOY]** **DONE 2026-05-03.** Operator funded deployer (1 ETH on `0xb29e6735629539cEd64F0d6f0c476Fe92539fD7B`); deploy ran in one shot, then the post-deploy hardening pass redeployed the patched coinflip (see `[BB_PHASE2_COMMIT_REPLAY_REDEPLOY]` above). Final live addresses: bankroll `0x3FF9Eb…AcBAa`, coinflip `0x963cAa…E11a` (chain 6343). (PROJECT:BUNNYBAGZ) (2026-05-03)
