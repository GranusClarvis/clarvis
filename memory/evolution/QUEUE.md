...
- [x] **[BB_PHASE2_BANKROLL_RECEIVE_EVENT]** `receive()` now emits `Deposited(msg.sender, msg.value)` (BunnyBagzBankroll.sol:138-141); test `test_receive_emitsDepositedAndAcceptsETH` asserts the event; full forge suite green (115/115, incl. invariant fuzz). Indexer schema unchanged — Ponder doesn't track bankroll events yet (`apps/indexer/ponder.config.ts` only filters `BunnyBagzCoinflip`). (2026-05-04)
