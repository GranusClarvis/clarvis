...
- [ ] **[SWO_V2_COMPANION_BOND_MILESTONE_CELEBRATE]** Detect when `bond_score` crosses one of `[25, 50, 75, 100]` between consecutive interactions and fire a celebration moment on the Companion screen (and in the V2 world via `companion-vfx` `heart` kind). **Acceptance:** (a) crossing detected once per threshold per companion (idempotent, persisted); (b) screen renders a brief banner like "💗 You and Pico are growing closer." with confetti/heart particles for ≤2.5s; (c) world `CompanionMenu` receives a heart VFX burst; (d) ≥3 vitest cases on the threshold-crossing detector (no double-fire on identical bonds, fires on upward cross, no-fire on downward); (e) no API change. (PROJECT:SWO, P1)

- [ ] **[SWO_V2_COMPANION_LOCAL_VISIT_MARKER]** Persist a per-companion "you opened the screen at" timestamp in `localStorage` independent of `stats_updated_at` so the cozy last-visit line refreshes on every page-open even when no interaction happened. Today the line uses `stats_updated_at`, so opening the screen without acting shows a stale phrase. **Acceptance:** (a) `lib/sanctuary/lastVisitStore.ts` reads/writes a small per-token-id map in `localStorage` (graceful when unavailable, e.g. SSR); (b) CompanionView prefers the local marker over `stats_updated_at` when it is fresher; (c) ≥3 vitest cases against an in-memory storage shim. (PROJECT:SWO, P1)

- [ ] **[SWO_V2_COMPANION_CHAT_REMEMBERS_RECENT_ACTIONS]** Today chat injects current stats. Extend it to also reference the last 1–3 interactions ("you fed me a cosmic treat about an hour ago — thanks, that was nice") so chat replies feel temporally anchored. **Acceptance:** (a) `lib/sanctuary/chatPersonality.ts` (or its system-prompt builder) appends a 1-line "recent care log" (≤80 tokens, joined by `; `) sourced from the last 3 `interaction` journal entries; (b) total injected blob still ≤200 tokens (existing budget); (c) at least one prompt test ensures the recent-actions line is rendered when interactions exist and omitted when empty; (d) no schema change. (PROJECT:SWO, P2)
