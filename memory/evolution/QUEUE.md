...
- [x] **[BB_INDEXER_PHASE1_PONDER_REAL]** (2026-05-03) Shipped real Ponder project under `apps/indexer/`: `ponder.config.ts` resolves coinflip address from `@bunnybagz/chain` ADDRESSES with env overrides; `schema.graphql` with `Bet` entity (betId, player, side, outcome, stake, clientSeed, serverCommit, nonce, status, payout, serverReveal, blockPlaced, settledAt); runtime-agnostic `src/handlers.ts` (BetPlaced/BetSettled/BetRefunded) feeding a `BetStore` abstraction so handlers run against an in-memory mock RPC fixture in tests; `api/history.ts` `GET /api/history?address=&game=&limit=` filters to settled rows + matches the empty/populated/bad-address contract `RecentOutcomesStrip` already consumes. `pnpm --filter @bunnybagz/indexer test` 21/21 pass; `pnpm --filter @bunnybagz/indexer dev:once` writes 3 bets rows; `pnpm typecheck` clean; full repo `pnpm -r test` clean (web 132/132, api 36/36). 14 new files (≥4 required: config + schema + handler + test). Fly.io + Neon deferred to operator. (PROJECT:BUNNYBAGZ)
