...
- [ ] **[BB_PHASE2_USDM_PERMIT_UI_PLAY_PAGES]** Wire the existing `apps/web/src/lib/permit.ts` strategy + `PermitForwarder.sol` into the bet flow on `/play/coinflip`, `/play/dice`, `/play/hilo`. ROADMAP exit verbatim: "all 3 games playable in ETH and USDm on testnet". Today the ETH path is the only path; the strategy helpers ship unused. **Acceptance:** (a) each game page has a token-toggle (ETH | USDm) directly above the stake input, persisted to URL `?token=usdm` so deep links carry intent; (b) on `usdm` the page calls `decidePermitStrategy()` and either signs a per-bet permit (`kind: "permit"`) or one-time-approves then forwards (`kind: "approve-and-forward"`), pulling `tokenName="USDM"` from `@bunnybagz/chain` `usdmAddress`; (c) the resulting bet calldata flows through `PermitForwarder.placeWithPermit(...)` (or `placeWithApproval(...)`) so the `Coinflip/Dice/HiLo` contract receives USDM as the stake; (d) at least 6 vitest cases per game cover ETH happy path, USDm fresh-permit happy path, USDm legacy-fallback approve-then-forward, permit signature rejection, insufficient-allowance error, network-mismatch error; (e) WalletSheet "Approved" pill flips on after the fallback path; (f) every game page passes the existing tabular-figures + thumb-zone + contrast audits unchanged. Web suite must stay green. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_RECENT_BETS_WALLET_SHEET]** ROADMAP exit verbatim: "recent bets in wallet sheet". Today `WalletSheet.tsx` surfaces address + ETH/USDm balance + Approved pill + Fluffle placeholder + theme toggle — **no recent-bets section**. The indexer already exposes cross-game aggregation in `apps/indexer/src/recent-bets.ts`. Wire it through. **Acceptance:** (a) new edge endpoint `apps/api/history/wallet.ts` calls `recent-bets.ts` aggregator and returns `{address, bets: [{game, betId, stakeWei, payoutWei, outcome, blockNumber, txHash}], cursor}` with `limit` default 10, max 50; (b) new component `apps/web/src/components/RecentBetsList.tsx` consumes that endpoint, renders one row per bet with `game` icon + outcome chip + stake → payout columns, links the `txHash` to MegaETH explorer, and the `betId` to `/verify/[betId]`; (c) `WalletSheet.tsx` mounts `<RecentBetsList />` directly under the balances block; (d) standalone `/wallet` route also surfaces it; (e) ≥8 vitest cases cover loading, empty state, mixed-game render, ETH+USDm payout formatting, error fallback, refetch on `bunnybagz:bet-settled` event, link `href` correctness, accessibility (single live-region announce per fetch); (f) tabular-figures audit extended to cover the new numeric cells. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_RECENT_OUTCOMES_DICE_HILO]** `RecentOutcomesStrip` hardcodes `&game=coinflip` and is only mounted on `/play/coinflip`. Phase 2 surfaces are silent on outcome history. **Acceptance:** (a) component takes `game: "coinflip"|"dice"|"hilo"` prop, threaded into the indexer query string; (b) `/play/dice` mounts `<RecentOutcomesStrip game="dice" address={address} />` directly above the slider; (c) `/play/hilo` mounts it above the card surface; (d) outcome chips use the right glyphs per game (Heads/Tails for coinflip, "≤R" pill for dice, "↑↓" + multiplier for hilo) — keep the per-game render branch inside `RecentOutcomesStrip.tsx`; (e) ≥6 vitest cases per game cover empty/error/loading/mixed/recent-event-refetch/keyboard; (f) thumb-zone audit unchanged. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_E2E_GREEN_IN_CI]** ROADMAP exit verbatim: "e2e green". Specs ship and the `e2e:` CI job is wired, but no run has been observed green end-to-end on `ubuntu-latest` — `playwright install --with-deps` was never validated against the actual CI image. Drive one full green run, capture the run URL, and pin Playwright + Chromium versions so the gate stops drifting. **Acceptance:** (a) push a no-op commit on a branch that triggers the `e2e:` job; (b) collect the GitHub Actions run URL of a green run for both unconnected (3 specs × 3 projects = 9) and connected (3 specs × 3 projects = 9) wallet-mock specs; (c) in `.github/workflows/ci.yml`, pin `playwright/setup-node` action SHA and freeze `@playwright/test` version in `apps/web/package.json` (no `^`); (d) write the run URL into `memory/evolution/bb_phase2_e2e_run_<YYYY-MM-DD>.md`; (e) if the run reveals real product breakage, file a `[BB_PHASE2_E2E_FIX_<TAG>]` per failure; (f) acceptance fails if any spec is `skipped` or `flaky`. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_TESTNET_DEPLOY_DICE_HILO_LIVE]** `Deploy.s.sol` and `addresses.generated.ts` carry the deterministic addresses, but no on-chain deploy has happened on testnet (Phase 1 deployer wallet `0xb29e6735…fD7B` is unfunded). **Operator-gated** for the funding step; everything else is autonomous. **Acceptance:** (a) operator funds deployer; (b) agent runs `packages/contracts/script/deploy-testnet.sh --apply` — the existing wait-for-funding poller closes the loop; (c) `cast call` shows non-zero code at the predicted Dice + HiLo addresses on chain 6343; (d) `bankroll.allowance(dice)` and `bankroll.allowance(hilo)` are non-zero; (e) `apps/web` running against testnet RPC shows both lobby cards `live` and a real bet round-trips end-to-end on each game; (f) write `memory/evolution/bb_phase2_testnet_live_<YYYY-MM-DD>.md` with explorer links and tx hashes. Until (a) clears, this stays here as the single operator blocker. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_TRUTH_AUDIT_FOLLOWUP_2]** After the five items above ship, run a fresh end-to-end audit mirroring `bb_phase2_truth_audit_2026-05-04.md` — every ROADMAP Phase 2 exit bullet must hold against on-disk artifacts, not archive entries. **Acceptance:** new doc `memory/evolution/bb_phase2_truth_audit_<YYYY-MM-DD>.md` walks each of {3 games × ETH+USDm playable, recent bets in wallet sheet, e2e green in CI, light+dark themes intentional}; if any line is short, file the gap as `[BB_PHASE2_<TAG>_FIX]`; if all four hold, archive Phase 2 as DONE and unblock Phase 3 P1 promotion. (PROJECT:BUNNYBAGZ)
...
- [ ] **[BB_PHASE3_ANIMATION_PASS]** ROADMAP Phase 3: "Animation pass: coin spin, dice slider, hi-lo card flip + reduced-motion fallbacks." Today: HiloCard already ships a flip transition; coin spin and dice slider have no settle animation. **Acceptance:** (a) `apps/web/src/components/CoinSpin.tsx` renders a 600ms-capped flip on `/play/coinflip` settle; (b) `DiceSlider.tsx` extends to a 350ms-capped roll-result needle animation that lands on the rolled value; (c) HiloCard's existing flip is verified against the same 600ms cap; (d) every animation is gated on `prefers-reduced-motion: no-preference` — under reduced-motion all three become an instant state swap; (e) ≥3 vitest + jsdom cases per component covering the no-motion path, motion path, and the reduced-motion fallback; (f) tabular-figures + thumb-zone + contrast audits stay green; (g) bundle-size delta for `apps/web` < +10KB gzipped. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_MASCOT_ART]** ROADMAP Phase 3: "Mascot art pass: BunnyBagz idle / win / loss-streak frames; one supporting character." Asset pipeline note: Retro Diffusion is paused operator-side, so the first cut is hand-drawn SVG (or commissioned). **Acceptance:** (a) `apps/web/public/mascots/bunnybagz_{idle,win,loss-streak}.svg` ship with stable filename contract for the React component; (b) `apps/web/public/mascots/<sidekick>_{idle,reaction}.svg` for the supporting character; (c) `apps/web/src/components/MascotFrame.tsx` picks frame from `{state, recentLossStreak}`; (d) mounted on `/play/*` settle to give an emotional beat; (e) all SVGs are <8KB gzipped, `prefers-reduced-motion` respected (no looping animations under reduced-motion); (f) BRAND.md or UX_PLAN updated with the locked filenames + state machine; (g) ≥6 vitest cases. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_BANKROLL_CIRCUIT_BREAKER]** ROADMAP Phase 3: "Bankroll circuit-breaker + 24h PnL monitor cron live." Contract-side. Today `BunnyBagzBankroll.sol` has per-bet 0.5% cap; no drawdown circuit-breaker. **Acceptance:** (a) `BunnyBagzBankroll.sol` adds `circuitBreaker: { maxDrawdown24hWei, drawdownStartedAt, halted }` storage + `tripCircuitBreaker()` (only-owner) + `pause(allow,disallow) modifier`-gated `place()` rejecting when `halted=true`; (b) ≥10 forge tests cover: trip-on-threshold, untrip-after-cooldown, place-reverts-when-halted, owner-only trip, event emitted; (c) `slither` + `aderyn` + `medusa` still pass; (d) `halmos` symbolic proof extended to cover the halted invariant; (e) `apps/contracts/script/Deploy.s.sol` deploys with sane defaults (`maxDrawdown24hWei = 50% of seed`); (f) UX_PLAN footnote on operator playbook for trip recovery. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_PNL_MONITOR_CRON]** ROADMAP Phase 3: "24h PnL monitor cron live." OZ Defender already ships 3 monitors (Phase 2). Add the 24h PnL one. **Acceptance:** (a) new monitor JSON in `packages/contracts/defender/monitors/` reads `Bankroll.balance()` every 15 min, computes 24h delta, alerts on `delta < -maxDrawdown24hWei * 0.5` (early warning at 50% of breaker); (b) routes to existing `actions/telegram-forwarder.ts`; (c) `scripts/deploy-monitors.ts` already idempotent — re-running on an existing org adds the 4th monitor without dupes; (d) `scripts/test-webhook.sh` extended with a synthetic 24h PnL alert; (e) runbook §7 updated with "tripping threshold = 50% of circuit-breaker" guidance; (f) operator-gated for actual provisioning (per existing `[BB_PHASE2_DEFENDER_MONITOR]` pattern — agent ships configs, operator runs `--apply`). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_GEO_ENFORCE_MODE]** ROADMAP Phase 3: "Geo edge check switched to **enforce** mode; blocklist reviewed by counsel." Today: log-only per Phase 1 install. **Acceptance:** (a) `apps/api/geo/index.ts` (or wherever the geo middleware ships) flips on env `BUNNYBAGZ_GEO_MODE=enforce` to return `403` with a `regulatory_block` JSON for blocked country codes; (b) blocklist constant moved to `packages/chain/src/geo.ts` with named-country comments per the BUILD_PLAN list; (c) all `/play/*` server entrypoints + the wallet edge endpoint route through the middleware; (d) ≥6 vitest cases cover: allowed country, blocked country, missing country header (defaults to allow but logs), mode=log preserves existing behavior, mode=enforce returns 403 with right body, env unset defaults to log; (e) UI surface: when `geo:blocked` flag is set the page renders a polite "this region is not yet supported" placeholder per UX_PLAN constraints (no game UI); (f) operator-gated **flip** to `enforce` waits on counsel review of the blocklist (filed inline, not as a separate task). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_BUG_BOUNTY_PAGE]** ROADMAP Phase 3: "Bug-bounty page published (Immunefi or in-house)." Phase 2 shipped a placeholder `/bounty` page. Replace placeholder with real terms. **Acceptance:** (a) `apps/web/src/app/bounty/page.tsx` renders scope (in-scope contracts list, out-of-scope), severity table (critical/high/medium/low payouts), submission channel (PGP key + email + Immunefi URL once chosen), and the responsible-disclosure window; (b) `BUG_BOUNTY.md` at the repo root carries the canonical text used by Immunefi; (c) `/bounty` keyboard-accessible + thumb-zone-audit-clean + contrast-audit-clean; (d) operator decides Immunefi vs in-house; until decided the page surfaces the in-house process and a "scope expanding" banner; (e) ≥4 vitest cases (links, structural guard, accessibility, contrast). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_LEGAL_PAGES]** ROADMAP Phase 3: "Privacy / terms / responsible-gaming pages drafted with counsel." **Acceptance:** (a) `apps/web/src/app/privacy/page.tsx` + `terms/page.tsx` + `responsible-gaming/page.tsx` ship with first-draft copy operator can review before counsel signoff; (b) responsible-gaming page links to `BeGambleAware.org` + the in-app self-exclusion request channel + session-time-limit guidance; (c) `TrustFooter` extended to link all three; (d) ≥3 vitest cases per page cover routing + key copy presence + accessibility; (e) operator-gated counsel review tracked inline as a sub-bullet — the pages can ship with a `Draft — counsel review pending` ribbon until cleared. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_STATUS_PAGE_ONCALL]** ROADMAP Phase 3: "Status page + on-call rotation defined." **Acceptance:** (a) `apps/web/src/app/status/page.tsx` reads from a `GET /api/status` edge endpoint that aggregates: indexer reachable? (latency under 1500ms?), Defender monitors green?, bankroll non-zero?, last-bet block lag <5min; (b) page renders a per-component pill + last-incident timeline (read from `data/status_incidents.jsonl` git-tracked file); (c) on-call rotation defined in `docs/ONCALL.md` with operator + 1 fallback + paging contract (Telegram channel); (d) ≥4 vitest cases (ok/degraded/down branches + render); (e) the status endpoint is rate-limited to 30 req/min per IP. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_INTERNAL_BETA_TESTNET_7D]** ROADMAP Phase 3: "Internal closed beta — operator + invited friends play ≥7 days. Bug list closed before audit." **Operator-gated** start (operator invites). Agent prep is the test plan + bug-tracker template + daily summary cron. **Acceptance:** (a) `docs/INTERNAL_BETA_PLAN.md` carries the participant onboarding flow, the bug-report Telegram channel + form, the success criteria (≥50 settled bets per game, no critical incidents, no invariant violations); (b) `scripts/cron/cron_bb_beta_summary.sh` runs daily at 09:00 CET during beta, polls indexer for last-24h stats (bets per game, edge realized, p95 latency, any reverts) + writes to `memory/cron/bb_beta_<YYYY-MM-DD>.md`; (c) cron entry installable via `clarvis cron`; (d) operator runs the beta; agent reports daily and reopens any `[BB_PHASE3_BETA_FIX_<TAG>]` from observed bugs; (e) Phase 3 cannot exit until 7 consecutive green days are recorded. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_TREASURY_MULTISIG_3OF5]** ROADMAP Phase 3: "Treasury migrated from 1/1 owner to 3-of-5 multisig." **Operator-blocking** (signer set selection, hardware wallets, ceremony). Agent prep is the runbook + transition contract. **Acceptance:** (a) `BunnyBagzTreasury.sol` ships supporting 3-of-5 (e.g. Safe 1.4.x or `OwnableMultisig` minimal) with ≥10 forge tests covering quorum, replay protection, owner add/remove, ETH + USDM transfer paths, reentrancy guard; (b) `docs/MULTISIG_RUNBOOK.md` walks operator through Safe deployment, signer key generation per the BUILD_PLAN constraints (5 keyholders), the `bankroll.transferOwnership(safe)` ceremony, and the rollback plan; (c) deploy script supports `--apply-treasury-migration` that owner runs after the multisig is live; (d) on-chain proof archived at `memory/evolution/bb_phase3_multisig_<YYYY-MM-DD>.md` with the Safe address, all 5 signer addresses (operator-shareable), and the ownership-transferred tx hash. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_AUDIT_FIRM_ENGAGEMENT]** ROADMAP Phase 3: "External audit — single firm, ~1 week scope (contracts <800 SLOC). Findings remediated before mainnet." **Operator-blocking** (firm selection, contract execution, payment). Agent prep: scope doc + remediation tracker. **Acceptance:** (a) `docs/AUDIT_SCOPE.md` enumerates contracts in scope (Bankroll, Coinflip, Dice, HiLo, PermitForwarder, Treasury, plus any phase-3 additions) with current SLOC count, threat model, prior internal review notes, and the test+fuzz+symbolic coverage table; (b) operator picks firm + signs SOW; (c) audit kickoff: agent runs a final pre-audit sweep (slither + aderyn + medusa + halmos) and freezes `audit-frozen-<YYYY-MM-DD>` git tag; (d) findings tracked one-task-per-finding as `[BB_PHASE3_AUDIT_FIX_<TAG>]` until 100% remediated or formally accepted; (e) final audit report committed to `docs/audits/<firm>_<YYYY-MM-DD>.pdf`. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_AUDIT_FIX_TRACKER_BOOT]** Lightweight scaffold that becomes useful **only after** the audit firm delivers preliminary findings. **Acceptance:** create `docs/audits/REMEDIATION_TRACKER.md` template with columns `{finding-id, severity, file:line, status, fix-commit, audit-firm-confirmation}`, plus a `clarvis cron` job that diffs the tracker daily and reports anything stuck >3 days at H/Critical to operator Telegram. Lands as a no-op until findings exist. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_TESTNET_7D_INVARIANT_LOG]** ROADMAP Phase 3 exit: "testnet runs 7 days with no invariant violations." Today: no continuous monitoring outside the Defender monitors (Phase 2). Add a daily on-chain assertion sweep. **Acceptance:** (a) `scripts/cron/cron_bb_invariant_check.sh` runs daily at 06:00 CET, calls a thin TypeScript harness that asserts `bankrollBalance == sum(deposits) - sum(payouts) - sum(houseTake)` on chain via the indexer's bet log (replays expected delta), `commitUsed[c] == true` for every settled bet's commit, and `block.timestamp - lastSettleTimestamp < expectedMaxLatencyMs`; (b) records a one-line PASS/FAIL with the offending tx if any to `memory/cron/bb_invariant_<YYYY-MM-DD>.md`; (c) seven consecutive PASS days unblock the Phase 3 exit gate; (d) ≥6 vitest unit tests cover the harness logic in offline replay against fixture transactions. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_MAINNET_ALLOWLIST_SOFT_LAUNCH]** ROADMAP Phase 3 exit: "soft-launch deploy to mainnet behind a small allowlist." **Operator-blocking** (mainnet deployer key, bankroll seed funding, public announcement). Agent prep: contract supports allowlist + deploy script. **Acceptance:** (a) `BunnyBagzAllowlist.sol` (or storage in Bankroll) supports `addAllowed(addr[])`, `removeAllowed(addr[])`, `allowlistEnabled` flag; (b) `place()` reverts with `NotAllowed()` when `allowlistEnabled=true && !isAllowed[msg.sender]`; (c) deploy script for mainnet (chain 6342 or final mainnet id) ships with `allowlistEnabled=true` and a seed list operator provides; (d) UX surface: connected wallet not on allowlist sees a polite "Soft-launch in progress — request access" CTA instead of the bet UI; (e) ≥8 forge tests + ≥4 vitest tests; (f) mainnet deploy artifact archived at `packages/contracts/deployments/<mainnetChainId>.json`; (g) operator triggers the allowlist flip-off when the soft-launch window closes. (PROJECT:BUNNYBAGZ)
