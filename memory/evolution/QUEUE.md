...
- [x] **[AUTONOMOUS_EXECUTION_CAPABILITY_LIFT]** Audited 20 recent autonomous episodes — all `EXECUTION` exits=0, no lock contention, no wrong-model spawns. The score-vs-reality gap was an *assessor bug*, not a runtime regression: when the assessor's source fell back to `autonomous.log` (predictions had less data after rotation), the failure regex `"FAILED" in l and "Verification FAILED" not in l` was matching `POSTFLIGHT: SELF-TEST FAILED` (pytest health check), `POSTFLIGHT: PERF GATE: FAIL`, and task descriptions containing `FAILURE_HANDLING`. This deflated 100%-success days to 63–87% via-log readings (see capability_history.json: 04-22 0.61, 04-23 0.63 vs. 04-21 0.90 / 04-24 0.90 with same predictions). Fix: anchored success on `Recording outcome: success` and failure on the canonical `Recording outcome: {failure,timeout,crash}` plus `EXECUTION: exit≠0` cross-check (`clarvis/metrics/self_model.py:478`). Added `crash` outcome (was previously not counted at all). Regression tests in `tests/clarvis/test_self_model_scoring.py` cover SELF-TEST FAILED, PERF GATE: FAIL, FAILURE-named tasks, crash, plus a lock-step test that fails fast if production drifts from the test mirror. 9/9 scoring tests + 36/36 self_model suite pass. Live assessor confirmed 1.0 against current log.
