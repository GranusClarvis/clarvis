...
- [ ] **[BB_INDEXER_PHASE1_PONDER_REAL]** ([REOPENED] from `[BB_INDEXER_PHASE1_PONDER]`, drift confirmed 2026-05-01 + 2026-05-02). Replace `apps/indexer/README.md`-only stub with a real Ponder project. Ship: `apps/indexer/ponder.config.ts` indexing `BetPlaced` + `BetSettled` + `BetRefunded` from a configurable `BunnyBagzCoinflip` address (read from `packages/chain/src/index.ts` `ADDRESSES` so the same address book drives codegen + indexer); `apps/indexer/schema.graphql` with `bets` table (betId, player, side, stake, clientSeed, serverCommit, nonce, status, payout, serverReveal, blockPlaced, settledAt); `apps/indexer/api/history.ts` (or `apps/api/api/history.ts` — match the existing api app shape) handling `GET /api/history?address=&game=&limit=`; existing `RecentOutcomesStrip` already calls this endpoint. **Acceptance:** `pnpm --filter @bunnybagz/indexer test` (or `dev --once`) runs against an in-memory mock RPC fixture and writes ≥1 `bets` row; `apps/indexer/__tests__/history-handler.test.ts` covers populated + empty + bad-address responses; new file count for `apps/indexer/` ≥4 (config + schema + handler + test). Defer Fly.io + Neon to operator. (PROJECT:BUNNYBAGZ)
- [ ] **[BB_WAGMI_CLI_ABI_GEN_REAL]** ([REOPENED] from `[BB_WAGMI_CLI_ABI_GEN]`, drift confirmed 2026-05-01 + 2026-05-02). `apps/web/src/lib/abis.ts` is still hand-rolled with the literal comment `"Phase 1 deploy will replace this..."` — replace with a generated artifact. Ship: a script under `packages/chain/scripts/codegen-abis.mjs` (mirror the existing `chain-addresses` codegen script) that reads `packages/contracts/out/BunnyBagzCoinflip.sol/BunnyBagzCoinflip.json` (and Bankroll/Dice when present) and writes `packages/chain/src/abis/coinflip.ts` (and friends) with the typed-narrowed ABI; `apps/web/src/lib/abis.ts` re-exports from `@bunnybagz/chain`; CI sync check (mirror the `chain-addresses` job in `.github/workflows/ci.yml`) fails when the generated file is out of date. **Acceptance:** the literal `"Phase 1 deploy will replace this"` comment is gone from `abis.ts`; `pnpm --filter @bunnybagz/chain codegen:abis` is idempotent (running twice produces no diff); CI job runs the codegen + `git diff --exit-code` step; web suite still 132/132. (PROJECT:BUNNYBAGZ)
- [ ] **[BB_PERSISTENT_KV_ADAPTER_REAL]** ([REOPENED] from `[BB_PERSISTENT_KV_ADAPTER]`, drift confirmed 2026-05-01 + 2026-05-02). `apps/api/lib/seedStore.ts` ships `MemorySeedKV` only; `getKV()` ignores `BUNNYBAGZ_KV_BINDING` (the only string referencing the env is a comment). Add a `CloudflareKVSeedKV` and an `UpstashRedisSeedKV` class behind the existing `SeedKV` interface; `getKV()` selects on `BUNNYBAGZ_KV_BINDING` env (`memory` | `cloudflare` | `upstash`); both new classes accept their dependency as constructor injection so tests can mock without hitting the network. **Acceptance:** `apps/api/lib/seedStore.test.ts` extended with new cases for both adapters using injected mocks (round-trip put/get/delete + TTL behavior); `BUNNYBAGZ_KV_BINDING=upstash` selects `UpstashRedisSeedKV`; `BUNNYBAGZ_KV_BINDING=cloudflare` selects `CloudflareKVSeedKV`; missing/empty env still returns memory adapter; api suite ≥40/40 (was 36; +4 for the two new adapters). Operator-gated for the production binding setup, but the impl + tests ship without that. (PROJECT:BUNNYBAGZ)
