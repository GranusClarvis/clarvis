...
- [x] **[FAILURE_HISTOGRAM_TRUTH_AUDIT]** Done 2026-05-02 — `docs/internal/audits/FAILURE_HISTOGRAM_AUDIT_2026-05-02.md`. Verdict: **claim confirmed and understated.** Of 20 non-success episodes in last 100, 16 (94% of plain-`action`) classify into a more specific root cause; the dominant root causes are `classifier_misclassified` (50%, agent reported success but episode tagged partial) and `lint_structure_fail` (25%, `code_validation` >100-line lint downgrading shipped work). Recommendation: ship `[FAILURE_TYPE_REFINEMENT]` **partial** — fix those two postflight artifacts first (expected ESR lift ~0.07-0.10), then build residual taxonomy. Two buckets to instrument first: `classifier_misclassified` and `lint_structure_fail`, both root-cause classifiers in `clarvis/cognition/metacognition.py`. Source canonical store is `data/episodes.json`, not `data/episodes/*.jsonl` (glob in task prompt was stale).
