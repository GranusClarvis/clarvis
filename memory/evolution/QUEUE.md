...
- [x] **[BB_PHASE2_THUMB_ZONE_DICE_HILO]** Shipped 2026-05-04 (commit `caabb1e` on `feature/mvp-planning-and-rebrand`). Added `dice-primary-cta` (`/play/dice`) + `hilo-primary-cta` (`/play/hilo`) to `THUMB_ZONE_TARGETS`; extended the test file with `DicePage` + `HiloPage` mounts. Audit grew from 6 → 10 layout cases (5 routes × 2 viewports), all passing `MIN_THUMB_ZONE_RATIO` via the existing `marginTop: auto` panel anchor on both pages — no layout fix needed. (PROJECT:BUNNYBAGZ)
