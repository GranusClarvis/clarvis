...
- [ ] **[SWO_V2_COMPANION_STATS_SCHEMA]** Add a tamagotchi-style stats layer to `SanctuaryCompanion`. Three stats minimum — **hunger**, **energy**, **happiness** (each 0–100, lower = needs attention) — plus a per-stat decay rate per real-time hour and a `stats_updated_at` timestamp. Implement a **pure** helper `decayStats(companion, now)` that returns projected current values without writing. Add GET `/api/sanctuary/companion/stats?token_id=X` returning `{ stats, needs[], updated_at }`. Add server-side `tickStats(companion)` invoked from existing `interact` paths so persisted values converge. Schema migration is additive (no removal); existing companion API stays green; no UI yet. **Acceptance:** SQLite migration ships with backfill of `stats_updated_at = now`, default stats `{hunger:80, energy:80, happiness:80}`; `decayStats` unit-tested for monotonic decay across N hours and capped at 0/100; stats GET returns deterministic values for a fixed clock; `npm run type-check && npm run test` pass. Touch: `lib/db.ts`, `lib/db/migrations/`, `app/api/sanctuary/companion/stats/route.ts`, `lib/sanctuary/decay.ts` (new). (PROJECT:SWO, P0, Track A)



- [ ] **[SWO_V2_COMPANION_SCREEN_SURFACE]** Add a dedicated Companion surface — either a new route `/sanctuary/companion` or a tab inside the existing `?v=2` page — that becomes the **default landing** for owners with an active Skrumpey (deep links to the world hub still work). Renders: large animated companion sprite (mood-driven), 3 stat bars (hunger/energy/happiness), single most-acute need callout, bond + level + nickname, last 3 journal lines, quick-action buttons (Feed / Pet / Talk / Sleep / Play), Chat button, and a clear "Enter Sanctuary" CTA returning the player to the V2 world. **No new art** — reuse companion sprite + existing HUD components. **Acceptance:** companion screen renders for an owner with an active Skrumpey; stat bars live-update after each action without full reload; "Enter Sanctuary" returns to the existing V2 hub; users without an active Skrumpey see the existing pick-a-Skrumpey UX (do not re-implement); screenshot diff in PR. Touch: `app/sanctuary/CompanionView.tsx` (new), `app/sanctuary/SanctuaryRouter.tsx`, `components/sanctuary/overlays/CompanionHUD.tsx`. (PROJECT:SWO, P0, Track A)

- [ ] **[SWO_V2_COMPANION_NEED_ALERTS]** Soft in-page alert in `CompanionHUD.tsx` when any stat drops below 30 — single icon + tooltip + click-to-jump-to-companion-screen. Never auto-opens an overlay; never blocks gameplay; clears once the relevant action is performed. Uses existing HUD layout — no relayout. **Acceptance:** alert appears at threshold, clears after action; never fires more than once per stat per cross; HUD remains pixel-stable when alert toggles. Touch: `components/sanctuary/overlays/CompanionHUD.tsx`, `lib/sanctuary/needs.ts` (new pure helper). (PROJECT:SWO, P1, Track A)

- [ ] **[SWO_V2_COMPANION_CHAT_KNOWS_STATS]** Inject current (projected) stats + active needs + last 3 actions into the LLM chat system prompt so the companion can reference them in voice ("I'm a bit hungry today" / "Thanks for the nap"). Keep the existing bond-tier tone layer. **No raw numbers in replies** — the system prompt asks the model to reference state qualitatively. Injected blob ≤200 tokens. **Acceptance:** 80%+ of test prompts produce replies that reference at least one current stat or recent action qualitatively; no replies leak raw stat numbers; chat history pagination unaffected; token budget for the injection ≤200 verified by an offline counter. Touch: `app/api/sanctuary/companion/chat/route.ts`, `lib/sanctuary/chatPersonality.ts`. (PROJECT:SWO, P1, Track A)

- [ ] **[SWO_V1_ARCHIVE_FORMALIZE]** Document and signal V1 archival posture. (1) Add a single banner inside the V1 surface in `SanctuaryContent.tsx`: "Sanctuary has moved — open the new Sanctuary" with a button linking to `?v=2`. (2) Add a one-line note in `SWO_TRACKER.md` and `swo_sanctuary_v3_deferred_2026-04-26.md` confirming V1 is fallback-only. **Do not remove V1 code** in this PR — avoid breaking deep links. **Acceptance:** banner ships; CTA link works; tracker + V3-deferred doc updated; no V1 code removed. Touch: `app/sanctuary/SanctuaryContent.tsx`, `memory/evolution/SWO_TRACKER.md`, `memory/evolution/swo_sanctuary_v3_deferred_2026-04-26.md`. (PROJECT:SWO, P1)
...
- [ ] **[SWO_V2_DESLOP_SHADER]** Phaser post-FX `PostFXPipeline` palette-quantize (+ optional Bayer dither) pass over the canvas to a fixed N-color SWO palette. Dev-flag toggle (`?v=2&deslop=1` + `localStorage.swo_deslop`); ship enabled-by-default only after operator A/B inspection. **Status: built locally on branch `clarvis/star-world-order/t0426200012-3b08` (commit `47d9b08`); needs PR + merge.** Highest-leverage V2 visual win. (PROJECT:SWO, P0, Track B)
- [ ] **[SWO_V2_PLAYER_SPRITE_ALIASING]** Fix `public/sanctuary/player.png` aliasing. Root cause is non-integer scaling at render time. Either (a) redraw the sprite at the exact display size used by `PlayerSprite.ts` / scene scale, or (b) lock the scale factor to the nearest integer (1×, 2×, 3×) and resize the asset to compensate, plus set `setScaleMode(ScaleModes.NEAREST)` and `roundPixels: true` in scene config. **Do NOT use RD.** Acceptance: at default zoom the player sprite shows hard pixel edges (no half-pixel smear), pixel-snap verified with screenshot, scale factor in code is an integer constant. (PROJECT:SWO, P0, Track B)
- [ ] **[SWO_V2_NPC_DISPLAY_SIZE]** Reduce NPC display size from 64 → 48 px in `game/sprites/NPCSprite.ts`. Update sprite scale + collider/hit-region constants accordingly so click targets and pathing still match. Verify NPCs no longer dwarf the player at 48 px and that NPC nameplate offsets still anchor correctly. Acceptance: constant changed, hit area/spacing updated, screenshot diff in PR. (PROJECT:SWO, P0, Track B)
- [ ] **[SWO_V2_ROOM_PALETTE_STANDARDIZE]** Standardize palette across the 8 painted room backgrounds in `public/sanctuary/rooms/` (or wherever they live in the V2 asset tree — verify path during the read step). Use a batch color-adjust script (Python+Pillow or ImageMagick) — **not** RD regeneration — that snaps each room PNG to the shared SWO palette established by `[SWO_V2_DESLOP_SHADER]`. Output deltas to a side-by-side review folder before overwriting. Acceptance: script lives under `scripts/v2/standardize_room_palette.{mjs,py}`, all 8 rooms re-exported, palette histogram diff shows convergence to the shared palette ±5% tolerance, no asset is RD-regenerated. (PROJECT:SWO, P1, Track B)
- [ ] **[SWO_V2_HALFWIRED_FEATURE_FINISH]** Pick **one** of the half-wired V2 features identified in the recent audit and either finish it or remove it cleanly: (a) `DevMapEditor` is mounted with no UI; (b) `MultiplayerBridge` is incomplete; (c) `click-to-move` is a stub. Recommended pick: **DevMapEditor** — surface the panel with a minimum viable UI (toggle hotkey, tile-paint mode, save-to-JSON button) since it's a dev-only tool with no risk surface; or, if the panel is dead weight, delete the mount + component file in one PR. Whichever is chosen, the other two are documented as "intentionally stubbed" in `components/sanctuary/README.md` (or equivalent) so future audits don't re-flag them. Acceptance: chosen feature is either functional end-to-end or fully removed; the other two are documented as deliberate stubs with one-line rationale. (PROJECT:SWO, P1, Track B)
- [ ] **[SWO_V2_NPC_SELECT_SKRUMPEY_GATE]** NPC click → "select-a-Skrumpey" UX gate. When the player clicks an NPC and there is no active Skrumpey, branch as: (a) operator owns exactly one Skrumpey → auto-select it and proceed with the interaction; (b) operator owns multiple → show a guidance overlay listing owned Skrumpeys with a "Jump to selection" button that opens the existing Skrumpey-picker; (c) operator owns none → show the existing "no Skrumpey" message with a clear CTA. Wire through `SanctuaryContent.tsx` / NPC click handler in `NPCSprite.ts`. Same UX gate is shared with the new companion screen → world transition. Acceptance: all three branches reachable in dev, auto-select path produces zero extra clicks for single-owner case, multi-owner path opens picker, screenshots in PR. (PROJECT:SWO, P1, Track B)
...
- [ ] **[GOAL_STATE_DEDUP_AND_PROGRESS_WIRING]** `brain.get_goals()` returned duplicate same-day priority entries with `progress: 0`, which makes the goal graph look richer than the execution layer actually is. Audit the goal write path that produced `clarvis-goals_evolved_20260426_070015` and `clarvis-goals_20260426_070016`, dedupe near-identical goals, and wire a minimal progress updater so active weekly priorities stop sitting at 0 forever. Acceptance: duplicate canonical-priority goals collapse to one live record and at least the active weekly goal carries nonzero or explicitly-updated progress metadata. (PROJECT:CLARVIS)
- [ ] **[AUTONOMOUS_OUTPUT_VALIDATION_FALSE_PARTIAL_AUDIT]** This week delivered many real successes that were still downgraded to `partial_success` because the output validator saw weak structure or missing markers even when PRs/tests landed cleanly. Audit the last 20 downgraded autonomous/project-agent outcomes, classify true vs false partials, and tune the validator so shipped work with passing tests/PR evidence is not mislabeled. Acceptance: report written with counts + root causes; at least one validator fix lands or a narrower downgrade policy is documented. (PROJECT:CLARVIS)
- [ ] **[DAILY_LOG_COVERAGE_GUARD]** Weekly review found missing daily logs for 2026-04-20 through 2026-04-22, which weakens continuity and makes trend review lossy. Add a lightweight guard in the daily/cron writing path that ensures `memory/YYYY-MM-DD.md` exists before first write and emits an alert if a UTC day closes without a daily file. Acceptance: backstop creates the file shell automatically and the next missed day is detectable from logs/alerting instead of silently disappearing. (PROJECT:CLARVIS)
