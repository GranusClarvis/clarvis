...
- [x] [UNVERIFIED] **[QUEUE_ARCHIVE_DRIFT_REPAIR]** Execution-shape warning surfaced: "8 task(s) succeeded but checkbox still [ ] — sidecar/QUEUE.md drift; archive_completed may be stalled." Diagnose `clarvis.queue.writer.archive_completed()` (and any cron caller) for why successful sidecar entries are not flipping the QUEUE.md checkboxes. Likely candidates: (a) ID match relies on a key the writer no longer stamps, (b) lockfile contention with the maintenance lock, (c) the cron entry that calls it stopped firing. Acceptance: a one-shot reconciliation script catches up the 8 stuck tasks; the underlying cause is fixed (not just the symptom); a regression test simulates a succeeded sidecar entry → verifies the checkbox flips on the next archiver run. (PROJECT:CLARVIS) — DONE 2026-04-30. Root cause: `engine._mark_checkbox` used `count=1` so when `auto_split` spawned multiple `[ ] [TAG]` lines sharing one tag, only the first flipped; the rest stayed unchecked while sidecar said "succeeded". Fixes: (1) `_mark_checkbox` now flips ALL matching lines (incl. indented), (2) `archive_completed` self-heals by flipping `[ ]` → `[x]` for any tag whose sidecar is `succeeded` before sweeping. One-shot reconciliation: ran `archive_completed()` live → archived 12 stuck entries, runnable_view severity now `ok` (no findings). Regression test: `tests/test_queue_archive_drift.py` (5 cases). All 78 existing queue tests still pass.
