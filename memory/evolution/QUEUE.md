...
### Star Sanctuary — V2-Only Operating Model (PROJECT:SWO, reset 2026-04-26)

> **V3 is DEFERRED.** Stop touching `game/v3/`, `public/sanctuary-v3/`, `scripts/v3/`, `docs/SANCTUARY_V3.md`. The `?v=3` route stays for archival access only. **No new PRs tagged `[SWO_V3_*]`. No further RD credit spend on V3** (~5 generation passes burned without parity vs V2; remaining RD balance reserved for V2 polish only after explicit operator approval). Why: V3 was a pixel-art-from-scratch parallel rebuild that never reached V2's painted-hub parity, and the FM aesthetic does not compose with V2's painted assets, so V3 work did not help V2. Full inventory: SWO repo `docs/SANCTUARY_V3_DEFERRED.md`. Replan + alignment-execution docs in this workspace are SUPERSEDED — see V3-DEFERRED note at top of each.

_**Active surface — V2:** page `app/sanctuary/SanctuaryV2.tsx` + `SanctuaryContent.tsx`; Phaser mount `components/sanctuary/PhaserGame.tsx`; game code under `game/scenes/`, `game/sprites/`, `game/systems/`, `game/config/`; assets in `public/sanctuary/`; routing `?v=2` (or no param + `NEXT_PUBLIC_SANCTUARY_V2=true`)._
...
_**Primary V2 goal (2026-04-26): reduce AI-slop.** Treat the painted hub map and 8 painted room backgrounds as fixed inputs — fix presentation (palette, scaling, aliasing, layout chrome) instead of regenerating._
...
_**Hard out-of-scope (operator brief 2026-04-26)** unless operator re-opens: replacing painted room backgrounds with new pixel art; replacing the hub map; touching V3; generating new RD assets; voice chat; multiplayer infra beyond what's already shipped._

#### V2 — Active Surface (priority queue, 2026-04-26)

_Six priorities ordered by operator brief. Items 1–3 are de-slop visual fixes; 4 standardises existing painted assets; 5 closes half-wired features; 6 is a UX gate. **No RD credits.** Acceptance criteria are concrete so each item produces one merge-able PR._

- [ ] **[SWO_V2_PLAYER_SPRITE_ALIASING]** Fix `public/sanctuary/player.png` aliasing. Root cause is non-integer scaling at render time. Either (a) redraw the sprite at the exact display size used by `PlayerSprite.ts` / scene scale, or (b) lock the scale factor to the nearest integer (1×, 2×, 3×) and resize the asset to compensate, plus set `setScaleMode(ScaleModes.NEAREST)` and `roundPixels: true` in scene config. **Do NOT use RD.** Acceptance: at default zoom the player sprite shows hard pixel edges (no half-pixel smear), pixel-snap verified with screenshot, scale factor in code is an integer constant. (PROJECT:SWO, P0)
- [ ] **[SWO_V2_NPC_DISPLAY_SIZE]** Reduce NPC display size from 64 → 48 px in `game/sprites/NPCSprite.ts`. Update sprite scale + collider/hit-region constants accordingly so click targets and pathing still match. Verify NPCs no longer dwarf the player at 48 px and that quest-NPC nameplate offsets still anchor correctly. Acceptance: constant changed, hit area/spacing updated, screenshot diff in PR. (PROJECT:SWO, P0)
- [ ] **[SWO_V2_ROOM_PALETTE_STANDARDIZE]** Standardize palette across the 8 painted room backgrounds in `public/sanctuary/rooms/` (or wherever they live in the V2 asset tree — verify path during the read step). Use a batch color-adjust script (Python+Pillow or ImageMagick) — **not** RD regeneration — that snaps each room PNG to the shared SWO palette established by `[SWO_V2_DESLOP_SHADER]`. Output deltas to a side-by-side review folder before overwriting. Acceptance: script lives under `scripts/v2/standardize_room_palette.{mjs,py}`, all 8 rooms re-exported, palette histogram diff shows convergence to the shared palette ±5% tolerance, no asset is RD-regenerated. (PROJECT:SWO, P1)
- [ ] **[SWO_V2_HALFWIRED_FEATURE_FINISH]** Pick **one** of the half-wired V2 features identified in the recent audit and either finish it or remove it cleanly: (a) `DevMapEditor` is mounted with no UI; (b) `MultiplayerBridge` is incomplete; (c) `click-to-move` is a stub. Recommended pick: **DevMapEditor** — surface the panel with a minimum viable UI (toggle hotkey, tile-paint mode, save-to-JSON button) since it's a dev-only tool with no risk surface; or, if the panel is dead weight, delete the mount + component file in one PR. Whichever is chosen, the other two are documented as "intentionally stubbed" in `components/sanctuary/README.md` (or equivalent) so future audits don't re-flag them. Acceptance: chosen feature is either functional end-to-end or fully removed; the other two are documented as deliberate stubs with one-line rationale. (PROJECT:SWO, P1)
- [ ] **[SWO_V2_NPC_SELECT_SKRUMPEY_GATE]** NPC click → "select-a-Skrumpey" UX gate. When the player clicks an NPC and there is no active Skrumpey, branch as: (a) operator owns exactly one Skrumpey → auto-select it and proceed with the interaction; (b) operator owns multiple → show a guidance overlay listing owned Skrumpeys with a "Jump to selection" button that opens the existing Skrumpey-picker; (c) operator owns none → show the existing "no Skrumpey" message with a clear CTA. Wire through `SanctuaryContent.tsx` / NPC click handler in `NPCSprite.ts`. Acceptance: all three branches reachable in dev, auto-select path produces zero extra clicks for single-owner case, multi-owner path opens picker, screenshots in PR. (PROJECT:SWO, P1)
