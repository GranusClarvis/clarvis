...
- [x] **[BB_PHASE3_PNL_MONITOR_CRON]** ROADMAP Phase 3: "24h PnL monitor cron live." (2026-05-06) **Configured (operator-gated for `--apply`).** Shipped: (a) `packages/contracts/defender/monitors/04-pnl-monitor-24h.json` STAT-style entry, cron `*/15 * * * *`, reads `bankroll.circuitBreakerState()` (which already tracks the 24h window on-chain), alerts when `windowStartBalance − currentBalance > maxDrawdown24hWei / 2` — 50% early warning of the auto-trip threshold; (b) `actions/telegram-forwarder.ts` extended with `handlePnlCheck()` route + scheduled-invocation detection; (c) `scripts/deploy-monitors.ts` now splits BLOCK monitors (existing `client.monitor.create`) from STAT entries (provisioned as a separate scheduled Defender Action `bunnybagz-pnl-watcher` sharing the same packaged source) — idempotent upsert by Action name; (d) `scripts/test-webhook.sh` fires the 3-monitor overview message AND a synthetic 24h PnL early-warning sample; (e) `docs/CRYPTO_RUNBOOK.md` §7.0 updated to a 4-row monitor table + explicit "tripping threshold = 50% of `maxDrawdown24hWei`" guidance + new P2 priority for the early-warning route; (f) provisioning still operator-gated on the Defender org token + `encodeAction` packing stub. (PROJECT:BUNNYBAGZ)
