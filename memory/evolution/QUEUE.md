...
- [x] **[CRON_SCHEDULE_DRIFT_AUDIT_BASH]** Done 2026-05-01 — `scripts/cron/cron_schedule_audit.sh` diffs `crontab -l` vs `clarvis cron list` vs CLAUDE.md schedule table. Exits 1 on drift; reports time mismatches, missing-managed, and unmanaged-crontab buckets into `monitoring/cron_drift.log`. Wired at Sun 05:24 (05:25 was already taken by `learning_strategy`; 05:24 still falls between worktree-cleanup at 05:22 and cleanup at 05:30 — same intent). CLAUDE.md schedule table updated with the new entry. Verified: baseline clean (exit 0); planted bogus crontab line → exit 1 with offending line logged; planted time-mismatch fixture → exit 1 with both schedules logged. Test override: `CRON_AUDIT_CRONTAB_FILE=<path>` substitutes for `crontab -l`.
