...
- [x] **[POSTFLIGHT_IMPORT_MISS_TELEMETRY]** (2026-05-01) Added `_probe_import_misses()` after import block in `scripts/pipeline/heartbeat_postflight.py` (tracks 8 hooks: conf_outcome, pred_resolve_enhanced, close_chain, record_use, learn_from_task, EpisodicMemory, write_digest, log_decision). Writes `data/audit/postflight_import_misses.jsonl` iff ≥1 None resolved. `cron_report_evening.sh` now surfaces `postflight_import_misses_24h: N` + top-3 missing names. Verified: probe correctly skips write when all hooks resolve (current state); parser tested with synthetic data.
