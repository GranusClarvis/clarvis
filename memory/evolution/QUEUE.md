...
- [x] **[BB_PHASE3_DESKTOP_LAYOUT_PASS]** Stop showing a 480px column on a 1920px monitor. Closes audit P1-7. UX_PLAN §2: "Desktop adaptation: same content, one extra column for activity & verify panel. Maximum width capped at 1280px." **Acceptance:** (a) at viewport ≥1024px, `play/{coinflip,dice,hilo}/page.tsx` shows a 3-column layout: left rail (mascot + verify badge + recent outcomes), center (game viewport + bet panel), right rail (live activity feed + verify-this-bet card); (b) page max-width caps at 1280px; (c) at <1024px the right rail collapses below; (d) at <768px the left rail also collapses (mobile-first stack); (e) ≥6 vitest tests across breakpoints + Playwright captures at 1440 / 1024 / 768 / 375. (PROJECT:BUNNYBAGZ) — shipped 2026-05-07: `apps/web/src/components/PlayPageFrame.tsx` (responsive 3-col CSS grid + `useBreakpoint` hook), `VerifyBadge.tsx`, `VerifyBetCard.tsx`, CSS rules in `apps/web/src/app/globals.css` (`.bb-play-frame*` with media queries 768/1024px, max-width 1280px), all three `/play/{coinflip,dice,hilo}/page.tsx` wired through the frame with rails (in-page mascot + recent-outcomes-strip moved to left rail to avoid testid duplication). Tests: 8 new vitest in `apps/web/src/components/__tests__/PlayPageFrame.test.tsx` (covers all 3 breakpoints + resize + a11y + max-width hook) + Playwright `e2e/play-desktop-layout.spec.ts` capturing 12 screenshots (3 games × 4 breakpoints: 1440/1024/768/375). Web suite: 432/432 vitest pass, typecheck clean.
