...
- [x] **[BB_PHASE3_AUDIT_FIRM_ENGAGEMENT]** Agent prep complete 2026-05-06: `docs/AUDIT_SCOPE.md` shipped (mega-house repo) with contracts in scope (Bankroll/Coinflip/Dice/HiLo/Randomness/PermitForwarder; **918 SLOC** — bankroll IS the treasury, no separate Treasury contract), full test+fuzz+invariant+symbolic+greybox+static coverage table (141 forge tests + Halmos + Medusa + Slither + Aderyn), threat model across 6 areas (solvency, randomness/commit-replay, payout math, bankroll boundary, PermitForwarder, ops), prior internal review notes pointing to PHASE1_REPORT, plus §6 kickoff procedure (forge clean+test → halmos → medusa → slither → aderyn → `audit-frozen-<YYYY-MM-DD>` tag). Steps (b)-(e) remain operator-blocking (firm selection, SOW, payment, kickoff trigger). ROADMAP Phase 3: "External audit — single firm, ~1 week scope (contracts <800 SLOC). Findings remediated before mainnet." (PROJECT:BUNNYBAGZ)

- [x] **[BB_PHASE3_AUDIT_FIX_TRACKER_BOOT]** Done 2026-05-06: `docs/audits/REMEDIATION_TRACKER.md` template shipped (mega-house repo) with all required columns plus severity legend, status state machine (`open`→`in-progress`→`fix-merged`→`firm-confirmed` or terminal `accepted`/`disputed`), aggregate state table, Phase-3 exit gate spec, and operating procedure. No-op cron `scripts/cron/cron_bb_audit_remediation_check.sh` ships with `BUNNYBAGZ_AUDIT_ACTIVE` gate, parses the tracker for non-terminal H/C rows, escalates to operator Telegram via inline urlopen using openclaw.json creds. Smoke-tested: gates off correctly, parses 0 overdue rows on empty template, exits clean. Schedule line will be added to crontab once audit firm engaged. (PROJECT:BUNNYBAGZ)
