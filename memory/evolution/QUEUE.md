...
- [x] **[BB_PHASE2_TRUTH_AUDIT_FOLLOWUP_2]** Audit shipped 2026-05-06 at `memory/evolution/bb_phase2_truth_audit_2026-05-06.md`. Verdict: **Phase 2 NOT archived as DONE**. 3 of 4 ROADMAP exit lines hold on disk (3 games × ETH+USDm playable, recent bets in WalletSheet, light+dark themes intentional). Fourth line ("e2e green") fails: latest CI run on `feature/mvp-planning-and-rebrand` (`08dcdfb`, run 25388220194) showed `1 skipped 6 failed 20 passed` — 3× `coinflip.spec.ts:15` + 3× `hilo.spec.ts:26` (HiLo direction CTAs only render after `openSession`, but unconnected smoke spec asserts pre-session). Forge build + halmos + medusa also red on the same run. Plus 4 unpushed local commits past the last CI'd head. Filed `[BB_PHASE2_E2E_GREEN_GAP_FIX]` + `[BB_PHASE2_FORGE_BUILD_RED_FIX]` + `[BB_PHASE2_PUSH_LOCAL_FEATURE_BRANCH]` below. Phase 3 P1 promotion stays blocked. Test inventory healthy: web 332/332, api+indexer+verify all 0-fail. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_E2E_GREEN_GAP_FIX]** Fix the 6 red Playwright specs blocking the ROADMAP Phase 2 "e2e green" exit gate. Failing on chromium + mobile-chromium + mobile-safari: (1) `apps/web/e2e/coinflip.spec.ts:15` "Home → /play → Coinflip card → /play/coinflip surface" — investigate via `playwright-report` artifact at run 25388220194; (2) `apps/web/e2e/hilo.spec.ts:26` "Higher / Lower direction buttons are present (UX_PLAN §3 thumb-zone)" — `getByTestId('hilo-dir-higher')` not visible because the HiLo state machine no longer renders the direction CTAs until `openSession` succeeds; either drive openSession in the spec, or render disabled CTAs pre-session so the testids are present from page load. **Acceptance:** one fresh `feature/mvp-planning-and-rebrand` CI run shows the `e2e:` job green (18/18 specs pass, 0 skipped, 0 flaky); run URL appended to `memory/evolution/bb_phase2_truth_audit_2026-05-06.md`. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_FORGE_BUILD_RED_FIX]** `packages/contracts (Foundry)` job failed at `forge build` on run 25388220194 (16s); halmos + medusa also failed downstream. Likely culprit: a Phase-3 contract change (`a11c14e` BunnyBagzBankroll circuit-breaker is currently *unpushed* but the pushed-most-recent `08dcdfb` may also have side effects). Run `forge build --sizes` locally on `packages/contracts/`, identify the failing source file, fix or revert, and verify halmos + medusa pass. **Acceptance:** the same CI workflow shows green `contracts` + `halmos` + `medusa` jobs on the next run. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_PUSH_LOCAL_FEATURE_BRANCH]** Push the 4 local commits ahead of `origin/feature/mvp-planning-and-rebrand` so CI gates the head of the lane: `f6993f5` (RECENT_OUTCOMES_DICE_HILO), `a11c14e` (BANKROLL_CIRCUIT_BREAKER), `03af4f0` (PNL_MONITOR_CRON), `3808b0b` (GEO_ENFORCE_MODE). Recurrent obligation: git-hygiene escalation level 1 today. **Acceptance:** `git ls-remote origin feature/mvp-planning-and-rebrand` returns `3808b0b` (or whatever the local head is at push time); a fresh CI run is triggered on the new head. Pair with `[BB_PHASE2_E2E_GREEN_GAP_FIX]` so the green run is captured on the up-to-date head, not the stale `08dcdfb`. (PROJECT:BUNNYBAGZ) (2026-05-06)
