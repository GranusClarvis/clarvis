...
- [x] **[BB_PHASE2_MEDUSA_FUZZ]** Coverage-guided invariant fuzzing via Trail of Bits Medusa (the 2026 standard, supersedes Echidna for new projects per `docs/CRYPTO_RUNBOOK.md` § 5). Layout from [Recon-Fuzz/create-chimera-app](https://github.com/Recon-Fuzz/create-chimera-app). Acceptance: `medusa.json` config; new CI job; one shared invariant file proving `invariant_systemNeverInsolvent` on Coinflip/Dice/HiLo runs in <2 min; coverage uploaded as artifact. (PROJECT:BUNNYBAGZ) (2026-05-03) — SHIPPED 2026-05-04: `packages/contracts/medusa.json` (90s timeout, property-mode, coverage HTML+lcov), `packages/contracts/test/MedusaInvariants.t.sol` (`BunnyBagzMedusaTester` exposes 9 funds-flow handlers across all 3 games + `property_systemNeverInsolvent`; `BunnyBagzMedusaInvariant` Foundry shim runs the same target → 4096 calls, 0 reverts, green in 686ms locally). New `medusa` CI job in `.github/workflows/ci.yml` pinned to `medusa@v0.1.6` via `go install`, uploads `medusa-corpus/ + crytic-export/` artifact. `.gitignore` updated. Full forge suite still 117/117 green.
