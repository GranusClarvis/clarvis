...
- [x] **[BB_PHASE3_MAINNET_ALLOWLIST_SOFT_LAUNCH]** Agent prep complete (2026-05-07): (a) `BunnyBagzAllowlist.sol` ships with `addAllowed(address[])`, `removeAllowed(address[])`, `setAllowlistEnabled(bool)`, `gateOpen()`, `enforceAccess()`. (b) `placeBet`/`openSession` on Coinflip+Dice+HiLo revert with `NotAllowed()` when allowlist is set + enabled and caller is not allowlisted. (c) `script/Deploy.s.sol` extended (BB_DEPLOY_ALLOWLIST=true, BB_ALLOWLIST_ENABLED=true, BB_ALLOWLIST_SEED CSV); `script/deploy-mainnet.sh` wrapper added. (d) `apps/web/src/components/AllowlistGate.tsx` + `lib/allowlist.ts` render polite "Soft-launch in progress — request access" CTA when blocked. (e) 14 forge tests pass (BunnyBagzAllowlistTest), 11 vitest tests pass (AllowlistGate.test.tsx); full suite 155 forge tests green. **Operator-blocking remains**: (f) mainnet deploy artifact (operator must run `deploy-mainnet.sh`) and (g) operator flips gate off post-soft-launch. (PROJECT:BUNNYBAGZ)
