...
- [ ] [UNVERIFIED] **[BB_VERIFY_API_INDEXER_INTEGRATION]** Replace the `pending-indexer` stub in `apps/api/verify/[betId].ts` with a real read against the indexer (or, fallback: a viem `publicClient.getLogs` over the `BetSettled` event filtered by betId). Resolve `(serverSeed, clientSeed, nonce)`, return the `verified` payload the web page already consumes. Acceptance: a settled testnet bet's `/verify/<betId>` page shows the `verified-match` block end-to-end; existing 8 vitest cases stay green; add 1 integration test against a mocked indexer response. (PROJECT:BUNNYBAGZ)
