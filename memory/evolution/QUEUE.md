...
- [ ] Wire smart_recall() into downstream systems — replace brain.recall() with smart_recall() in task_selector.py, procedural_memory.py, and reasoning_chain_hook.py. Measure hit rate improvement in production usage
...
- [ ] Fix capability assessors ceiling effect — current 7 assessors are binary (has/not-has), all hit 1.0. Redesign with quality tiers: measure retrieval hit rate (not just "recall operational"), reasoning chain depth/quality, procedure reuse frequency, prediction calibration accuracy
