...
- [x] **[BB_PHASE2_HILO_STEP_API_AND_KEEPER]** SHIPPED 2026-05-05 — `apps/api/hilo/step.ts` edge handler + `apps/web/src/app/api/hilo/step/route.ts` Next.js proxy + `HILO_PLAY_STEP_ABI` / `playStepSettler` / `pickHiloStepEvent` discriminated-union decoder + `setSessionCommitResolver` injection in `apps/api/lib/settler.ts`. Tests: 10 cases in `apps/api/hilo/step.test.ts` (happy win + happy tie + reveal-mismatch revert + commit-already-used revert + 405/400/404/502 paths) and 5 cases in `apps/api/lib/settler.test.ts` for the event decoder. Both api (75/75) and web (229/229) suites green; `tsc --noEmit` clean for api. (PROJECT:BUNNYBAGZ)
