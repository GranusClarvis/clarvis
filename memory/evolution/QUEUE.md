...
- [ ] [RESEARCH_REPO_OPENSTINGER] Review repo: https://github.com/srikanthbellary/openstinger — what it is, how it works, and whether any components/patterns are worth integrating into Clarvis (agent autonomy, security, orchestration, memory, tooling). Output: 10 bullets + 3 concrete integration ideas or "discard" with reason.
...
- [ ] [SPINE_TEST_SUITE] Create `clarvis/tests/` with integration tests: (1) brain store→recall roundtrip, (2) hook registration completeness check, (3) preflight JSON schema validation, (4) CLI smoke tests (`clarvis brain health`, `clarvis bench pi`, `clarvis queue status`). Target: 5+ tests, all pass in <30s. Currently zero test coverage for spine package. (Source: ARCH_AUDIT_2026-03-05)
