...
- [~] [STALLED] **[AUDIT_PHASE_0_INSTRUMENTATION]** Implement the Phase 0 measurement substrate that blocks every downstream audit phase: (1) per-spawn `audit_trace_id` linking preflight→execution→postflight→outcome, (2) `data/audit/traces/<date>/<id>.json` writer with ≥45d retention, (3) `data/audit/feature_toggles.json` registry supporting `shadow` mode, (4) `scripts/audit/trace_exporter.py` CLI, (5) `scripts/audit/replay.py` for deterministic prompt rebuild. PASS gate: ≥95% of real Claude spawns in a 7-day window have a complete recoverable trace. Canonical plan: `docs/internal/audits/CLARVIS_DEEP_AUDIT_PLAN_2026-04-16.md`. (2026-04-16: substrate shipped — `clarvis/audit/{trace,toggles}.py`, trace_exporter + replay CLIs, spawn_claude + heartbeat wiring, `audit_trace_id` on CostEntry. Awaiting 7-day trace window before PASS ruling. Decision doc: `docs/internal/audits/decisions/2026-04-16_phase0_instrumentation.md`.)
  - [x] [STALLED_2] ~~Implement: core logic change in one focused increment~~ Obsolete 2026-04-19: substrate already shipped (2026-04-16). No remaining core logic change identified — all 5 components delivered, awaiting 7-day trace window only.
  - [x] [STALLED_3] ~~Test: add/update test(s) covering the new behavior~~ Obsolete 2026-04-19: no new behavior to test (STALLED_2 was obsolete).
  - [x] [STALLED_4] ~~Verify: run existing tests, confirm no regressions~~ Done 2026-04-19: 124 passed, 1 pre-existing failure (unrelated import error in test_graph_compaction_sqlite.py).
...
- [x] **[SELF_MODEL_TEST_COVERAGE]** ~~Add test suite for `clarvis/metrics/self_model.py`.~~ Done 2026-04-18: `tests/test_self_model.py` — 27 tests covering model I/O, meta-cognition (think_about_thinking, awareness, working_memory, cognitive_state), CAPABILITY_DOMAINS structure, assess_all_capabilities, remediation generation, weekly regression detection, and update_model. All 27 pass.
...
- [x] **[PHASE10_GATEWAY_SYSTEMD_HARDENING]** ~~Add `PrivateTmp=true`, `NoNewPrivileges=true`, `ProtectHome=read-only`, `ProtectSystem=strict` to `openclaw-gateway.service`.~~ Done 2026-04-18: all 4 directives added + `ReadWritePaths=/home/agent/.openclaw /tmp`. Verified: `systemctl --user show` confirms `PrivateTmp=yes`, `NoNewPrivileges=yes`, `ProtectHome=read-only`, `ProtectSystem=strict`.
...
- [x] **[PHASE14_DREAM_AZR_FREQUENCY_REDUCTION]** ~~Reduce dream_engine from daily (02:45) to weekly (Sun 02:45) and absolute_zero from weekly (Sun 03:00) to monthly (1st Sun 03:00) in system crontab.~~ Done: crontab verified — dream=`45 2 * * 0` (weekly Sun), absolute_zero=`0 3 1-7 * 0` (monthly 1st Sun). Already applied in prior session.
...
- [x] **[SPINE_ORCH_SCOREBOARD_SURFACE_TRIM]** ~~Drop `scoreboard_record`, `scoreboard_show`, `scoreboard_trend` re-exports from `clarvis/orch/__init__.py`.~~ Done 2026-04-18: 3 dead re-exports removed. Direct `from clarvis.orch.scoreboard import ...` still works. Verified: orch imports clean, scoreboard submodule unaffected.
...
- [x] ~~Diagnose near-zero bridge-type edges and fix bridging pipeline.~~ Done 2026-04-19: Diagnosed — literal `bridge` type has 0 edges (never created by code), but bridging is healthy: `semantic_bridge`=7710, `cross_collection`=25122. The root cause was misleading reporting from `total_edges=0` bug (fixed below). Cross-collection ratio=53.9%.
- [x] ~~Fix bulk_cross_link total_edges returning 0 in SQLite mode.~~ Done 2026-04-19: Fixed `graph.py:483` — was reading empty JSON dict instead of `_sqlite_store.edge_count()`. Now returns correct count (81314). Same pattern as existing fix in `bulk_intra_link`.
- [x] ~~Restore cross-collection bridge capacity in cron_reflection.sh.~~ Already done: crosslink step present at line 131 (`python3 -m clarvis brain crosslink --timeout 600`), reordered to run last to avoid pipeline stall.
...
- [x] **[CALIBRATION_CURVE_EMPTY_INPUT_GUARD]** ~~`scripts/metrics/calibration_curve.py` divides by `total_n` in `generate_report()` — add empty-file guard.~~ Done 2026-04-18: early return when `total_n == 0`, verified with empty and non-empty inputs.
