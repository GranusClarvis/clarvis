...
- [ ] [UNVERIFIED] **[BB_PHASE1_TESTNET_DEPLOY]** Run `forge script script/Deploy.s.sol --rpc-url megaeth_testnet --broadcast` against MegaETH testnet (chain 6342) to deploy `BunnyBagzBankroll` + `BunnyBagzCoinflip`, register the game + seed deposit, and write `deployments/6342.json`. Then run `pnpm --filter @bunnybagz/contracts forge verify` (or manual etherscan-verify) for both addresses. Acceptance: `deployments/6342.json` committed; `ADDRESSES[6342]` in `packages/chain/src/index.ts` populated; CI green; `/play/coinflip` no longer shows "not deployed" CTA when `NEXT_PUBLIC_COINFLIP_ADDRESS` is unset. **Operator-gated:** needs a funded testnet deployer key (`PRIVATE_KEY` env). Defer first run until operator provides the key. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_INDEXER_PHASE1_PONDER]** Replace the `apps/indexer/README.md`-only stub with a real Ponder project: `ponder.config.ts` indexing `BetPlaced` + `BetSettled` + `BetRefunded` from the testnet `BunnyBagzCoinflip` address, `schema.graphql` with a `bets` table (betId, player, side, stake, clientSeed, serverCommit, nonce, status, payout, serverReveal, blockPlaced, settledAt), and a `GET /api/history?address=` handler. Acceptance: `pnpm --filter @bunnybagz/indexer dev` runs against testnet RPC + a local Postgres and writes a `bets` row for any test bet. Defer Fly.io + Neon hosting to operator. (PROJECT:BUNNYBAGZ)
- [x] **[BB_VERIFY_API_INDEXER_INTEGRATION]** Replaced `pending-indexer` stub with `resolveBet()` test seam in `apps/api/verify/[betId].ts`: indexer-first via `BUNNYBAGZ_INDEXER_URL` (`/api/bet/:betId`) → viem `publicClient.getLogs` fallback (`makeChainResolver`, BetSettled+BetPlaced filtered by betId, recomposes `(serverSeed, clientSeed, nonce)` and `declaredOutcome`) → pending. Web 8/8 vitest cases stay green; api tests grew 4→8 (added mocked-indexer verified, indexer 404, indexer-pending → chain fallback, no-source pending). Pending operator step: set `BUNNYBAGZ_INDEXER_URL` (or `BUNNYBAGZ_RPC_URL`+`BUNNYBAGZ_COINFLIP_ADDRESS`) in Vercel env once the testnet deploy lands to flip the live `verified-match` end-to-end check. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_KEEPER_BOT_SETTLE]** Wire the on-chain `settleBet(betId, serverReveal)` call in `apps/api/seed/reveal.ts`. Today the handler returns the seed but doesn't broadcast. Use viem's `walletClient` with `SETTLEMENT_PRIVATE_KEY` (already in `.env.example`) to sign + send. Add a watchdog test that asserts: seed lookup succeeds → tx broadcast → `BetSettled` log observed. **Operator-gated:** needs a funded keeper key. Local anvil dry-run is sufficient for the tests. (PROJECT:BUNNYBAGZ)

#### Pre-operator-test polish (P1, blocks "looks smooth" gate)

- [ ] [UNVERIFIED] **[BB_TAILWIND_TOKENS_INSTALL]** Install Tailwind v4 + the token table from `docs/UX_PLAN.md §Visual system` (carrot-gold, deep, surface, elevated, etc.) under `apps/web/tailwind.config.ts`. Replace the inline `style={{...}}` blocks across `app/page.tsx`, `app/layout.tsx`, `app/play/coinflip/page.tsx`, `app/verify/[betId]/page.tsx`, `components/SiteHeader.tsx`, `components/Providers.tsx` with token-based class names. Acceptance: no inline `style={{...}}` for color/spacing tokens; `pnpm --filter @bunnybagz/web build` succeeds; vitest CTA-state suite still passes. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_LIGHT_THEME_PARITY]** Both themes "from day one" is a `docs/DECISIONS.md` and `BRAND.md` lock. Add a `prefers-color-scheme` listener + a `data-theme` toggle in `/wallet`. The current `<html data-theme="dark">` is hardcoded. Acceptance: Coinflip + Verify pages render legibly in both themes; tokens from `[BB_TAILWIND_TOKENS_INSTALL]` cover the swap; AA contrast verified with axe-core in vitest. Depends on `[BB_TAILWIND_TOKENS_INSTALL]`. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_FRAMER_MOTION_COIN_SPIN]** Install `framer-motion` and add the 1.2s coin spin per `docs/UX_PLAN.md §5 Animation contract`. Reduced-motion fallback (crossfade, no spin) when `prefers-reduced-motion: reduce`. Acceptance: coin spin runs on a successful settle in a test fixture; reduced-motion test asserts the crossfade variant; spin duration locked to 1200ms. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_PLAY_LOBBY_ROUTE]** Add `/play/page.tsx` lobby with a single Coinflip card today (Dice + HiLo cards land in Phase 2). Today the landing page (`/`) dead-ends — the only way to reach the game is to type `/play/coinflip` directly. Acceptance: `/` "Play" CTA links to `/play`; `/play` shows the Coinflip card with current bankroll TVL pulled from the indexer (when wired) or an empty-state when not; vitest covers the card render. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_WALLET_SHEET_MVP]** Minimal `/wallet` modal/slide-over per `docs/UX_PLAN.md §1 Surfaces`. Shows: connected address, ETH balance, Fluffle-holder badge placeholder (no economic perk), light/dark theme toggle. No deposit/withdraw flow yet (Phase 2). Acceptance: header `⋯` opens the sheet on mobile; theme toggle persists to `localStorage`; vitest covers open/close. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_MASCOT_PLACEHOLDER_ART]** Drop the BunnyBagz mascot per `docs/BRAND.md` — at minimum SVG sprites for `idle`, `win`, `loss-streak`. Use as decorative on landing + a 2-frame celebrate on coinflip win. Operator-grade art is post-MVP; Phase 1 just needs *something* better than the empty header. Acceptance: SVGs in `apps/web/public/mascot/`; mascot renders on `/`, `/play/coinflip` win state; reduced-motion shows a static frame. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_A11Y_KEYBOARD_SCREEN_READER]** Per `docs/UX_PLAN.md §7 Accessibility (non-negotiable, even in MVP)`: Enter = primary CTA on each game screen; screen-reader announcement on every game-state change ("You won 0.002 ETH on heads."); 44px touch targets are already met (CTAs are 64px). Acceptance: keyboard-only walk-through of `/play/coinflip` selects side + stake + places bet; `aria-live="polite"` region announces the outcome; vitest with `@testing-library` keyboard helpers covers the path. (PROJECT:BUNNYBAGZ)
...
- [ ] [UNVERIFIED] **[BB_WAGMI_CLI_ABI_GEN]** Replace the hand-rolled `COINFLIP_ABI` in `apps/web/src/lib/abis.ts` with a generated ABI from `forge inspect` (or `wagmi/cli`) wired into `@bunnybagz/chain`. Today a contract change requires a manual sync. Acceptance: a build step exports `apps/web/src/lib/abis.ts` from compiled artifacts; CI fails when `abis.ts` is out of date. (PROJECT:BUNNYBAGZ)
- [ ] [UNVERIFIED] **[BB_ADDRESS_BOOK_CODEGEN]** Auto-populate `ADDRESSES[chainId]` in `packages/chain/src/index.ts` from `packages/contracts/deployments/<chainId>.json` (output of `[BB_PHASE1_TESTNET_DEPLOY]`). Acceptance: post-deploy, `ADDRESSES[6342].coinflip` is non-empty without a manual edit; a CI check asserts both stay in sync. (PROJECT:BUNNYBAGZ)
