...
- [x] **[HEARTBEAT_NOTASK_ROOT_CAUSE_REPORT]** Done 2026-05-01. Report at `docs/internal/audits/HEARTBEAT_NO_TASK_RATE_2026-04-30.md`. Bucketed last 14 wake cycles (the analyzer's full window since 04-12 rotation): 12 executed_ok / 1 queue_empty / 1 unknown (in-flight) — 14.3% no-task vs 40% on the original 04-30 30-cycle sample. Anchored on the 04-30 30-cycle dataset where 12/12 of no-task cycles were `all_filtered_by_v2`. Top-3 remediations cite `clarvis/orch/queue_engine.py::ranked_eligible()` (eligibility-fallback knob), `clarvis/orch/queue_writer.py` (low-water-mark replenish), and `clarvis/heartbeat/health.py:90` + `scripts/cron/cron_autonomous.sh` (in_flight vs unknown disambiguation). Reconciles against source log within ±1.
