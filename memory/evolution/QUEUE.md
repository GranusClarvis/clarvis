...
- [x] **[EDIT_OLDSTRING_FAILURE_TELEMETRY]** Done 2026-05-01. New module `scripts/cognition/edit_failure_telemetry.py` scans for two canonical Edit-tool markers (`old_string_not_found`, `old_string_not_unique`) and appends `{ts, task_id, marker, file_path_seen}` rows to `data/action_failure_modes.jsonl`. Wired into `heartbeat_postflight.py` §9.55 next to transcript_log. Threshold >5 in rolling 7d emits a P2 `[EDIT_TOOL_PROMPT_TEMPLATE_RETUNE]` task via `queue.writer.add_task` with a 7-day cooldown stored in `data/edit_failure_telemetry_state.json`. Sandbox test exercised all paths (scan, append, threshold trip, queue inject, cooldown block, stats CLI).
