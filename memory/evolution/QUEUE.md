...
- [ ] **[BB_QA_MOBILE_HORIZONTAL_OVERFLOW_SWEEP]** Eliminate the mobile horizontal overflow flagged in ~30 variants. **Acceptance:** (a) identify the fixed-width offender(s) on `/`, `/play`, `/play/coinflip`, `/play/dice`, `/play/hilo`, `/wallet`, `/audit`, `/bounty`, `/verify` at 375px light+dark; (likely trust strip / primary CTA cluster / any new fixed min-width row) and replace hard widths/min-widths with clamp/flex-wrap/overflow-safe sizing; (b) add a deterministic audit helper that fails when `document.documentElement.scrollWidth > window.innerWidth + 1` inside the main `bb-game-viewport` / page shell after fonts settle; (c) Playwright QA harness report records the first offending selector + route instead of only the screenshot; (d) at least 3 targeted vitest/RTL coverage cases for the specific component(s) that caused the overflow; (e) rerun the visual QA matrix and land with **0 horizontal-overflow findings**. (PROJECT:BUNNYBAGZ) (2026-05-10)

- [ ] **[BB_QA_MOBILE_TOUCH_TARGET_FLOOR]** Fix undersized interactive targets (<44×44) across the mobile matrix. **Acceptance:** (a) enumerate every failing element from the QA report and normalize all tappable controls to a 44px min hit area on 375px width without breaking desktop density; (b) the smallest current offender (`BunnyBagz home` at 8×8) is either enlarged directly or wrapped in an invisible hit target with preserved semantics/focus ring; (c) add/update a shared audit helper so icon-only buttons, header links, wallet actions, segmented controls, toasts, and dismiss affordances are all measured by rendered bounding box, not nominal icon size; (d) QA harness emits selector + measured dimensions for each violation; (e) rerun matrix and land with **0 touch-target findings**. (PROJECT:BUNNYBAGZ) (2026-05-10)

- [ ] **[BB_QA_DICE_SETTLE_ANIMATION_RESTORE]** Restore the dice settle animation path that never mounted during QA (`dice-result-needle` missing in all six dice variants). **Acceptance:** (a) root-cause whether the mount predicate, settle-event timing, reduced-motion branch, or testid placement regressed; (b) ensure the dice result needle/settle animation mounts on real settle in both themes and all audited viewports; (c) add component-level tests covering won + lost settle branches and reduced-motion behaviour; (d) add one Playwright/e2e assertion that waits for the needle animation hook/testid after a mocked settle rather than only checking final text; (e) rerun QA with **0 `dice-needle-target-missing` findings**. (PROJECT:BUNNYBAGZ) (2026-05-10)

- [ ] **[BB_QA_WALLET_LIGHT_THEME_HYDRATION_FIX]** Fix the `/wallet` light-theme hydration mismatch surfaced in all light variants. **Acceptance:** (a) reproduce with the QA env on the existing harness/dev server and capture the exact React/Next hydration warning payload; (b) identify the SSR/client branch divergence (likely theme-dependent markup, random/id generation, media-query branch, or mount-only wallet/history surface) affecting `wallet-page` and `wallet-profile-history`; (c) make the wallet shell render byte-stable HTML across SSR and hydration in light mode while preserving the intended client-only enhancements; (d) add a regression test that fails on hydration warnings for `/wallet` light theme; (e) rerun QA and land with **0 hydration mismatch findings**. (PROJECT:BUNNYBAGZ) (2026-05-10)

- [ ] **[BB_QA_TRUST_STRIP_LEGIBILITY_PASS]** Raise the mobile trust-strip / “House liquidity” copy above the 12px legibility floor without reopening overflow. **Acceptance:** (a) locate every text style in the trust-strip cluster that falls below 12px on 375px width; (b) retune the typography/spacing hierarchy so the strip remains single-purpose and readable in both themes; (c) verify the fix does not reintroduce horizontal scroll or reduce nearby touch targets; (d) add audit coverage for minimum computed font-size on that surface; (e) rerun QA with **0 legibility advisories** for the trust strip. (PROJECT:BUNNYBAGZ) (2026-05-10)

- [ ] **[BB_QA_HARNESS_DIAGNOSTICS_HARDENING]** Improve the QA harness itself so future failures are more actionable and this defect class is less likely to slip through as vague screenshot-only output. **Acceptance:** (a) extend `apps/web/scripts/qa-harness` / report JSON so each finding includes route, viewport, theme, selector/testid (when resolvable), measured box metrics, and a short machine-readable `causeCode`; (b) add dedicated audits for overflow (`scrollWidth/clientWidth delta`), touch targets (`boundingBox < 44`), hydration warnings (capture stderr/browser console and route-scope them), and missing animation mount points (`expected testid absent within timeout`); (c) produce a per-run summary that groups findings by root cause instead of only by scenario; (d) document the QA-ready dev-server contract in the runbook, including mock-wallet env vars, pm2/manual-server distinction, and how to rerun from the preserved `latest` symlink; (e) add fixture-backed tests for the report normalizer so harness regressions are caught without running a full browser matrix. (PROJECT:BUNNYBAGZ) (2026-05-10)
...
- [x] **[P3_HEALTH_PROBE_CLEANUP]** Shipped 2026-05-10. (a) Wrote `scripts/brain_mem/cleanup_health_probes.py` (dry-run default, `--apply` to delete, per-batch logging). Dry-run reported **342 candidates (58.9% of clarvis-memories=581)** — within 300–400 acceptance band. `--apply` deleted all 342 in 6.7s, failed=0; clarvis-memories count dropped 581→239 (delta=342 ✓). Post-cleanup re-scan: 0 remaining health-probe records. (b) Replaced silent `except Exception: pass` at `clarvis/brain/store.py:1023-1026` with `_store_log.warning("health probe cleanup failed: %s", e)` so future leaks page someone. Retrieval benchmark P@3=0.6833 pre and post (no regression, met "≥ baseline" criterion); MRR=0.8417, Recall=0.90, P@1=0.80 unchanged. No P@3 lift — the 342 probes were apparently not surfacing in benchmark queries, but their removal cuts collection size by 59% and unblocks future audits. (PROJECT:CLARVIS) (2026-05-10)
