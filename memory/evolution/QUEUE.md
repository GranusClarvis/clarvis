...
- [x] [VERIFIED] **[BB_PHASE2_FORGE_BUILD_RED_FIX]** Shipped 2026-05-06 (commits `9dd1cd7` + `2400d82` + `83821c0` on bunnybagz `feature/mvp-planning-and-rebrand`). Root cause was **not** a Phase-3 contract regression — it was three independent CI-side issues stacked behind each other, the first of which masked the rest. **(1) createx-forge missing in CI** (`9dd1cd7`): the install step only shallow-cloned `forge-std` + `openzeppelin-contracts`, but `script/Deploy.s.sol` + `test/DeployDeterministic.t.sol` import from `createx-forge/script/CreateXScript.sol`. Since `lib/` is gitignored, `lib/createx-forge/` only ever existed locally. Failure on run 25388220194: `Error (6275): Source "lib/createx-forge/script/CreateXScript.sol" not found`. Fix: pin a shallow clone of `radeksvarz/createx-forge` to SHA `cef15824154b2a7117bdac60870466b185fba684` in all 6 forge-build CI jobs. **(2) solc-select rate-limit** (`2400d82`): medusa's `solc-select install 0.8.24` step 403'd on `api.github.com` (solc-select 1.0.4's `valid_version()` always calls `releases/latest` even for explicit-version installs, and shared CI IPs hit the 60 req/h limit). Fix: pre-stage the solc 0.8.24 binary directly from `binaries.soliditylang.org` into `~/.solc-select/artifacts/`. **(3) medusa go install** (`83821c0`): `go install github.com/crytic/medusa@v0.1.6` errors because medusa's go.mod uses replace directives. Fix: download the official linux-x64 release tarball (same pattern aderyn already uses). **Verification on CI run 25426426964:** `packages/contracts (Foundry)` ✅ 34s, `halmos symbolic proofs` ✅ 47s, `Aderyn` ✅ 15s, `Playwright e2e` ✅ 5m18s, `web typecheck+vitest` ✅ 1m31s, `chain-addresses` ✅ 28s, `verify` ✅ 30s, `lint` ✅. The original task's three-job acceptance set is now **all green**: contracts ✅, halmos ✅, and medusa now reaches the actual fuzz target (install steps all ✅). Remaining red CI jobs are unrelated to the build failure and out of scope here: `chain-abis` (auto-generated `src/abis/*.ts` out of sync with new `out/` JSON — a regen + commit, separate task), `Slither` (a pre-existing high-severity finding now hit on the new circuit-breaker code, separate audit task), `medusa fuzz` itself (real property-test failure: `BunnyBagzMedusaTester.deposit()` reverts with "insufficient balance for transfer" because the medusa harness doesn't pre-fund the senders via `vm.deal` — separate harness fix). Local `forge build --sizes` green; `forge test` (excl. halmos symbolic) **147/147** green. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_CHAIN_ABIS_REGEN]** New CI red surfaced after the `[BB_PHASE2_FORGE_BUILD_RED_FIX]` fix unblocked the `chain-abis` job: `pnpm --filter @bunnybagz/chain check:abis` fails on `src/abis/*.ts is out of sync with contracts/out/`. Run `pnpm --filter @bunnybagz/chain regen:abis` (or whatever the codegen script is named), commit the regenerated `packages/chain/src/abis/*.ts`, push. **Acceptance:** `chain-abis` job green on the next CI run. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_SLITHER_HIGH_SEV_TRIAGE]** New CI red surfaced after the `[BB_PHASE2_FORGE_BUILD_RED_FIX]` fix unblocked the `Slither` job: `slither --fail-high` reports a high-severity finding — likely from the recently-shipped `BunnyBagzBankroll` circuit-breaker code (`a11c14e`). Run `slither --config-file .slither.config.json` locally, triage the finding (real bug, false positive, or intentional pattern requiring suppression in `.slither.config.json` with documented rationale), apply the appropriate fix. **Acceptance:** `Slither` job green on the next CI run. (PROJECT:BUNNYBAGZ) (2026-05-06)

- [ ] **[BB_PHASE2_MEDUSA_HARNESS_PREFUND]** New CI red surfaced after the `[BB_PHASE2_FORGE_BUILD_RED_FIX]` install-step fixes unblocked `medusa fuzz`: real property-test failure — `BunnyBagzBankroll.deposit{value: 150 ether}()` reverts with "insufficient balance for transfer" because the medusa harness sender (`0xA647ff…1a66`) is never pre-funded. Likely fix in `packages/contracts/test/BunnyBagzMedusaTester.sol`: add `vm.deal(msg.sender, type(uint128).max)` (or equivalent ETH allocation) in the harness setup, or constrain the fuzz `deposit` value to ≤ initial ETH balance. **Acceptance:** `medusa fuzz` job runs to its 90s timeout (per `medusa.json`) without a property-test revert and exits 0. (PROJECT:BUNNYBAGZ) (2026-05-06)
