...
- [x] **[BB_PHASE3_GEO_ENFORCE_MODE]** Geo enforce-mode wiring landed (2026-05-06). `packages/chain/src/geo.ts` is the single source of truth (named-country blocklist + `isBlockedCountry`), `apps/api/geo/index.ts` reads `BUNNYBAGZ_GEO_MODE=enforce` and returns 403 `{regulatory_block:true,country,reason,mode}` (legacy `GEO_ENFORCE=1` still honoured for back-compat), and `geoGuard()` short-circuits `/play/*` server fns (`apps/api/hilo/step.ts`, `apps/api/seed/{claim,commit}.ts`) + the wallet edge fn (`apps/api/history/wallet.ts`). Web side: `apps/web/src/middleware.ts` rewrites blocked traffic for `/play/*` and `/wallet` to a new `/region-not-supported` page rendering `GeoBlockedNotice` (no game UI). 11 node:test cases pass (decide log/enforce, env unset, log explicit, enforce blocked/allowed, missing header, legacy GEO_ENFORCE, geoGuard log/enforce). **Operator/counsel gate:** flip to enforce in production stays blocked on counsel-signoff of the blocklist — env defaults to `log`, so ship-and-leave is safe.
