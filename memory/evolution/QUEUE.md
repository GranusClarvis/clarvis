...
### BunnyBagz — MegaETH Casino (rewritten 2026-04-29)

_Repo: `bunnybagz/mega-house` (rename to `bunnybagz` pending). Active branch: `feature/mvp-planning-and-rebrand`._
_**Workflow:** BunnyBagz is managed and tested **directly in its own repo**. Do NOT route through the SWO PR workflow (no `gh pr create --repo InverseAltruism/...`). Standard flow: clone/pull → branch → edit → `pnpm test` + `forge test` locally → push → open a PR against BunnyBagz's own branch **only when the operator explicitly asks**. Default: commit + push directly to the working branch._
_**Done so far:** Phase 0 scaffold + CI ✅; Phase 1 contracts (`BunnyBagzRandomness/Bankroll/Coinflip.sol`), Foundry tests, edge fns (`apps/api/{seed,verify,geo}`), web pages (`/play/coinflip`, `/verify/[betId]`), and JS↔Solidity parity vectors ✅ (commit `990d554`). `pnpm --filter @bunnybagz/verify test`: 19/19 green._
_**Operator-blocking — do NOT attempt autonomously:** domain confirmation, X/TG/Discord handle squat, repo rename `mega-house → bunnybagz`, `forge install` of OZ v5 + forge-std (network-bound)._

- [ ] **[BB_COINFLIP_WALLET_INTEGRATION]** Replace the placeholder ready/waiting states on `apps/web/src/app/play/coinflip/page.tsx` with real wallet integration: add RainbowKit/wagmi providers (chain config for MegaETH testnet), a connect button, network-switch UX, and the actual `placeBet` write call wired to `BunnyBagzCoinflip` (address from `NEXT_PUBLIC_COINFLIP_ADDRESS`). Acceptance: `pnpm --filter @bunnybagz/web typecheck && build` green; wallet connect renders; with a local anvil deploy and env address set, `placeBet` issues a tx. Tested directly in the BunnyBagz repo. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_FOUNDRY_DEPLOY_SCRIPT]** Add `packages/contracts/script/Deploy.s.sol` that deploys `BunnyBagzBankroll`, then `BunnyBagzCoinflip`, then calls `Bankroll.setAllowance(coinflip, …)` to wire them; writes `{bankroll, coinflip, chainId}` to `packages/contracts/deployments/<chainId>.json`. Acceptance: `forge script script/Deploy.s.sol --rpc-url <local-anvil> --broadcast` prints both addresses and writes the JSON; deploy is idempotent (re-running on a fresh anvil works). (Run once operator unblocks `forge install`.) (PROJECT:BUNNYBAGZ)

- [ ] **[BB_VERIFY_PAGE_ERROR_STATES]** Tighten `apps/web/src/app/verify/[betId]/page.tsx` error/edge handling: missing/invalid `betId` (route param), API 404 from `/api/verify/[betId]`, parity-mismatch state (recomputed commit ≠ on-chain commit), stale-revealed-but-unsettled state, and network-error fallback. Each branch must have a visible UI state. Add unit tests (Vitest + React Testing Library) under `apps/web/src/app/verify/[betId]/__tests__/page.test.tsx` covering all five branches. Acceptance: tests green via `pnpm --filter @bunnybagz/web test`. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_GEO_ENFORCE_FLAG]** `apps/api/geo/index.ts` currently ships in log-only mode. Add a `GEO_ENFORCE=1` env-flag enforce path that returns HTTP 403 `{country, blocked: true}` for blocked regions, leaves log-mode behavior unchanged when the flag is unset, and unit-tests both modes. Acceptance: `pnpm --filter @bunnybagz/api test` green; toggling the env flips the response without code change. (PROJECT:BUNNYBAGZ)
