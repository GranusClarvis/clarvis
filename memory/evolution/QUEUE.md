...
- [ ] **[BB_PHASE3_INDEXER_LOCAL_RUNNABLE]** Make `pnpm --filter @bunnybagz/indexer dev` actually runnable locally instead of echoing `'ponder dev — install ponder before running (pnpm add ponder)'`. Operator audit point: "local indexer not runnable." **Acceptance:** (a) install `ponder` in `apps/indexer/package.json` (workspace lockfile updated); (b) `pnpm --filter @bunnybagz/indexer dev` boots Ponder against a local `anvil` (or a sentinel local-dev mode that reads from a fixture chain) without needing a real RPC; (c) `apps/indexer/README.md` gets a **Local dev loop** section with the exact command sequence (`anvil --chain-id 6343 --port 8545` → `BUNNYBAGZ_RPC_URL=http://127.0.0.1:8545 pnpm --filter @bunnybagz/indexer dev`) including how to point `apps/web` at the local indexer (`INDEXER_BASE_URL=http://127.0.0.1:42069`); (d) `pnpm --filter @bunnybagz/indexer dev:once` already works; this item adds the *streaming* equivalent; (e) ≥1 smoke test (`__tests__/dev-mode.test.ts` or expanded `dev-once`) that asserts the streaming runtime starts and shuts down cleanly. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_WALLET_PNL_AGG]** Add a PnL aggregation card to `/wallet`. Today the wallet surface ships `<RecentBetsList>` (history rows) but no rollup. Operator audit point: "wallet page missing PnL/history" — history is present but PnL is missing. **Acceptance:** (a) new `apps/web/src/components/WalletPnLCard.tsx` rendered by `<WalletPanel>` between the balance hero and `<RecentBetsList>`; (b) reads from `GET /api/history/wallet?address=…` (existing endpoint), aggregates `sum(stakeWei) - sum(payoutWei)`, win-rate (`won / settled`), per-game breakdown chips (coinflip / dice / hilo); (c) tabular figures, `bb-tabular` class, sign-aware formatting (green for net-positive, neutral for break-even, danger for net-negative); (d) refreshes on the same `bunnybagz:bet-settled` CustomEvent the recent-bets list listens for — ride the bridge shipped in `[BB_PHASE3_PRODUCTION_SETTLE_EVENT_BRIDGE]`; (e) ≥6 vitest cases (empty / single-bet / multi-bet / mixed-token / loss-only / win-only / event-driven refresh); (f) USDm-token PnL is split out as a separate row (don't pretend mixed currencies are commensurable). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_DEV_COMPILE_SPEEDUP]** Address cold dev-server compile latency. Operator audit point: "cold dev compile slowness." Measured 2026-05-08 evening: first `/play` request after `pnpm dev:lan` boot takes ~10.8s; subsequent requests <100ms (so it's strictly cold-compile). **Acceptance:** (a) profile with `next dev --turbopack` enabled (already on Next 15.5) and confirm whether the regression is pre-Turbopack default or a heavy import chain on `/play`; (b) audit the import graph for `apps/web/src/app/play/page.tsx` — `<RainbowKitProvider>`, `<WagmiProvider>`, `@bunnybagz/chain` ABIs (large JSON), `@bunnybagz/ui` primitives; (c) one of: lazy-load `<Providers>` children, code-split RainbowKit with a dynamic import boundary, or split the chain ABIs out of the eagerly-loaded surface; (d) target: cold first-render of `/play` <4s on the same hardware that today shows ~10s; (e) measurement: a documented `scripts/perf-cold-compile.mjs` that boots `dev:lan`, curls `/play` once, prints the compile time, and is checked into the repo; (f) ≥1 vitest case for the perf harness's parsing logic. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_WALLETCONNECT_NOISE_CLEANUP]** Quiet the WalletConnect/RainbowKit dev console noise. Operator audit point: "WalletConnect/metadataBase noise" — metadataBase side closed by `[BB_PHASE3_METADATABASE_NOISE_FIX]`; this item handles the WalletConnect half. **Acceptance:** (a) audit `apps/web/src/lib/wagmi.ts` for the `bunnybagz-dev-placeholder` WalletConnect project-id sentinel — when this sentinel is in use, suppress the WC "no project id" warning at the boundary (one warning, one time, with a pointer to `cloud.walletconnect.com`); (b) on real configs (`NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` set), pass the id through unchanged; (c) document the dev-mode behaviour in `apps/web/README.md` Dev section; (d) ≥2 vitest cases (sentinel suppresses warning; real id surfaces no warning). Do not silence *other* warnings — only the noisy WC dev-mode init log. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE3_LAN_DEV_PROD_RUNBOOK]** Document the operator-facing local LAN serve workflow. Operator audit point: "dev/prod local serving guidance." Today the `dev:lan` script exists but there is no doc telling the operator which URL to hit, where logs go, how to run a `next start` in prod-mode for hot reload-free testing, and which env vars to set. **Acceptance:** (a) `apps/web/README.md` (or `docs/LOCAL_LAN.md`) gets a **Local LAN runbook** section: `pnpm --filter @bunnybagz/web dev:lan` → "open `http://<your-LAN-ip>:3000/play` from any device on the same network" + `hostname -I` snippet; (b) parallel **Prod-mode local serve** section: `pnpm --filter @bunnybagz/web build && PORT=3000 pnpm --filter @bunnybagz/web start --hostname 0.0.0.0` with the same LAN URL guidance; (c) **Required env vars** subsection: `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`, the `NEXT_PUBLIC_*_ADDRESS` overrides for pre-deploy chains; (d) **Indexer** subsection that links to `[BB_PHASE3_INDEXER_LOCAL_RUNNABLE]`; (e) link from the top-level `README.md` "Quick start" to the new section. No code change required beyond docs + maybe a tiny `pnpm start:lan` convenience script. (PROJECT:BUNNYBAGZ)
