...
- [x] **[DASHBOARD_FRESHNESS_AUDIT_2026-05-04]** Shipped 2026-05-04. `docs/internal/audits/DASHBOARD_FRESHNESS_AUDIT_2026-05-04.md` enumerates 28 dashboards (6 fresh / 4 recent / 6 stale / 13 dead — 5 of the dead are by-design archival research). Cross-cut finding: 3 of 8 actionable dead dashboards (`clr_benchmark_latest`, `brain_eval/latest`, `calibration_curve_report`) share a "silent-cron" failure mode where the cron logs "completed" but the output file's mtime never advances; recommended a generic `assert_mtime_advanced` helper for `lock_helper.sh`. Each dead entry has a concrete recommendation. (PROJECT:CLARVIS)
