...
- [ ] **[COGNITION_FEATURE_TRANSLATION_REVIEW]** Review neuroscience/paper-inspired features for translation quality: keep / revise / demote / archive classification.
- [ ] **[CONSCIOUSNESS_EMULATION_BLOAT_GUARD]** Anti-bloat review pass for consciousness-/neuroscience-inspired features. Demote if not showing value in task success, retrieval, or prompt quality.
- [ ] **[EXECUTION_FIRST_COGNITION_POLICY]** Define policy: cognitive features justified by better execution, not by resemblance to consciousness on paper. Triage active cognition tasks.
- [ ] **[BRAIN_WIKI_CONTEXT_ROUTE_BENCH]** Benchmark four retrieval routes on representative tasks: raw brain, wiki-first, combined, minimal direct. Compare usefulness and task outcome quality.
- [ ] **[LOW_VALUE_FEATURE_DEMOTION_SWEEP]** Sweep active queue/cognition work for features adding complexity without clear output quality gains.
...
- [ ] **[SPINE_COGNITION_DEAD_SURFACE_PHASE9_INTAKE]** No code change. Feed the 20 DEAD cognition exports (across `cognitive_load`, `conceptual_framework`, `intrinsic_assessment`, `attention` module surfaces) into the Phase 9 EVS/TCS scorecard as inputs. Subtle-feature guard (§0.3.5) applies — any move past SHADOW requires operator signoff because these touch attention / phi-adjacent subsystems. Acceptance: the Phase 9 working doc lists these 20 exports with TCS estimates before the first A/B cycle.
