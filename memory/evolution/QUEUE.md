...
- [x] **[BB_PHASE2_DEPLOY_DICE_HILO_REGISTER]** SHIPPED 2026-05-04. Deploy.s.sol extended with Dice + HiLo (defaults 0xBBD1, 0xBBA1 — note `0xBBH1` was not valid hex so HiLo entropy is encoded as 0xBBA1, env-overridable as `BB_HILO_SALT`). DeployDeterministic test now asserts all 4 addresses match across anvil/testnet/mainnet chainids. Live testnet broadcast 2026-05-04 to chain 6343 from deployer 0xb29e...fD7B: bankroll=0x33C5...2dFf, coinflip=0x064b...1983, dice=0xAC02...d84B, hilo=0xd9B9...0d6e (all 4 verified via eth_getCode). `deployments/6343.json` has all 4 addresses + 4 salts. `addresses.generated.ts[6343]` resolves dice + hilo. `chain check:addresses` + `check:abis` both green. Full forge suite 120/120 still passing. Deploy script also made idempotent (`_create3IfMissing` + `_registerIfMissing`) so Phase 3 re-runs on testnet won't blow up on existing addresses or clobber operator-tuned allowances. ORIGINAL: Phase 2 BLOCKER #1. `packages/contracts/script/Deploy.s.sol` only deploys `BunnyBagzBankroll` + `BunnyBagzCoinflip`; `deployments/6343.json` has no `dice` / `hilo` keys; `addresses.generated.ts[6343]` resolves both as `undefined`. Extend the deploy script: import `BunnyBagzDice` + `BunnyBagzHiLo`, derive two new CREATE3 salts via the existing `_packSalt(deployer, entropy)` helper (defaults `0xBBD1` for dice, `0xBBH1` for hilo, env-overridable as `BB_DICE_SALT` / `BB_HILO_SALT`), `create3(...)` both with constructor args matching the existing tests (`(deployer, bankroll, minBet, maxBet)` for Dice; `(deployer, bankroll, minBet, maxBet, maxPayout)` for HiLo — pull `BB_HILO_MAX_PAYOUT` env with default `1 ether`), and call `bankroll.registerGame(diceAddr, allowance)` + `bankroll.registerGame(hiloAddr, allowance)` after the existing coinflip register. Update `_writeDeploymentJson` to include `dice` + `hilo` + their salts. Run the deploy against `https://carrot.megaeth.com/rpc` once the deployer wallet is topped up; commit the updated `deployments/6343.json` + regenerated `addresses.generated.ts`. **Acceptance:** new `DeployDeterministic` test asserts both addresses match across chainid 31337/6343; live testnet broadcast succeeds; `deployments/6343.json` has all 4 addresses; `addresses.generated.ts[6343]` resolves `dice` + `hilo`; `chain check:addresses` + `check:abis` both green; full forge suite still 120/120. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_LOBBY_UNLOCK_DICE_HILO]** Phase 2 BLOCKER #2. `apps/web/src/app/play/page.tsx:36-58` still tags both Dice + HiLo as `status: "phase2"` (locked, non-tappable, opacity 0.55, `cursor: not-allowed`) — even though the real surfaces shipped at `/play/dice` + `/play/hilo`. Lobby is misleading the user. Replace the static `GAMES` array with a function that resolves each card's status from `ADDRESSES[DEFAULT_CHAIN_ID]`: when the `dice` / `hilo` address is present, the card is `live` (tappable Link); when absent, it stays `phase2`. Order: per UX_PLAN §3 thumb-zone law, the live cards must occupy the bottom of the grid. Once `[BB_PHASE2_DEPLOY_DICE_HILO_REGISTER]` lands, all 3 cards become `live` automatically. **Acceptance:** new vitest case `lobby renders dice + hilo as live when their addresses resolve`; existing `lobby renders dice + hilo as phase-2 when their addresses are absent` test added (covers the no-deploy fallback); thumb-zone audit re-run with all 3 live and the live grid still passes the `MIN_THUMB_ZONE_RATIO` gate; `data-testid="game-card-dice-link"` and `-hilo-link` exist when addresses resolve. (PROJECT:BUNNYBAGZ)
...
- [ ] **[BB_PHASE2_VERIFY_DICE_HILO_BRANCHES]** Phase 2 trust-surface gap. `apps/api/verify/[betId].ts:167` hard-codes `game: "coinflip"` even though the response type union admits `"dice" | "hilo"`. `apps/web/src/app/verify/[betId]/page.tsx` only imports `coinflipOutcome`. UX_PLAN §1 calls `/verify/[betId]` the public trust surface for *every* game; today it lies about non-coinflip results. Fix: **(a)** verify API resolves the bet's contract address against the indexer / chain to determine the game (or threads `game` through from a per-game indexer table), branches the recompute, and returns `game` set correctly; **(b)** verify page imports `diceRoll` + `hiloCard` from `@bunnybagz/verify`, switches on `data.game`, computes the appropriate outcome, and matches against the declared one; **(c)** new `/verify/session/[sessionId]/page.tsx` route for HiLo (sessions, not bets — sessionId comes from `BunnyBagzHiLo.SessionOpened`); it walks the session's step log and shows each card draw with its (revealed) seed for replay. **Acceptance:** new vitest specs in `apps/web/src/app/verify/[betId]/__tests__` cover dice + hilo recomputation paths; new spec for `/verify/session/[sessionId]` covers a 3-step HiLo session; api `apps/api/verify/[betId].test.ts` covers all 3 game branches; verify suite still 40/40 + new specs. (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_INDEXER_DICE_HILO]** Phase 2 ROADMAP exit verbatim: "Indexer (Ponder) wired for `BetPlaced` / `BetSettled` across all 3 games". Today: 1 of 3. `apps/indexer/ponder.config.ts` filters `BunnyBagzCoinflip` only. Add `BunnyBagzDice` + `BunnyBagzHiLo` contract entries with their respective ABIs (already in `apps/indexer/abis/` or generate from `out/`). Extend `apps/indexer/schema.graphql` with `DiceBet` + `HiLoSession` + `HiLoStep` tables (mirror the existing `CoinflipBet` shape; HiLo additionally tracks step events). Implement handlers under `apps/indexer/src/` (or wherever `BetSettled` is currently handled) for the new events: `BetPlaced` / `BetSettled` on Dice, `SessionOpened` / `StepPlayed` / `SessionCashedOut` / `SessionRefunded` / `SessionPushed` on HiLo. **Acceptance:** updated config compiles; new handlers covered by tests in `apps/indexer/__tests__/`; indexer suite grows from 21 to ≥30; recent-bets aggregation across all 3 games queryable; documented in `apps/indexer/README.md`. (PROJECT:BUNNYBAGZ)
...
- [ ] **[BB_PHASE2_THUMB_ZONE_DICE_HILO]** Operator's recurring rule: "must adapt well on mobile". `apps/web/src/__tests__/thumb-zone-audit.test.tsx` covers `Home` + `CoinflipPage` + `PlayLobbyPage` only — `/play/dice` + `/play/hilo` are not in the audit, so the new Phase 2 surfaces have zero thumb-zone enforcement. Extend `THUMB_ZONE_TARGETS` in `apps/web/src/lib/thumb-zone-audit.ts` with both routes pointing at their primary CTAs (`hilo-primary-cta` and the dice equivalent). Add the rendering branches in the test file (mock wagmi the same way; render each page; snapshot the CTA's top ratio against iPhone-SE 375×667 + Pixel-7 412×915). **Acceptance:** thumb-zone audit grows from 6 to 10 cases (5 routes × 2 viewports); all pass `MIN_THUMB_ZONE_RATIO`; if HiLo or Dice fails, fix the layout (anchor primary CTA to bottom 30%, mirroring Coinflip's `marginTop: auto`). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_HILO_CARD_VISUAL]** Operator-set "super clean, nice-looking UI" rule. UX_PLAN §1 surface table calls Hi-Lo a *card-flip widget*; UX_PLAN §5 game-screen anatomy lists the card flip as the differentiator vs Coinflip. Today the HiLo page renders the current card as a plain `<span>` digit (1..13). Build a real card visual: rank (`A 2..10 J Q K`) + suit glyph (deterministic from `(sessionId, stepCount)` so it's stable; spec the suit-mapping rule in code so the verify page can replay it), card-flip transition on `stepPlayed` (CSS-only is fine — `transform: rotateY()` + `transition: transform 600ms cubic-bezier(0.22, 1, 0.36, 1)`, capped at 600ms per UX_PLAN §6 motion rules), reduced-motion fallback that crossfades instead. Component lives under `apps/web/src/components/HiloCard.tsx` so the verify page can reuse it. **Acceptance:** new vitest specs covering rank + suit mapping (rank 1→"A", 11→"J", etc.), flip transition mounts, reduced-motion fallback when `prefers-reduced-motion: reduce` is set; HiLo page test grows from 19 to ≥24; UX_PLAN §5 / §6 motion contract honoured (≤600ms, ease-out-quart). (PROJECT:BUNNYBAGZ)

- [ ] **[BB_PHASE2_DICE_SLIDER_VISUAL]** Operator-set "super clean, nice-looking UI" rule. UX_PLAN §5 game-screen anatomy lists the dice viewport as a *slider*. Today `/play/dice` uses a plain `<input type="number">` for `rollUnder`. Replace with a real range-slider widget covering [2, 98]: tick marks at 5/25/50/75/95, drag handle ≥44px (UX_PLAN §7 touch-target rule), live multiplier + win-prob preview already present (keep them; bind to slider value), keyboard arrow-left/right step ±1 + page-up/down step ±10 + home/end snap to bounds (UX_PLAN §7 keyboard rule), aria-valuemin/max/now wired. Component under `apps/web/src/components/DiceSlider.tsx` for reuse. **Acceptance:** new vitest specs cover keyboard nav, touch-target size assert, aria-valuetext when changing, multiplier preview stays in lockstep with `multiplierFor()` formula; dice page test grows from 10 to ≥16; thumb-zone audit on `/play/dice` re-passes with the slider in place. (PROJECT:BUNNYBAGZ)
