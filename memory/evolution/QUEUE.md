...
- [x] **[ACTION_FAILURE_TAXONOMY_REFINE]** (2026-05-01) Added 8 sub-buckets to `EpisodicMemory.FAILURE_TYPES` (`action.path_missing`, `action.command_nonzero`, `action.assertion_failed`, `action.lint_typecheck_error`, `action.edit_string_not_found`, `action.git_conflict`, `action.test_failed`, plus `action.unverified` for spawned-agent partial-success self-reports). New `_classify_action_subtype()` runs ordered regex over `error + last 50 lines of stderr` with infrastructure reclassifier (auth → `external_dep`, import → `import_error`). `_get_failure_type()` retroactively refines legacy `action`-tagged episodes. `benchmark_episodes()` excludes `action.unverified` from action_accuracy denominator and weakest-mode pick, exposes `top_failure_subtypes` (top 3 concrete sub-buckets) for the PI digest. **Result on live data**: action_accuracy 0.875→0.971, weakest_failure_mode `action`→`external_dep`, real action failures (16 episodes after isolating self-reports) now max at 31.2% per sub-bucket (was 95% in single `action`). 13 new unit tests in `tests/test_clarvis_memory.py::TestActionSubtypeClassifier`. Files: `clarvis/memory/episodic_memory.py`, `scripts/metrics/performance_benchmark.py`, `tests/test_clarvis_memory.py`.
- [ ] **[EDIT_OLDSTRING_FAILURE_TELEMETRY]** Targets weakest metric (Action Accuracy). The `Edit` tool's "old_string not unique / not found" pattern is a high-frequency action failure but is invisible in current taxonomy. In `scripts/pipeline/heartbeat_postflight.py`, scan the Claude Code transcript for the canonical Edit-tool error markers and append a structured row to `data/action_failure_modes.jsonl` (fields: `ts, task_id, marker, file_path_seen`). When count >5 in a rolling 7-day window, auto-emit a P2 queue task to retune Edit-tool prompt templates (prefer larger context windows / `replace_all`). Acceptance: jsonl populates from real episodes; auto-task fires once threshold tripped; no double-fires within 7 days. (PROJECT:CLARVIS)
- [ ] **[CRON_SCHEDULE_DRIFT_AUDIT_BASH]** Non-Python item (bash). Add `scripts/cron/cron_schedule_audit.sh` that diffs three sources of truth: (1) live `crontab -l`, (2) `clarvis cron list` output, (3) the schedule table documented in `CLAUDE.md`. Report unmanaged entries, missing managed entries, and time mismatches into `monitoring/cron_drift.log` with a non-zero exit on drift. Wire it into Sun 05:25 (between worktree cleanup and cleanup). Acceptance: script runs clean today (zero drift baseline); planted-drift unit fixture (extra crontab line) makes it exit non-zero with the offending line in the log. (PROJECT:CLARVIS)
- [ ] **[HEARTBEAT_NOTASK_ROOT_CAUSE_REPORT]** Non-Python item (analysis + markdown). The 04-30 audit flagged a 46% heartbeat no-task rate. Read the last 200 preflight skip reasons from `data/heartbeat_skips.jsonl` (or equivalent log), bucket by reason (queue_empty, attention_below_threshold, all_items_blocked, lock_held, precheck_fail, etc.), and write `docs/internal/audits/HEARTBEAT_NO_TASK_RATE_2026-04-30.md` with: per-bucket counts, top 3 fixable buckets, one concrete remediation per top bucket. No code changes — pure analysis output. Acceptance: report exists, totals reconcile against the source log within ±2, each remediation cites a specific file path or config knob. (PROJECT:CLARVIS)
