...
- [x] **[BB_PHASE3_RECENT_OUTCOMES_CROSS_GAME]** Shipped 2026-05-10. Truth-audit gap #2 closeout. Production code was already done (strip mounted on /play/dice:382 + /play/hilo:620 with respective `game` props; WalletSheet renders cross-game `<RecentBetsList>` via `<WalletPanel>` against `/api/history/wallet` aggregator), but the test pins were missing. Added 8 vitest cases (target ≥6): per-game `data-game` mount assertion in coinflip + dice + hilo page tests (3 cases); WalletSheet × cross-game `<RecentBetsList>` integration (5 cases — mount-inside-sheet, aggregator URL shape, mixed-game fixture row count + indexer-order preservation, per-row game tag + outcome chip + stake column, disconnected → empty-state + no fetch). web 616/616, api 159/159, indexer 38/38, web typecheck clean. Forge not run (forge binary not on PATH; no contract files touched, so no contract regression possible). Files: `apps/web/src/app/play/{coinflip,dice,hilo}/__tests__/page.test.tsx` (+8 LOC each), `apps/web/src/components/__tests__/WalletSheet.test.tsx` (+5 cases, +112 LOC). (PROJECT:BUNNYBAGZ) (2026-05-10)
...
- [ ] **[BB_PHASE3_KBNAV_FOCUS_RING_FIX]** Non-Python (CSS + vitest). Source: `memory/cron/bb_keyboard_nav_audit_2026-05-09.md` defect D1 (CRITICAL). Add a single global `:focus-visible` rule in `apps/web/src/app/globals.css` so every interactive surface (button, a, summary, [role=button]) paints a 2-px `var(--bb-brand-gold)` outline with 2-px offset, plus a 1-px ink inner-ring on the gold CTAs themselves so they hit AA contrast in both themes (gold-on-gold otherwise fails). Verify on six surfaces: `apps/web/src/app/play/coinflip/page.tsx:494-505` (primaryButtonStyle), `…/coinflip/page.tsx:475-492` (sideStyle/sideSelectedStyle), `apps/web/src/app/play/dice/page.tsx:528`, `apps/web/src/app/play/hilo/page.tsx:528`, `apps/web/src/components/HeroGameCard.tsx:207-211` (cardLinkStyle), `apps/web/src/components/WalletSheet.tsx:344` (closeButtonStyle). **Acceptance:** (a) new vitest case in `apps/web/src/components/__tests__/FocusRing.test.tsx` (≥6 cases) asserts `getComputedStyle(el).outlineColor !== 'rgba(0, 0, 0, 0)'` and `outlineWidth ≥ 2px` after `el.focus()` for every `data-testid="*-primary-cta"` element + lobby card link + WalletSheet close, in both `data-theme="dark"` and `data-theme="light"`; (b) re-run `apps/web/scripts/keyboard-nav-audit.mjs` and confirm `summary.json` shows non-zero outline width on every focused CTA in dark theme; (c) full web suite green; (d) Playwright e2e `apps/web/e2e/focus-ring.spec.ts` ships ≥3 specs (lobby card, coinflip CTA, WalletSheet close) running with `page.keyboard.press('Tab')` and asserting visible outline via `axe-core` keyboard rule. Closes audit P0 in `bb_keyboard_nav_audit_2026-05-09.md`. (PROJECT:BUNNYBAGZ)
