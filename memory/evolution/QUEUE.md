...
- [x] **[DAILY_LOG_COVERAGE_GUARD]** Done 2026-04-30: extended `scripts/tools/daily_memory_log.py` with `audit_coverage()` that scans the trailing 7 closed UTC days, treats both `.md` and `.md.gz` as present (so post-cleanup days aren't false positives), backfills a flagged stub for any gap, and writes a `[ALERT] Daily log missing` line to `monitoring/alerts.log` on first observation (idempotent via `data/daily_log_audit_state.json`). `ensure_daily_log()` now calls the audit on every health-monitor tick, so a missed day surfaces within 15 minutes. Tests: `tests/scripts/test_daily_log_coverage_guard.py` (7 passing). (PROJECT:CLARVIS)
...
- [x] **[DIGEST_LINT_PRECOMMIT]** Non-Python regression guard for the digest writer pathway, complementing `[DIGEST_GARBLE_FIX]` (root cause) and `[DIGEST_ARCHIVE_IMPLEMENTATION]` (history). Done 2026-04-30: `scripts/cron/digest_lint.sh` checks (a) `,n  [a-z_]+:` chains, (b) brace balance outside fenced blocks, (c) mid-word EOF truncation; `digest_writer.py` runs it post-write, rolls back, and re-renders payload in a fenced code block on non-zero exit. `tests/test_digest_lint.sh` exercises 7 cases including the 2026-04-25 garbled sample (fail) and a clean sample (pass) plus the wired writer fallback. (PROJECT:CLARVIS)
