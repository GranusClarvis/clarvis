...
## Archived 2026-03-23
- [x] [CLR_ORACLE_RETRIEVAL_MODE] Add an evaluation mode to CLR-Benchmark adapters that uses gold/oracle evidence when available. Purpose: separate retrieval failure from reasoning/reading failure so improvements are scientifically interpretable. _(2026-03-23: Added gold_evidence to all 20 benchmark pairs, `_oracle_recall()` function, `oracle` param to `run_benchmark()`, `run_oracle_comparison()` for gap analysis, CLI `golden-qa-oracle` command. Oracle mode: 20/20 recall, MRR=1.0.)_
- [x] [MEMBENCH_ADAPTER_REFLECTIVE_OBSERVATION] Build a `clarvis bench membench` adapter covering the four MemBench quadrants: participation-factual, participation-reflective, observation-factual, observation-reflective. Report effectiveness, recall, capacity, and temporal efficiency splits. _(2026-03-23: Built clarvis/metrics/membench.py with 20 tasks across 4 quadrants. CLI: `clarvis bench membench [--quadrant X] [--oracle]`. Oracle mode: 100% effectiveness. Real retrieval: 45% effectiveness — retrieval is the bottleneck, not evidence quality. Saves to data/benchmarks/membench_*.json.)_
- [x] [ACTION_ACCURACY_REGRESSION_GUARD] Add an action-accuracy regression test to the heartbeat postflight: if trailing-20 action accuracy drops below 0.95, auto-push a P1 diagnostic task to QUEUE.md with the failing action IDs. Prevents silent regression of the current 0.979 score. _(2026-03-23: implemented as §7.495 in heartbeat_postflight.py. Computes trailing-20 AA excluding soft_failures and timeouts. Tested with real episodic data — current AA=1.0, guard correctly silent.)_

## Archived 2026-03-23
- [x] [LONGMEMEVAL_ADAPTER_AND_BASELINE_RUN] _(2026-03-23: Built `clarvis/metrics/longmemeval.py` — 25 tasks across 5 abilities (IE/MR/KU/TR/ABS), full-history + oracle modes, stage diagnostics, CLI `clarvis bench longmemeval` + `longmemeval-oracle`. Baseline: 88% eff, 56% P@1. Oracle comparison: 3 pure retrieval failures (IE03, IE05, KU05), 0 reasoning failures.)_
- [x] [CLR_SPLIT_INTERNAL_VS_BENCHMARK] _(2026-03-23: Split done. `clr.py` → CLR-Internal (architecture health, 7 dims). New `clr_benchmark.py` → CLR-Benchmark (external task evaluation, 8-ability taxonomy from LongMemEval+MemBench). CLI: `clarvis metrics clr` (internal) vs `clarvis metrics clr-benchmark` (external). Baseline: 71.1% eff across 45 tasks.)_
- [x] [CLR_FAILURE_STAGE_BREAKDOWN] _(2026-03-23: Built into `longmemeval.py` StageDiagnostics class — tracks retrieval, evidence_quality, answer stages per task. Oracle mode feeds directly: normal vs oracle comparison isolates retrieval failures from reasoning failures. Per-ability stage breakdown via `stage_by_ability`. CLR-Benchmark surfaces stage diagnostics from LongMemEval.)_
...
## Archived 2026-03-24
- [x] [LLM_BRAIN_REVIEW 2026-03-24] [LLM_BRAIN_REVIEW] Investigate why temporal/recency queries return only 3 results with poor relevance. _(2026-03-24: Root cause identified — created_at stored as ISO string, ChromaDB where-clause needs numeric. Post-query filtering decimates results: 1 of 64 recent episodes survives. Fix plan: add created_epoch int metadata + backfill + over-fetch multiplier + chronological fallback. See memory/research/temporal_retrieval_fix_2026-03-24.md)_
- [x] [TEMPORAL_RETRIEVAL_FIX] Implement temporal retrieval fixes: (1) Add created_epoch int metadata to store.py, (2) Backfill all 2295 existing memories, (3) Use ChromaDB where-clause for temporal filtering in search.py, (4) Add over-fetch 3x multiplier when since_days set, (5) Chronological fallback for pure temporal queries, (6) Auto-increase n to 15 for temporal queries. _(2026-03-24: All 5 fixes implemented + backfill applied. "what happened recently" now returns 45 results (was 1-3). "tasks completed" with since_days=3 returns 135. Both store.py files updated. 25 tests pass. Brain healthy.)_
...
## Archived 2026-03-26
- [x] [EPISODE_CRASH_GUARD] Harden `cron_autonomous.sh` against instant-fail episodes (5 failures on 3/25 at 2-4s duration — task selection crashed before execution). Add pre-execution validation: verify selected task is non-empty, queue file parseable, graceful fallback when queue empty. _(Done 2026-03-26: queue file validation, eval error catch, task length check, instant-fail detection <10s→crash status, postflight excludes crashes from accuracy/evolution)_
...
- [x] [E3_BENCHMARK_RECORD] Run `performance_benchmark.py record` and `cron_clr_benchmark.sh` — record final PI and CLR scores. Both above thresholds. _(Completed 2026-03-29 UTC: PI=1.0000, brief_compression=0.615, Phi=0.8304, CLR=0.815, value_add=0.625; both gates passed.)_
- [x] [E4_CRON_AUDIT] All cron jobs ran successfully in the last 24h (no stale locks, no failures in watchdog logs). _(Verified 2026-03-29 22:00 UTC: watchdog 0 failures, all 14 monitored jobs OK, no stale locks. Advisory: brain_hygiene.py persistently failing since 03-22 due to OpenBLAS/NPROC contention — fix: set OPENBLAS_NUM_THREADS=1 in cron_env.sh. 183 transient fork-retry errors during peak cron, all recovered.)_
...
- [x] [DECOMP_PLAN_P0_SEARCH_FIX] Fix `clarvis/brain/search.py` text-search fallback regression. _(Fixed: added `query` param to `_query_single_collection`, plumbed through `_dispatch_collection_queries`. Regression test added. 2026-03-29.)_
- [x] [DECOMP_PLAN_P1_RECOMBINE] Recombine audit-flagged over-fragmented helpers. _(Merged `_apply_recency_scores`+`_sort_results` into `_score_and_sort`, inlined `_finalize_recall` into `recall()`, inlined `_preflight_reasoning_chain`/`_preflight_routing`/`_preflight_compress_episodes`, merged `_score_*` helpers into `_assess_quality()`, merged `_pf_prompt_optimization`+`_pf_prediction_resolve` into `_pf_prompt_and_prediction`. 2026-03-29.)_
- [x] [DECOMP_PLAN_P2_CLEANUPS] Minor cleanups. _(Added logging to `_mark_task_in_queue` exception, refactored `_preflight_assemble_context` from 16 positional args to context dict, expanded cryptic locals in `_weighted_means`. 2026-03-29.)_
- [x] [DECOMP_PLAN_PHASE0_QUEUE_WRITERS] Audit all queue-writing call sites. _(18 callers verified safe — none encode structural/line-count criteria. Findings documented in plan. 2026-03-29.)_
- [x] [DECOMP_PLAN_PHASE2_METRIC] Metric replacement. _(Raised `reasonable_function_length` threshold 100→200. Added separate advisory `structural_complexity_risk()` with 3-signal model. Does not feed PI or quality scoring. 2026-03-29.)_
- [x] [DECOMP_PLAN_PHASE3_POSTFLIGHT] Postflight complexity gate replacement. _(Replaced raw 80-line gate with role-aware `structural_complexity_risk` analysis on changed files. Emits `data/structural_review.json` artifact. Report-only, no queue mutation. 2026-03-29.)_
- [x] [DECOMP_PLAN_PHASE5_GOVERNANCE] Queue governance. _(Added provenance-based gate in `queue_writer.add_tasks()`: structural refactor tasks blocked from auto sources, require manual/audit/user/cli source. Pattern-based + source-based dual gate. 2026-03-29.)_
- [x] [DECOMP_PLAN_PHASE6_FIREWALL] Structural-risk firewall. _(Verified `structural_complexity_risk` is architecturally isolated from PI/quality scoring. Added 2 firewall tests. Documented invariant in code and plan. 2026-03-29.)_
- [x] [STRATEGIC_AUDIT 2026-03-28] [STRATEGIC_AUDIT/metric_integrity] Raise TARGET_DEGREE to 25 or increase semantic_overlap weight to 0.50 so Phi reflects real integration quality. _(Raised TARGET_DEGREE 10→25 in clarvis/metrics/phi.py. Phi dropped 0.8304→0.7337. intra_collection_density now 0.4358 (was near-saturated), revealing real integration gaps. 2026-03-29.)_
- [x] [STRATEGIC_AUDIT 2026-03-28] [STRATEGIC_AUDIT/metric_integrity] Investigate CLR ablation methodology. Use harder test tasks, remove scoring floors, or test with 2+ components ablated simultaneously to find interaction effects. _(Rewrote clr_perturbation.py v2: (1) Added assembly-sensitive measurement via `_measure_assembly_quality()` — runs `generate_tiered_brief()` and measures section diversity + volume, fixing fundamental flaw where CLR dimensions were insensitive to budget changes. (2) Added 4 harder reference tasks requiring multiple cognitive modules. (3) Added `--pairs` flag for 2-module combinatorial ablation with interaction effect detection (synergy vs redundancy). (4) 7 new tests, all pass. First real run revealed: all modules show +0.10 AQ delta when removed (suggesting noise), strong synergy effects in pairs. Schema bumped to v2.0. 2026-03-29.)_
- [x] [RESEARCH_PRECISION_WEIGHTED_ACTIVE_INFERENCE_CONSCIOUSNESS] Precision-weighted active inference and consciousness. _(Completed 2026-03-29.)_
- [x] [RESEARCH_PHI_COMPUTATION_IIT] Phi computation in Integrated Information Theory, approximations, and tractability limits. _(Completed 2026-03-29.)_
...
## Archived 2026-04-02
- [x] [STRATEGIC_AUDIT 2026-04-01] [STRATEGIC_AUDIT/metric_integrity] [CLR_OUTCOME_ABLATION_V3] Build v3 ablation harness measuring actual task outcome quality (LLM-judged blind comparison) across 20+ diverse test tasks. Must show non-uniform deltas or confirm modules are redundant. ✅ 2026-04-02: Built `clarvis/metrics/ablation_v3.py` — 24 tasks × 6 categories, LLM blind A/B + offline deterministic mode, post-hoc section stripping, per-category heatmap, uniformity analysis. Result: NON_UNIFORM — decision_context & graph_expansion CRITICAL, working_memory neutral.
- [x] (2026-04-02) [RESEARCH 2026-04-02] [IMPL] [COORDINATOR_PROMPT_TEMPLATES] Create 3 worker prompt templates (research/implementation/maintenance) in scripts/worker_templates/. Wire cron_autonomous.sh to select and prepend template based on preflight worker_type field. ✅ Created 3 templates + wired classification in preflight JSON parsing + template injection in run_claude_code().
- [x] [RESEARCH_PHI_SCALABLE_APPROXIMATION_HEURISTICS] Study scalable approximation strategies for integrated information Phi and extract practical lessons for agent-memory/consciousness architectures. (2026-04-02)
...
- [x] [INSTALL_VERIFY_NO_BRAIN_FALSE_FAILURE] Fix fresh-clone install verification so `bash scripts/setup.sh --no-brain --verify` does not falsely fail. `verify_install.sh` currently treats `import clarvis.brain` as a mandatory PASS even though setup explicitly advertises a no-brain install mode. _(Done 2026-04-02: verify_install.sh accepts --no-brain, setup.sh passes it through; brain+metrics imports downgraded to warn_check.)_
- [x] [CRON_LOG_REFLECTION_CAP] Add size-based rotation to `cron_cleanup.sh` for `memory/cron/*.log` files — `reflection.log` alone is 1.1 MB across 3 rotated copies, `autonomous.log` is 480K+. Current rotation is numeric suffix only with no size cap or max-generations policy. Implement: keep at most 2 rotated copies, gzip .log.2+, delete .log.3+. _(Done 2026-04-02: cleanup_policy.py MAX_ROTATED_COPIES 3→2, rotate_log gzips .log.2→.log.2.gz, purges .log.3+. Tested: reflection.log.2 deleted, .1 gzipped to .2.gz at 90% compression.)_

## Archived 2026-04-02
- [x] [RESEARCH_PHI_COMPUTATION] Research practical Phi computation, exact vs approximate measures, and implications for scalable system analysis. (2026-04-02)

## Archived 2026-04-02
- [x] [VERIFY_INSTALL_ROOT_DISCOVERY_FIX] Fix `scripts/verify_install.sh` so install verification exercises the canonical root pytest discovery path (or curated equivalent), not just `tests/test_open_source_smoke.py`. Current flow can still report green while `clarvis/tests/` or `packages/*/tests/` regress. _(Done 2026-04-02: verify_install.sh now runs full pytest collection + per-package test runs)_
- [x] [OSR_TEST_CONSOLIDATION_PLAN] Document the test fragmentation (4 locations: `tests/`, `clarvis/tests/`, `packages/*/tests/`) and decide on a consolidation strategy. At minimum: ensure `python3 -m pytest` from root discovers all tests. _(Done 2026-04-02: Root pytest already discovers all 2082 tests across `tests/` and `packages/`. `clarvis/tests/` does not exist — test locations are `tests/` (1931) + `packages/*/tests/` (151). Verification now confirms both locations. No consolidation needed.)_
- [x] [OSR_GUIDED_INSTALLER] Build a guided install flow for fresh users (`install.sh` and/or `python3 -m clarvis install`) with clear step-by-step prompts, environment checks, and sane defaults. It should feel like a product installer, not a pile of shell commands. _(Done 2026-04-02: `scripts/install.sh` — interactive profile selection, env checks, PEP 668 handling, auto-verification. 4 profiles: standalone, openclaw, fullstack, docker.)_
- [x] [OSR_HARNESS_SELECTION_MATRIX] Add harness selection during install for the top 4 prominent deployment modes. Start with `OpenClaw`, `Hermes`, and 2 additional well-supported options after a repo/ecosystem audit. Each option must explain what it installs, prerequisites, and which Clarvis features are enabled/disabled. _(Done 2026-04-02: 4 profiles — Standalone, OpenClaw, Full Stack, Docker. Hermes was researched but rejected as incompatible with dual-layer architecture. Full comparison matrix in docs/INSTALL.md.)_
- [x] [OSR_HARNESS_PROFILE_TEMPLATES] Create config/profile templates for each supported harness option so the guided installer can lay down the right files, env vars, and startup commands without manual editing. _(Done 2026-04-02: `config/profiles/{standalone,openclaw,fullstack,docker}.env` — installer auto-creates `.env` from profile template.)_
- [x] [OSR_CRON_INSTALLER] Add guided cron/system-scheduler installation. _(Done 2026-04-02: `clarvis cron install <preset>` with dry-run preview (default), --apply to commit, sentinel-wrapped blocks for safe update/remove. `clarvis cron remove` to clean up. Backs up crontab before changes. Also generated `scripts/crontab.reference` from live crontab.)_
- [x] [OSR_CRON_PROFILE_PRESETS] Define cron presets mapping to actual scripts/schedules. _(Done 2026-04-02: 4 presets — minimal (12 jobs, no Claude spawning), recommended (27 jobs, 4x autonomous), full (46 jobs, complete schedule), research (40 jobs, extra research + dream + benchmarks). View with `clarvis cron presets`.)_
- [x] [OSR_FIRST_RUN_VALIDATION] After install, run a first-run validation suite that checks env, writable paths, selected harness integration, enabled cron jobs, and one smoke execution path. Must end with a human-readable PASS/WARN/FAIL summary. _(Done 2026-04-02: verify_install.sh now runs 28 checks — core imports, CLI, brain, canonical pytest discovery, per-package tests, profile-specific checks. Clear PASS/WARN/FAIL summary.)_
- [x] [OSR_INSTALL_DOCS_WALKTHROUGH] Write a proper install walkthrough for new users: prerequisites, harness choice guide, cron choice guide, first-run validation, and troubleshooting. Match the guided installer exactly so docs and reality stay in sync. _(Done 2026-04-02: `docs/INSTALL.md` — prerequisites, profile comparison matrix, guided installer flow, non-interactive mode, troubleshooting. README.md and CONTRIBUTING.md updated to reference.)_
- [x] [JSONL_ROTATION_POLICY] Implement automatic rotation/GC for unbounded JSONL files. _(Done 2026-04-02: Extended cleanup_policy.py from 9 to 44 explicit JSONL trim entries + auto-discovery for any unlisted .jsonl >500KB. Covers hebbian/access_log, retrieval events, dashboard events, conflict log, broadcast, calibration, benchmarks, etc.)_
- [x] [CRON_EVENING_MISSING_SCRIPT_FIX] Fix `cron_evening.sh` which references deleted `code_quality_gate.py`. _(Done 2026-04-02: Removed dead call to code_quality_gate.py — script was deleted 2026-03-18 in commit 0eed458, call was silently failing via || true.)_
- [x] [VERIFY_TEST_DISCOVERY_GAP] Fix install verification coverage gap: `verify_install.sh` only runs `tests/test_open_source_smoke.py`, so package tests under `clarvis/tests/` and `packages/*/tests/` can silently regress while verification still passes. Make the verifier exercise the canonical root test discovery path or an equivalent curated smoke matrix. _(Done 2026-04-02: merged with VERIFY_INSTALL_ROOT_DISCOVERY_FIX — verify_install.sh now runs canonical pytest collection + per-package tests)_
- [x] [REASONING_CHAIN_ROTATION] Implement archival policy for `data/reasoning_chains/`. _(Done 2026-04-02: data_lifecycle.py already archives chains >14d and sessions >30d — fixed missing datetime import that crashed the JSON summary. 144 chains pending archival on next Sunday run.)_
- [x] [CLEANUP_POLICY_JSONL_COVERAGE] Extend `cleanup_policy.py` to cover all unbounded JSONL files. _(Done 2026-04-02: Explicit JSONL_TRIM expanded from 9→44 entries. Added auto-discovery scan: any unlisted .jsonl in data/ >500KB gets trimmed to 1000 lines. Cron log rotation expanded from 16→41 entries covering all known logs.)_
...
## Archived 2026-04-03
- [x] [OSS_HARDCODED_PATHS] _(2026-04-03: Fixed 9 spine modules (15 hardcoded paths → env-fallback): brain/graphrag.py, brain/retrieval_quality.py, cognition/reasoning.py (×2), cognition/somatic_markers.py (×2), learning/meta_learning.py (×8), memory/synaptic_memory.py (×2), memory/soar.py, memory/cognitive_workspace.py (×2), cli_context.py. All spine paths now resolve via `CLARVIS_WORKSPACE` env var. Remaining: ~140 scripts/ files still hardcoded — lower priority, not imported by spine.)_
...
- [x] [ROADMAP_MULTI_AGENT_PARALLEL] _(2026-04-03: Implemented `cmd_spawn_parallel()` in `project_agent.py` + CLI `spawn-parallel --tasks '[...]'`. Uses ThreadPoolExecutor bounded by MAX_PARALLEL_AGENT_CLAUDE (default 3). Concurrency infrastructure already existed (per-agent locks + global semaphore slots). Returns aggregated results with success/fail counts.)_
- [x] [ROADMAP_MEMORY_EVOLUTION] _(2026-04-03: Verified complete. `clarvis/brain/memory_evolution.py` (261L) implements all 3 A-Mem features: `record_recall_success()` (importance boost on retrieval), `evolve_memory()` (supersession with `evolved_from`/`superseded_by` + graph link), `find_contradictions()` (negation asymmetry + low-Jaccard detection). Wired into `heartbeat_postflight.py`. 18/18 tests pass.)_
...
- [x] [EXTERNAL_CHALLENGE:bench-reasoning-01] _(Done 2026-04-04)_ ARC-AGI solver at `scripts/arc_solver.py`: 6 rule extractors (geometric transforms, color mapping, scaling, fill-enclosed, crop, composite), 5 built-in puzzles (color_swap, rotate_90, flip_horizontal, fill_enclosed, scale_2x), all solved correctly. 25 tests in `tests/test_arc_solver.py`. CLI: `arc_solver.py bench`.
...
## Archived 2026-04-04
- [x] [LLM_BRAIN_REVIEW 2026-04-04] [LLM_BRAIN_REVIEW] Investigate temporal retrieval — consider adding date-tagged metadata or a dedicated 'recent activity' collection with TTL-based eviction so recency queries return meaningful results — Temporal queries have failed across multiple review cycles. This is a structural limitation, not a content gap — embedding similarity cannot capture recency without explicit temporal signals. (2026-04-04 07:05 UTC)
- [x] [LLM_BRAIN_REVIEW 2026-04-04] [LLM_BRAIN_REVIEW] Store and weekly-refresh a single authoritative 'current priorities' memory in clarvis-goals — Done 2026-04-04: `scripts/refresh_priorities.py` created, memory upserted (ID=current-priorities, importance=0.95), Sunday 05:05 cron installed.
- [x] [RESEARCH_PHI_COMPUTATION] Investigate Phi computation in Integrated Information Theory — exact scalability limits, approximations, and current tooling. (2026-04-04)

## Archived 2026-04-05
- [x] [PATH_HYGIENE_HEREDOC_LITERAL_ENV_BUG] Fix shell heredoc/Python snippets that now embed literal `$CLARVIS_WORKSPACE` / `$OPENCLAW_HOME` strings inside Python because the heredoc is quoted or the value is inserted as a literal string. Resolve via env reads inside Python or unquoted shell interpolation where safe. (2026-04-05 fixed: 7 files, 11 literal refs → os.environ lookups)
...
- [x] [QUEUE_V2_RECONCILE_HYGIENE] Harden reconcile/soak behavior so recent manual edits to QUEUE.md never produce transient sidecar drift or stale completed/orphan task entries after archive/manual cleanup. (2026-04-05 fixed: soak_check reconciles first, writer syncs sidecar on add/complete/archive)
